...
run_asan() {
    ...
    cmake --fresh --preset sanitizers || return 1
    cmake --build --preset sanitizers --parallel || return 1
    ASAN_OPTIONS="detect_leaks=0:halt_on_error=1" \
        UBSAN_OPTIONS="halt_on_error=1:print_stacktrace=1" \
        ctest --preset sanitizers --timeout 120 || return 1
...
run_lsan() {
    ...
    clang++ -std=c++17 -g -fno-omit-frame-pointer -fsanitize=address,undefined \
        "$repository_root/xWalkTool/quality/probes/xWalkLeakSanitizerProbe.cpp" \
    ...
    cmake --fresh --preset leak-sanitizer || return 1
    cmake --build --preset leak-sanitizer --parallel || return 1
    ASAN_OPTIONS="detect_leaks=1:halt_on_error=1" \
        UBSAN_OPTIONS="halt_on_error=1:print_stacktrace=1" \
        ctest --preset leak-sanitizer --timeout 120 >"$repository_root/build-host/leak-sanitizer/ctest.log" 2>&1
    local test_status=$?
...
run_tsan() {
    ...
    cd "$repository_root" || return 1
    cmake --fresh --preset thread-sanitizer || return 1
    cmake --build --preset thread-sanitizer --parallel || return 1
    local test_log="$repository_root/build-host/thread-sanitizer/ctest.log"
    TSAN_OPTIONS="halt_on_error=1:history_size=7" \
        ctest --preset thread-sanitizer --timeout 120 \
