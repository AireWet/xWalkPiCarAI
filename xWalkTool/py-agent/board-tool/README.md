...
The installable distribution lives under `xWalkTool/py-agent/board-tool`, while its import package lives under
`xWalkTool/py-agent/board-tool/py-src/xWalkJiraImport`. Executable Python development tools and their host tests are
stored in `xWalkTool/py-agent/dev-tool`, while `xWalkTool/py-agent` contains Python environment guidance.
...
python -m pip install ./xWalkTool/py-agent/board-tool
```

Use an editable installation while developing the importer:

```sh
python -m pip install --editable ./xWalkTool/py-agent/board-tool
```
...
```sh
PYTHONPATH=xWalkTool/py-agent/board-tool/py-src python3 -m unittest discover -s xWalkTool/py-agent/board-tool/test -p 'test_xWalkJiraImport*.py'
```
