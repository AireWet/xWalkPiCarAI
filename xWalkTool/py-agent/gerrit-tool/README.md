...
reconnect, the worker compares the configured Gerrit and GitHub integration
branch tips. When Gerrit is ahead because a live merge event was missed, it
recovers the exact merged change metadata and runs the same guarded uplift
before resuming the event stream. Matching branch tips do not create a
duplicate uplift entry.
