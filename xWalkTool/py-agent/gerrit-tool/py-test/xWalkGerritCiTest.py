...
class XWalkGerritQualityTest(unittest.TestCase):
    ...
    def test_compiler_uses_integrated_product_cmake_source(self) -> None:
        """Configure the relocated product instead of the repository tool root."""

        quality = XWalkGerritQuality(pathlib.Path("/workspace"), io.StringIO())
        configure = quality.compiler_steps("gcc", "Debug")[0][0]
        source_index = configure.index("-S") + 1
        self.assertEqual(configure[source_index], "/workspace/xWalk-rpi5")

    def test_preset_job_runs_from_integrated_product(self) -> None:
        """Resolve relocated CMake presets from the xWalk-rpi5 directory."""

        quality = XWalkGerritQuality(pathlib.Path("/workspace"), io.StringIO())
        with mock.patch.object(quality, "run_steps", return_value=True) as run_steps:
            self.assertTrue(quality.preset_job("stress-tests", "sanity", "host-stress"))
        self.assertEqual(run_steps.call_args.args[2], pathlib.Path("/workspace/xWalk-rpi5"))
    ...
    def test_tsan_uses_non_root_namespace_with_loopback(self) -> None:
        ...
        self.assertIn("xWalkTool/shell-agent/quality-tool/run-host-sanitizer.sh", command[-1])
