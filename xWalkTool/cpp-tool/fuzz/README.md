...
```bash
CC=clang CXX=clang++ cmake -S xWalk-rpi5 -B build-host/fuzz -G Ninja -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=OFF -DXWALK_BUILD_FUZZERS=ON
```

Run a short smoke pass for one target with:

```bash
build-host/fuzz/xWalkTool/cpp-tool/fuzz/xWalkFuzz_http_requests -runs=1000 xWalkTool/cpp-tool/fuzz/corpus/http_requests
```
