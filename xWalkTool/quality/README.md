...
```bash
sudo apt update
sudo apt install -y clang clang-tools llvm gcovr lcov valgrind shellcheck
```

Inspect paths, versions, and availability without changing the machine:

```bash
xWalkTool/shell/check-host-quality-dependencies.sh
```
...
```bash
xWalkTool/shell/run-host-sanitizer.sh asan
xWalkTool/shell/run-host-sanitizer.sh lsan
xWalkTool/shell/run-host-sanitizer.sh tsan
```
...
```bash
xWalkTool/shell/run-host-coverage.sh run gcc
xWalkTool/shell/run-host-coverage.sh run clang
```
...
percent respectively in `xWalkTool/environment/gcovr.cfg`. The report includes
...
```bash
xWalkTool/shell/run-host-valgrind.sh
```
...
```bash
xWalkTool/shell/run-clang-static-analyzer.sh
```
...
```bash
xWalkTool/shell/run-host-shellcheck.sh
xWalkTool/shell/run-host-quality.sh
```
