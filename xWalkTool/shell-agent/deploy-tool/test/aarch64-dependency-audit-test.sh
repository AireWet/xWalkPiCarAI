...
audit_script="$repository_root/xWalkTool/shell-agent/deploy-tool/aarch64-dependency-audit.sh"
...
apply_fake_pkg_config()
{
    ...
    sed "s|@SYSROOT@|$sysroot|g; s|@MODE@|$mode|g" \
        "$repository_root/xWalkTool/shell-agent/deploy-tool/test/fixtures/aarch64-fake-pkg-config.sh.in" \
        > "$fake_pkg_config"
