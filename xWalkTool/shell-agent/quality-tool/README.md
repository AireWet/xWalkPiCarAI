...
root. The wrappers configure the product from `xWalk-rpi5` and keep generated
...
```bash
bash xWalkTool/shell-agent/quality-tool/test/validate-valgrind-descriptors-test.sh
```
...
`xWalkTool/shell-agent/gerrit-tool/run-host-ci-job.sh` instead.
