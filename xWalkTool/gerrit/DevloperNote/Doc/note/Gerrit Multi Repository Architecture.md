...
Copy and edit `xWalkTool/gerrit/config/multi-repo.conf`. Resolve the Gerrit
...
```bash
export XWALK_GERRIT_MULTI_REPO_CONFIG="$PWD/xWalkTool/gerrit/config/multi-repo.conf"
```
...
```bash
export XWALK_CONFIRM_PROVISION="CREATE_XWALK_REPOSITORIES"; export XWALK_CONFIRM_ACL="APPLY_XWALK_ACL"; xWalkTool/gerrit/shell-script/gerrit-multi-repo-provision.sh --apply
```
...
```bash
xWalkTool/gerrit/shell-script/gerrit-history-split.sh --dry-run
```

```bash
export XWALK_SPLIT_OUTPUT_DIR="/safe/new/component-splits"; export XWALK_CONFIRM_SPLIT="SPLIT_COMPONENT_HISTORY"; export XWALK_IMPORT_MODE="none"; xWalkTool/gerrit/shell-script/gerrit-history-split.sh --apply
```
...
```bash
xWalkTool/gerrit/shell-script/gerrit-submodule-migrate.sh --dry-run
```
...
```bash
xWalkTool/gerrit/shell-script/gerrit-auto-uplift.sh --dry-run xWalkHal FULL_COMMIT_ID GERRIT_CHANGE TOPIC
```
...
```bash
xWalkTool/gerrit/shell-script/gerrit-github-sync.sh --dry-run
```
...
```bash
xWalkTool/gerrit/shell-script/gerrit-permission-check.sh --dry-run
```
