...
void testCommands(ctrl::stringview configPath)
{
    ...
    assert(xwalk::ctrl::XWALK_runControllerCommand(cli,
        {"sound", "play", "../xWalkAudioResources/sounds/car-double-horn.wav", "--volume", "80"}) == 0);
    assert(backend.soundRequest.operation == xwalk::ctrl::XWalkSoundOperation::Play);
    assert(backend.soundRequest.filePath == "../xWalkAudioResources/sounds/car-double-horn.wav");
    ...
    assert(xwalk::ctrl::XWALK_runControllerCommand(cli, {"sound", "music",
        "../xWalkAudioResources/music/slow-trail-Ahjay_Stelino.mp3"}) == 0);
