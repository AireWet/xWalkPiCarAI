...
::ctrl::int32 XWalkController::XWALK_handlerVideoRecording(
  ...
  if (started == false) {
    XWALK_CTRL_ERROR(XWALK_EXCEPTION,
                     "Video-recording camera could not be started");
    return 2;
  }

  XWALK_CTRL_TRACE_UID0(
      CTRL .071, "Recording keys: q start/pause/continue; e stop; x exit.");
  ...
  while (processingLoopRequested) {
    ...
    if (result.event == agent::XWalkVideoRecordingEvent::Started) {
      ...
      XWALK_CTRL_TRACE_UID1(CTRL .075, "The video saved as %s",
                            result.videoPath.c_str());
    }
