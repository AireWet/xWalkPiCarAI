...
::ctrl::int32 XWalkController::XWALK_handlerCalibration(
  ...
  if (request.mode == XWalkCalibrationMode::Grayscale) {
    ...
    XWALK_CTRL_TRACE_UID1(CTRL .086, "%s",
                          calibrated ? "Grayscale calibration complete!"
                                     : "Grayscale calibration incomplete");
    return calibrated ? 0 : 2;
  }
  if (request.mode == XWalkCalibrationMode::ServoMotor) {
    XWALK_CTRL_TRACE_UID0(CTRL .005,
                          "=== PiCar-X Servo and Motor Calibration ===");
    XWALK_CTRL_TRACE_UID0(
        CTRL .006,
        "Raise the wheels before accepting any motor verification prompt.");
    const ::ctrl::boolean verified = calibrateServoMotor(true);
    XWALK_CTRL_TRACE_UID1(CTRL .087, "%s",
  ...
  XWALK_CTRL_TRACE_UID0(
      CTRL .008,
      "This will help you calibrate the steering servo and camera gimbal.");
  ...
  XWALK_CTRL_TRACE_UID1(CTRL .088, "%s",
