 ...
 * @return Zero after stationary sampling or verified actuator calibration; otherwise two.
 */
::ctrl::int32 XWalkController::XWALK_handlerCalibration(
    const XWalkCalibrationRequest& request)
{
    if (request.mode == XWalkCalibrationMode::Grayscale)
    {
        output("=== PiCar-X Grayscale Calibration ===");
        const ::ctrl::boolean calibrated = calibrateGrayscaleReferences();
        output(calibrated ? "Grayscale calibration complete!" :
            "Grayscale calibration incomplete");
        return calibrated ? 0 : 2;
    }
    if (request.mode == XWalkCalibrationMode::ServoMotor)
    {
        output("=== PiCar-X Servo and Motor Calibration ===");
        output("Raise the wheels before accepting any motor verification prompt.");
        const ::ctrl::boolean verified = calibrateServoMotor(true);
        output(verified ? "Servo and motor calibration complete!" :
    ...
    output(completed ? "Calibration complete!" : "Calibration incomplete");
