 ...
 * @brief       Implements the SoundBackgroundMusicHandler command responsibility.
 *
 * @details
...
::ctrl::int32 XWalkController::XWALK_handlerSoundBackgroundMusic(
    const XWalkNoArgumentRequest& request)
{
    static_cast<void>(request);
    if (soundBackgroundMusicObject == nullptr)
    {
        output("Sound-background-music backend unavailable");
        return 3;
    }
    const ::ctrl::boolean started = soundBackgroundMusicObject->start();
    if (started == false)
    {
        output("Sound-background-music resources could not be opened");
        return 2;
    }

    output("Sound/music keys: space horn; c background horn; "
    ...
    while (processingLoopRequested)
    {
        ...
        const ::ctrl::boolean soundCommandHandled =
            static_cast<::ctrl::boolean>(
                soundBackgroundMusicObject->handleKey(key).event ==
            agent::XWalkSoundBackgroundMusicEvent::Cancelled);
    ...
    output("Sound and music stopped");
    return 0;
}
