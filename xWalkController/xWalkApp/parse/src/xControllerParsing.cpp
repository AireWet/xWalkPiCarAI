...
::ctrl::bytevector XWALK_parseHexBytes(::ctrl::stringview text) {
  ...
  const ::ctrl::boolean textXInvalid =
      static_cast<::ctrl::boolean>((text.size() >= 2U) && (text[0U] == '0') &&
                                   ((text[1U] == 'x') || (text[1U] == 'X')));
  if (textXInvalid) {
    startIndex = 2U;
  }
  ...
  for (::ctrl::size index = startIndex; index < text.size(); index += 2U) {
    ::ctrl::uint8 value{};
    for (::ctrl::size digitIndex = index; digitIndex < (index + 2U);
         ++digitIndex) {
      ...
      if ((character >= '0') && (character <= '9')) {
        ...
        XWALK_CTRL_ERROR(XWALK_INVAL,
                         "SPI data contains a non-hexadecimal digit");
      }
      value = static_cast<::ctrl::uint8>(
          (static_cast<::ctrl::uint32>(value) * 16U) + digit);
    }
...
XWALK_parseNoArgumentRequest(const ::ctrl::stringvector &arguments,
                             ::ctrl::cstring errorMessage) {
  const ::ctrl::boolean argumentsDifferent =
      static_cast<::ctrl::boolean>(arguments.size() != 1U);
...
XWALK_parseLifecycleRequest(const ::ctrl::stringvector &arguments,
                            ::ctrl::cstring errorMessage) {
  ...
  const XWalkLifecycleAction action = (arguments[1U] == "start")
                                          ? XWalkLifecycleAction::Start
                                          : XWalkLifecycleAction::Stop;
  return {action};
}
...
XWalkMoveRequest XWALK_parseMoveRequest(const ::ctrl::stringvector &arguments) {
  const ::ctrl::boolean argumentsTooSmall =
      static_cast<::ctrl::boolean>(arguments.size() < 2U);
  ...
  if (arguments[1U] == "forward") {
    ...
    XWALK_CTRL_ERROR(XWALK_INVAL,
                     "move action must be forward, backward, or demo");
  }
  ...
  const ::ctrl::float64 speedPercent =
      XWALK_parseNumber(XWALK_optionValue(options, "speed", "50", false),
                        "move speed", 0.0, 100.0);
  const ::ctrl::float64 durationSeconds =
      XWALK_parseNumber(XWALK_optionValue(options, "duration", "1.0", false),
...
XWalkTurnRequest XWALK_parseTurnRequest(const ::ctrl::stringvector &arguments) {
  ...
  const ::ctrl::float64 angleDegrees =
      XWALK_parseNumber(XWALK_optionValue(options, "angle", "30", false),
                        "turn angle", 0.0, 30.0);
 ...
 */
XWalkCameraRequest
XWALK_parseCameraRequest(const ::ctrl::stringvector &arguments) {
  ...
  const controlleroptions options = XWALK_parseOptions(arguments, 2U);
  XWALK_validateOptions(options, {"angle"});
  const ::ctrl::string angleText =
      XWALK_optionValue(options, "angle", {}, true);
  if (arguments[1U] == "pan") {
 ...
 */
XWalkSensorRequest
XWALK_parseSensorRequest(const ::ctrl::stringvector &arguments) {
...
XWALK_parseSelfDriveRequest(const ::ctrl::stringvector &arguments) {
 ...
 */
XWalkSoundRequest
XWALK_parseSoundRequest(const ::ctrl::stringvector &arguments) {
  ...
  if ((arguments[1U] == "play") || (arguments[1U] == "music")) {
    const ::ctrl::boolean soundFileMissing = static_cast<::ctrl::boolean>(
        (arguments.size() < 3U) || (arguments[2U].rfind("--", 0U) == 0U));
    if (soundFileMissing) {
      XWALK_CTRL_ERROR(XWALK_INVAL, "sound play and music require a file");
    }
    request.operation = (arguments[1U] == "play") ? XWalkSoundOperation::Play
    ...
    const ::ctrl::boolean volumeArgumentCountInvalid =
        static_cast<::ctrl::boolean>(arguments.size() != 3U);
    ...
    request.volumePercent =
        XWALK_parseNumber(arguments[2U], "sound volume", 0.0, 100.0);
  ...
  const ::ctrl::boolean optionsCountVolumeDifferent =
      static_cast<::ctrl::boolean>(options.count("volume") != 0U);
  if (optionsCountVolumeDifferent) {
    request.volumePercent =
        XWALK_parseNumber(XWALK_optionValue(options, "volume", {}, true),
...
XWalkSpiRequest XWALK_parseSpiRequest(const ::ctrl::stringvector &arguments) {
  const ::ctrl::boolean argumentsTransferInvalid = static_cast<::ctrl::boolean>(
 ...
 */
XWalkGptCarRequest
XWALK_parseGptCarRequest(const ::ctrl::stringvector &arguments) {
  const ::ctrl::boolean gptArgumentCountInvalid = static_cast<::ctrl::boolean>(
  ...
  const ::ctrl::boolean argumentsValid = static_cast<::ctrl::boolean>(
  ...
  request.action = (arguments[1U] == "start") ? XWalkLifecycleAction::Start
                                              : XWalkLifecycleAction::Stop;
  for (::ctrl::size index = 2U; index < arguments.size(); ++index) {
...
XWALK_parseCalibrationRequest(const ::ctrl::stringvector &arguments) {
...
controlleroptions XWALK_parseOptions(const ::ctrl::stringvector &arguments,
  ...
  while (processingLoopRequested) {
    const ::ctrl::boolean argumentAvailable =
        static_cast<::ctrl::boolean>(index < arguments.size());
    if (argumentAvailable == false) {
      break;
    }
    const ::ctrl::string &token = arguments[index];
    const ::ctrl::boolean tokenInvalid = static_cast<::ctrl::boolean>(
    ...
    const ::ctrl::string name =
        token.substr(2U, equals == ::ctrl::string::npos ? equals : equals - 2U);
    ::ctrl::string value;
    ...
    const ::ctrl::boolean nameValueOptionsInvalid =
        static_cast<::ctrl::boolean>(name.empty() || value.empty() ||
                                     (options.count(name) != 0U));
    if (nameValueOptionsInvalid) {
      XWALK_CTRL_ERROR(XWALK_INVAL,
                       "PiCar-X CLI option is empty or duplicated");
    }
...
::ctrl::string XWALK_optionValue(const controlleroptions &options,
  ...
  const ::ctrl::boolean optionOptionsDifferent =
      static_cast<::ctrl::boolean>(option != options.end());
...
void XWALK_validateOptions(const controlleroptions &options,
  ...
  for (const auto &option : options) {
    const ::ctrl::boolean allowedOptionFirstMatched =
        static_cast<::ctrl::boolean>(std::find(allowed.begin(), allowed.end(),
                                               option.first) == allowed.end());
    if (allowedOptionFirstMatched) {
      XWALK_CTRL_ERROR(XWALK_INVAL,
                       "PiCar-X CLI option is not supported by this command");
    }
  }
}
...
::ctrl::float64 XWALK_parseNumber(::ctrl::stringview text, ::ctrl::cstring name,
                                  ::ctrl::float64 minimum,
                                  ::ctrl::float64 maximum) {
  ...
  const ::ctrl::boolean parsedLengthTextValueInvalid =
      static_cast<::ctrl::boolean>((parsedLength != text.size()) ||
                                   !XHAL_IS_FINITE(value));
  if (parsedLengthTextValueInvalid) {
    const std::string exceptionMessage =
        std::string(name).append(" must be one finite number");
    XWALK_CTRL_ERROR(XWALK_INVAL, exceptionMessage);
  }
  if ((value < minimum) || (value > maximum)) {
    const std::string exceptionMessage =
        std::string(name).append(" is outside its range");
...
::ctrl::string formatOneDecimal(::ctrl::float64 value) {
  ...
  const ::ctrl::boolean tenthsInvalid = static_cast<::ctrl::boolean>(
      ...
      (tenthsValue <
       static_cast<::ctrl::float64>(hal::common::INT32_MINIMUM)) ||
      (tenthsValue > static_cast<::ctrl::float64>(hal::common::INT32_MAXIMUM)));
  if (tenthsInvalid) {
    XWALK_CTRL_ERROR(XWALK_RANGE,
                     "PiCar-X CLI sensor value cannot be formatted");
  }
  const ::ctrl::int32 tenths = static_cast<::ctrl::int32>(tenthsValue);
  const ::ctrl::int32 whole = tenths / 10;
  const ::ctrl::int32 digit =
      static_cast<::ctrl::int32>(XHAL_ABSOLUTE_VALUE(tenths % 10));
  return hal::common::int32ToString(whole) + "." +
         hal::common::int32ToString(digit);
}
...
::ctrl::string formatValues(const hal::linetrackervalues &values) {
  return ::ctrl::string("[") + hal::common::int32ToString(values[0U]) + ", " +
         hal::common::int32ToString(values[1U]) + ", " +
         hal::common::int32ToString(values[2U]) + "]";
}
...
::ctrl::string formatStatus(const hal::linetrackerstatus &status) {
  return ::ctrl::string("[") + hal::common::uint32ToString(status[0U]) + ", " +
         hal::common::uint32ToString(status[1U]) + ", " +
         hal::common::uint32ToString(status[2U]) + "]";
}
 ...
 */
::ctrl::string
formatDetection(const agent::XWalkComputerVisionDetection &detection) {
