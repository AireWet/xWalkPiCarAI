...
::ctrl::int32 XWALK_runController(::ctrl::contextpointer context, agent::XWalkBootServices& services)
{
    ...
    const XWalkControllerCallbacks callbacks{
                                &XWALK_outputLine, &XWALK_inputLine,
    ...
    if ((services.languageModel != nullptr) && (services.cameraCapture != nullptr))
    {
        ...
        xwalk::agent::XWalkTextVisionTalk textVisionTalk(
                            *services.languageModel, *services.cameraCapture,
                            &applicationContext, talkCallbacks
                            );
        XWalkController cli(
                    textVisionTalk,
                    &applicationContext, callbacks
    ...
    if (services.languageModel != nullptr)
    {
        ...
        xwalk::agent::XWalkOnlineLlmTest onlineLlmTest(
                                        *services.languageModel,
                                        &applicationContext, onlineCallbacks
                                        );
        XWalkController cli(
                    onlineLlmTest,
                    &applicationContext, callbacks
    ...
    if (services.faceTracking != nullptr)
    {
        ...
        {
            XHAL_THROW_LOGIC_ERROR("xWalkBoot did not provide PiCar-X for face tracking");
        }

        XWalkController cli(
            *services.picarx, *services.faceTracking,
            &applicationContext, callbacks
            );
    ...
    if (services.bullFight != nullptr)
    {
        if (services.picarx == nullptr)
        {
            XHAL_THROW_LOGIC_ERROR("xWalkBoot did not provide PiCar-X for bull fight");
        }

        XWalkController cli(
            *services.picarx, *services.bullFight,
            &applicationContext, callbacks
            );
    ...
    if (services.treasureHunt != nullptr)
    {
        if (services.picarx == nullptr)
        {
            XHAL_THROW_LOGIC_ERROR("xWalkBoot did not provide PiCar-X for treasure hunt");
        }

        XWalkController cli(
                *services.picarx, *services.treasureHunt,
                &applicationContext, callbacks
                );
    ...
    if (services.videoCar != nullptr)
    {
        if (services.picarx == nullptr)
        {
            XHAL_THROW_LOGIC_ERROR("xWalkBoot did not provide PiCar-X for video car");
        }

        XWalkController cli(
                *services.picarx, *services.videoCar,
                &applicationContext, callbacks
                );
    ...
    if (services.appControl != nullptr)
    {
        if (services.picarx == nullptr)
        {
            XHAL_THROW_LOGIC_ERROR("xWalkBoot did not provide PiCar-X for app control");
        }

        XWalkController cli(
            *services.picarx, *services.appControl,
            &applicationContext, callbacks
            );
    ...
    {
        XWalkController cli(
            *services.picarx, *services.lineTracking,
            &applicationContext, callbacks
            );
    ...
    if (services.voiceAssistant != nullptr)
    {
        const ::ctrl::boolean commandArgumentsValid =
                            static_cast<::ctrl::boolean>(!commandArguments.empty() &&
                            (commandArguments[0U] == "voice-chat"));
        ...
        if (
                (services.selfDrive == nullptr) ||
                (services.voiceStatusLed == nullptr) ||
                (services.cameraCapture == nullptr)
           )
        {
            XHAL_THROW_LOGIC_ERROR("xWalkBoot did not provide the voice-active-car hardware graph");
        }
        ...
        xwalk::agent::XWalkVoiceActiveCar voiceActiveCar(
                                    *services.picarx, *services.selfDrive,
                                    *services.voiceAssistant, *services.voiceStatusLed,
                                    services.cameraCapture, voiceCallbacks, voiceConfiguration
                                    );
    ...
    if (services.textToSpeech != nullptr)
    {
        ...
        if (storytellingRobotRequested)
        {
            ...
            xwalk::agent::XWalkStorytellingRobot storytellingRobot(
                                                            ...
                                                            *services.textToSpeech,
                                                            nullptr, storyCallbacks
                                                            );
            XWalkController cli(
                        *services.picarx, storytellingRobot,
                        &applicationContext, callbacks
                        );
        ...
        xwalk::agent::XWalkVoicePromptCar voicePromptCar(
            *services.picarx, *services.textToSpeech,
            nullptr, voiceCallbacks
            );

        XWalkController cli(
            *services.picarx, voicePromptCar,
            &applicationContext, callbacks
            );
    ...
    xwalk::agent::XWalkGrayscaleCalibration grayscaleCalibration(
        *services.picarx, &applicationContext,
        &XWALK_delayMilliseconds,
        &XWALK_continueOperation);
    ...
    xwalk::agent::XWalkCliffDetection cliffDetection(
        *services.picarx, &applicationContext,
        ...
        &XWALK_continueOperation);
    XWalkController cli(*services.picarx,
        grayscaleCalibration, servoMotorCalibration, moveExample, keyboardControl,
        obstacleAvoidance, cliffDetection, &applicationContext, callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
}
