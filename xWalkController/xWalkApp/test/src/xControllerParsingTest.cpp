...
TEST(XWalkAppParsingGroup, VehicleRequests) {
  ...
  const xwalk::ctrl::XWalkMoveRequest move =
      xwalk::ctrl::XWALK_parseMoveRequest(
  ...
  EXPECT_NO_THROW(
      xwalk::ctrl::XWALK_parseNoArgumentRequest({"doctor"}, "invalid"));
  EXPECT_THROW(
      xwalk::ctrl::XWALK_parseNoArgumentRequest({"doctor", "extra"}, "invalid"),
...
TEST(XWalkAppParsingGroup, SpecializedRequests) {
  const xwalk::ctrl::XWalkSoundRequest play =
      xwalk::ctrl::XWALK_parseSoundRequest(
  ...
  const xwalk::ctrl::XWalkGptCarRequest gpt =
      xwalk::ctrl::XWALK_parseGptCarRequest(
  ...
  EXPECT_EQ(
      xwalk::ctrl::XWALK_parseCalibrationRequest({"calibrate", "grayscale"})
          .mode,
      xwalk::ctrl::XWalkCalibrationMode::Grayscale);
  EXPECT_EQ(
      xwalk::ctrl::XWALK_parseCalibrationRequest({"calibrate", "servo-motor"})
          .mode,
...
TEST(XWalkAppParsingGroup, OptionsAndNumbers) {
  const xwalk::ctrl::controlleroptions options =
      xwalk::ctrl::XWALK_parseOptions(
          {"move", "forward", "--speed=40", "--duration", "2.5"}, 2U);

  EXPECT_EQ(xwalk::ctrl::XWALK_optionValue(options, "speed", "50", false),
            "40");
  EXPECT_EQ(
      xwalk::ctrl::XWALK_optionValue(options, "missing", "default", false),
      "default");
  EXPECT_NO_THROW(
      xwalk::ctrl::XWALK_validateOptions(options, {"speed", "duration"}));
  EXPECT_THROW(xwalk::ctrl::XWALK_validateOptions(options, {"speed"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_optionValue(options, "missing", "", true),
               ctrl::invalidargument);
...
TEST(XWalkAppParsingGroup, ValidBoundaryRequests) {
  EXPECT_EQ(xwalk::ctrl::XWALK_parseLifecycleRequest({"service", "start"},
                                                     "invalid")
                .action,
  ...
  EXPECT_EQ(xwalk::ctrl::XWALK_parseCameraRequest(
                {"cam", "pan", "--angle=-90"})
                .axis,
  ...
  EXPECT_EQ(xwalk::ctrl::XWALK_parseSelfDriveRequest(
                {"self-drive", "shake", "head"})
                .action,
            "shake head");
  ...
  EXPECT_DOUBLE_EQ(xwalk::ctrl::XWALK_parseNumber("0", "value", 0.0, 100.0),
                   0.0);
...
TEST(XWalkAppParsingGroup, RejectsMalformedRequests) {
  EXPECT_THROW(xwalk::ctrl::XWALK_parseLifecycleRequest({}, "invalid"),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseLifecycleRequest({"service", "pause"},
                                                        "invalid"),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseMoveRequest({"move"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseMoveRequest({"move", "demo", "extra"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseMoveRequest({"move", "sideways"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseMoveRequest(
                   {"move", "forward", "--speed", "-1"}),
               ctrl::outofrange);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseTurnRequest({"turn"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseTurnRequest({"turn", "around"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseCameraRequest({"cam"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseCameraRequest(
                   {"cam", "roll", "--angle", "0"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseSensorRequest({"sensor"}),
               ctrl::standardexception);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseSensorRequest({"sensor", "temperature"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseSelfDriveRequest({"self-drive"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseSoundRequest({"sound"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseSoundRequest({"sound", "play"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseSoundRequest(
                   {"sound", "music", "--volume", "10"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseSoundRequest({"sound", "volume"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseSoundRequest({"sound", "unknown"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseSpiRequest({"spi", "read", "00"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseGptCarRequest({"gpt-car"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseGptCarRequest({"gpt-car", "pause"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseGptCarRequest(
                   {"gpt-car", "stop", "--keyboard"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseGptCarRequest(
                   {"gpt-car", "start", "--unknown"}),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseCalibrationRequest(
                   {"calibrate", "unknown"}),
               ctrl::invalidargument);
}
...
TEST(XWalkAppParsingGroup, RejectsMalformedOptionsAndFormatting) {
  EXPECT_THROW(xwalk::ctrl::XWALK_parseOptions({"move", "forward", "speed"},
                                               2U),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseOptions({"move", "forward", "--speed"},
                                               2U),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseOptions(
                   {"move", "forward", "--=10"}, 2U),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseOptions(
                   {"move", "forward", "--speed="}, 2U),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseOptions(
                   {"move", "forward", "--speed=1", "--speed=2"}, 2U),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseNumber("1x", "value", 0.0, 100.0),
               ctrl::invalidargument);
  EXPECT_THROW(xwalk::ctrl::XWALK_parseNumber("nan", "value", 0.0, 100.0),
               ctrl::invalidargument);
  EXPECT_THROW(XWALK_FORMAT_ONE_DECIMAL(
                   std::numeric_limits<ctrl::float64>::infinity()),
               ctrl::outofrange);
}
...
TEST(XWalkAppParsingGroup, ResultFormatting) {
  ...
  const xwalk::agent::XWalkComputerVisionDetection detection{1U, 10, 20, 30U,
                                                             40U};
  ...
  EXPECT_EQ(XWALK_FORMAT_LINE_TRACKING_STATE(
  ...
  EXPECT_EQ(XWALK_FORMAT_LINE_TRACKING_STATE(
            ...
            "right");
  EXPECT_EQ(XWALK_FORMAT_DETECTION(detection),
