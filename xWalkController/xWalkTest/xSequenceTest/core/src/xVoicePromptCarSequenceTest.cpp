...
void testVoicePromptCar(xwalk::agent::test::ControllerCommandTestContext& context)
{
    ...
    assert(sequence.run({{"voice-prompt-car", "start"},
        {"voice-prompt-car", "stop"}}) == 0);
    assert(context.state->spokenText == ctrl::stringvector({
        "Hello! I'm PiCar-X.", "Moving forward", "Moving backward",
        "Turning left", "Turning right"}));
    assert(context.state->delays == ctrl::uint32vector(
        {2'000U, 2'000U, 2'000U, 2'000U}));
    ...
    assert(context.state->steeringAngles == ctrl::float64vector(
        {0.0, 0.0, -20.0, 20.0}));
