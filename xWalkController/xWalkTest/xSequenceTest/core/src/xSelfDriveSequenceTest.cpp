...
{
    const ctrl::stringvector actions{"shake-head", "nod", "wave-hands", "resist",
        "act-cute", "rub-hands", "think", "twist-body", "celebrate", "depressed",
        "forward", "backward", "honking", "start-engine"};
    ...
    assert(context.state->musicSoundFile.find("car-start-engine.wav") !=
        ctrl::string::npos);
    assert(xwalk::agent::test::containsOrderedEvents(context.state->eventLog,
        {"controller.continue", "hal.i2c.write", "controller.delay",
            "controller.continue", "hal.music.sound"}));
}
}
