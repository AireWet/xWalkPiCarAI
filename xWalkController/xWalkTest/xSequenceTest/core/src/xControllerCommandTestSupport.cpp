...
void interruptGpio(::ctrl::contextpointer context, ::ctrl::uint8 pin,
    ...
    ::ctrl::contextpointer handlerContext, XWalkHal::gpiointerrupthandler handler)
{
    static_cast<void>(context);
    static_cast<void>(pin);
...
::ctrl::string input(::ctrl::contextpointer context, ::ctrl::stringview prompt)
{
    ...
    const ::ctrl::boolean inputUnavailable =
        static_cast<::ctrl::boolean>(
            state.inputIndex >= state.inputLines.size());
    if (inputUnavailable)
    {
        return "skip";
    }
...
::ctrl::boolean startAppControl(::ctrl::contextpointer context,
    ::ctrl::stringview name, ::ctrl::stringview type, ::ctrl::uint16 port)
{
...
xwalk::agent::XWalkAppControlInput pollAppControl(
...
void publishAppControl(::ctrl::contextpointer context,
...
xwalk::agent::XWalkComputerVisionObservation observeVision(
    ::ctrl::contextpointer context)
{
    ControllerCommandTestState& state = *static_cast<CallbackContext*>(context)->state;
...
xwalk::agent::XWalkComputerVisionColor selectTreasureColor(
    ::ctrl::contextpointer context)
{
    ControllerCommandTestState& state = *static_cast<CallbackContext*>(context)->state;
...
void stopVideo(::ctrl::contextpointer context)
{
    ControllerCommandTestState& state = *static_cast<CallbackContext*>(context)->state;
...
void setServoZeroingAngle(::ctrl::contextpointer context,
    ::ctrl::uint8 servoId, ::ctrl::float64 angleDegrees)
{
    ...
    ::ctrl::uint32 timeoutMs)
{
    ...
    const ::ctrl::boolean recognitionTranscriptUnavailable =
        static_cast<::ctrl::boolean>(
            state.recognitionTranscriptIndex >= state.recognitionTranscripts.size());
...
void addMessage(::ctrl::contextpointer context, XWalkHal::XWalkLanguageModelRole role,
...
::ctrl::string promptModel(::ctrl::contextpointer context,
    ::ctrl::stringview prompt, ::ctrl::stringview imagePath)
{
    ControllerCommandTestState& state =
        *static_cast<CallbackContext*>(context)->state;
    ...
    ::ctrl::charpointer argumentValues[], controllercommandtestcallback callback)
{
    ...
    const xwalk::ctrl::XWalkControllerCallbacks controllerCallbacks{&output, &input, &delay,
        &continueOperation, &sound};
    XWalkLineTracking lineTracking(picarx, &callbackContext, &delay);
    const hal::XWalkMusicCallbacks musicCallbacks{&enableMusic, &playSound,
        &playSound, &playMusic, &setVolume, &musicControl, &musicControl,
        &musicControl, &soundLength, &playTone};
    ...
    XWalkSelfDrive selfDriveGpt(picarx, music, &callbackContext,
        &selfDriveQuietDelay, nullptr, XWALK_TEST_SOUND_DIRECTORY);
    XWalkSelfDrive selfDriveVoice(picarx, music, &callbackContext,
        &selfDriveQuietDelay, nullptr, XWALK_TEST_SOUND_DIRECTORY);
    XWalkSelfDrive selfDriveGptCar(picarx, music, &callbackContext,
        &selfDriveQuietDelay, nullptr, XWALK_TEST_SOUND_DIRECTORY);
    XWalkGrayscaleCalibration grayscaleCalibration(
        picarx, &callbackContext, &delay, &continueOperation);
    XWalkServoMotorCalibration servoMotorCalibration(
        picarx, &callbackContext, &delay, &continueOperation);
    XWalkMoveExample moveExample(
        picarx, &callbackContext, &delay, &continueOperation);
    XWalkKeyboardControl keyboardControl(
        picarx, &callbackContext, &delay, &continueOperation);
    XWalkObstacleAvoidance obstacleAvoidance(
        picarx, &callbackContext, &delay, &continueOperation);
    XWalkCliffDetection cliffDetection(
        picarx, &callbackContext, &delay, &continueOperation);
    xwalk::ctrl::XWalkController controller(picarx, grayscaleCalibration, servoMotorCalibration,
        moveExample, keyboardControl, obstacleAvoidance, cliffDetection,
        &callbackContext, controllerCallbacks);
    xwalk::ctrl::XWalkController lineController(picarx, lineTracking, &callbackContext,
        controllerCallbacks);
    xwalk::ctrl::XWalkController selfDriveController(picarx, selfDrive, &callbackContext,
        controllerCallbacks);
    ...
    xwalk::ctrl::XWalkController doctorController(passingReport, &callbackContext,
        controllerCallbacks);
    xwalk::ctrl::XWalkController failingDoctorController(failingReport, &callbackContext,
        controllerCallbacks);
    ...
    xwalk::ctrl::XWalkController servoZeroingController(servoZeroing, &callbackContext,
        controllerCallbacks);

    hal::XWalkBoardControl boardControl(reset, speaker, adc4, &callbackContext,
        &primeSpeaker);
    const hal::XWalkSpeechToTextCallbacks speechCallbacks{&recognitionReady,
        &recognizeSpeech, &transcribeFile, &stopRecognition};
    hal::XWalkSpeechToText speechToText(&callbackContext, speechCallbacks);
    const hal::XWalkLanguageModelCallbacks modelCallbacks{&setModelText,
        &setModelText, &setMaximumMessages, &addMessage, &promptModel};
    ...
    hal::XWalkVoiceAssistant chatbotAssistant(
        ...
        chatbotAssistantConfiguration);
    hal::XWalkVoiceAssistant assistant(speechToText, languageModel, textToSpeech,
    ...
    const XWalkLocalVoiceChatbotCallbacks chatbotCallbacks{&output,
        &continueOperation, &delay};
    XWalkLocalVoiceChatbot voiceChat(
        chatbotAssistant, &callbackContext, chatbotCallbacks);
    const XWalkVoiceActiveCarCallbacks voiceCallbacks{&output, &continueOperation,
        &delay, &captureVoiceActiveCarImage, &input};
    ...
    XWalkVoiceActiveCar voiceActiveGpt(picarx, selfDriveGpt, gptAssistant, led,
        &callbackContext, voiceCallbacks,
        XWalkVoiceActiveCarGpt::carConfiguration());
    XWalkVoiceActiveCar gptCarCoordinator(picarx, selfDriveGptCar,
        gptCarAssistant, led, &callbackContext, voiceCallbacks,
        XWalkGptCar::carConfiguration());
    XWalkGptCar gptCar(gptCarCoordinator);
    const XWalkVoiceControlledCarCallbacks controlledCallbacks{&output,
        &continueOperation, &delay};
    XWalkVoiceControlledCar voiceControlled(picarx, speechToText,
        &callbackContext, controlledCallbacks);
    const XWalkVoicePromptCarCallbacks promptCallbacks{&output,
        &continueOperation, &delay};
    XWalkVoicePromptCar voicePrompt(picarx, textToSpeech, &callbackContext,
        promptCallbacks);
    xwalk::ctrl::XWalkController voiceChatController(picarx, voiceChat, &callbackContext,
        controllerCallbacks);
    xwalk::ctrl::XWalkController voiceActiveController(picarx, voiceActive, &callbackContext,
        controllerCallbacks);
    ...
    xwalk::ctrl::XWalkController voiceControlledController(picarx, voiceControlled,
        &callbackContext, controllerCallbacks);
    xwalk::ctrl::XWalkController voicePromptController(picarx, voicePrompt, &callbackContext,
        controllerCallbacks);
    const XWalkStorytellingRobotCallbacks storyCallbacks{
        &delay, &continueOperation};
    XWalkStorytellingRobot storytellingRobot(picarx, textToSpeech,
        &callbackContext, storyCallbacks);
    xwalk::ctrl::XWalkController storytellingController(picarx, storytellingRobot,
        &callbackContext, controllerCallbacks);
    ...
    hal::XWalkCamera textVisionCamera(&callbackContext,
        &captureTextVisionImage, textVisionCameraConfiguration);
    XWalkCameraCapture textVisionCapture(textVisionCamera, "/tmp/llm-img.jpg");
    const XWalkTextVisionTalkCallbacks textVisionCallbacks{&output, &input,
        &delay, &continueOperation};
    XWalkTextVisionTalk textVisionTalk(languageModel, textVisionCapture,
        &callbackContext, textVisionCallbacks);
    xwalk::ctrl::XWalkController textVisionTalkController(textVisionTalk,
        &callbackContext, controllerCallbacks);
    ...
    XWalkOnlineLlmTest onlineLlmTest(languageModel, &callbackContext,
        onlineCallbacks);
    xwalk::ctrl::XWalkController onlineLlmTestController(onlineLlmTest,
        &callbackContext, controllerCallbacks);
    const XWalkComputerVisionCallbacks visionCallbacks{&startVision, &stopVision,
        &captureVision, &setVisionColor, &setVisionFace, &setVisionQr,
        &observeVision, &delay, &continueOperation};
    XWalkComputerVision computerVision(&callbackContext, visionCallbacks);
    xwalk::ctrl::XWalkController computerVisionController(computerVision,
        &callbackContext, controllerCallbacks);
    XWalkFaceTracking faceTracking(picarx, &callbackContext, visionCallbacks);
    xwalk::ctrl::XWalkController faceTrackingController(picarx, faceTracking,
        &callbackContext, controllerCallbacks);
    XWalkBullFight bullFight(picarx, &callbackContext, visionCallbacks);
    xwalk::ctrl::XWalkController bullFightController(picarx, bullFight,
        &callbackContext, controllerCallbacks);
    const XWalkTreasureHuntCallbacks treasureCallbacks{
        visionCallbacks, &selectTreasureColor};
    XWalkTreasureHunt treasureHunt(
        picarx, textToSpeech, &callbackContext, treasureCallbacks);
    xwalk::ctrl::XWalkController treasureHuntController(picarx, treasureHunt,
        &callbackContext, controllerCallbacks);
    const XWalkVideoRecordingCallbacks videoCallbacks{&startVision, &stopVision,
        &beginVideo, &pauseVideo, &continueVideo, &stopVideo, &delay,
        &continueOperation, &videoTimestamp};
    XWalkVideoRecording videoRecording(&callbackContext, videoCallbacks);
    xwalk::ctrl::XWalkController videoRecordingController(videoRecording,
        &callbackContext, controllerCallbacks);
    XWalkVideoCar videoCar(picarx, &callbackContext, visionCallbacks);
    xwalk::ctrl::XWalkController videoCarController(picarx, videoCar,
        &callbackContext, controllerCallbacks);
    XWalkAppControlCallbacks appCallbacks{&callbackContext, &startAppControl,
        &stopAppControl, &pollAppControl, &publishAppControl, &callbackContext,
        visionCallbacks};
    ...
    xwalk::ctrl::XWalkController appControlController(picarx, appControl,
        &callbackContext, controllerCallbacks);
    XWalkSoundBackgroundMusic soundBackgroundMusic(music, &callbackContext,
        &delay, &continueOperation, XWALK_TEST_SOUND_DIRECTORY,
        XWALK_TEST_MUSIC_DIRECTORY);
    xwalk::ctrl::XWalkController soundBackgroundMusicController(soundBackgroundMusic,
        &callbackContext, controllerCallbacks);

    ControllerCommandTestContext testContext{&state, &controller, &lineController,
        &selfDriveController, &spiController, &doctorController,
        &servoZeroingController, &failingDoctorController,
        &voiceChatController, &voiceActiveController,
        &voiceActiveGptController, &gptCarController,
        &voiceControlledController, &voicePromptController,
        &storytellingController,
        &textVisionTalkController,
        &onlineLlmTestController,
        &computerVisionController, &faceTrackingController, &bullFightController,
        &treasureHuntController,
        &videoRecordingController, &videoCarController, &appControlController,
        &soundBackgroundMusicController,
        &motors, &picarx, &configuration};
