...
{
    context.state->inputLines = {"skip", "skip", "skip", "skip", "10", "raised", "y", "y",
        "y", "y", "y", "ready", "q", "e", "y"};
    xwalk::agent::test::XWalkControllerSequence sequence(*context.controller);
    assert(sequence.run({{"calibrate"}}) == 0);
    assert(context.state->outputLines.back() == "Calibration complete!");
    ...
    assert(xwalk::agent::test::containsOrderedEvents(context.state->eventLog,
        {"controller.output", "controller.input", "hal.i2c.write",
            "controller.input", "controller.continue", "hal.i2c.write",
            "controller.delay", "hal.i2c.write", "controller.input",
            "hal.i2c.read", "controller.output"}));
    ...
    assert(context.state->outputLines.back() == "Grayscale calibration complete!");
    assert(context.configuration->get("cliff_reference") == "[1000,1000,1000]");
    assert(xwalk::agent::test::containsOrderedEvents(context.state->eventLog,
        {"controller.output", "hal.i2c.write", "controller.input",
            "hal.i2c.write", "controller.delay", "controller.input",
            "hal.i2c.write", "hal.i2c.read", "controller.delay",
            "controller.output", "controller.input", "hal.i2c.write",
            "hal.i2c.read", "controller.output", "controller.input",
            "controller.output"}));

    context.state->inputLines = {"skip", "5", "y", "-2", "y", "3", "y", "10", "1", "-1",
        "raised", "y", "y", "y", "y", "y"};
    context.state->inputIndex = 0U;
    context.state->eventLog.clear();
    assert(sequence.run({{"calibrate", "servo-motor"}}) == 0);
    assert(context.state->outputLines.back() ==
        "Servo and motor calibration complete!");
    ...
    assert(xwalk::agent::test::containsOrderedEvents(context.state->eventLog,
        {"controller.output", "controller.input", "hal.i2c.write",
            "controller.input", "hal.i2c.write", "controller.input",
            "hal.i2c.write", "controller.input", "controller.input",
            "controller.input", "hal.i2c.write", "controller.input",
