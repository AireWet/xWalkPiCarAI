...
void runCommandSequence(sequencetestwithargs function, ctrl::stringview name)
{
    ...
    ASSERT_FALSE(pathError) << "cannot resolve CLI sequence executable: "
                            << pathError.message();
    ASSERT_FALSE(executablePath.empty());

    const ctrl::filesystempath path =
        executablePath.parent_path() /
        "xWalkController" / "xWalkTest" / "sequence-data" /
...
int main(int argumentCount, char* argumentValues[])
{
    ...
    {
        std::cerr << "xCliSequenceTest path error: cannot resolve /proc/self/exe: "
                  << pathError.message() << '\n';
        return EXIT_FAILURE;
    }
    ...
    const ctrl::filesystempath configurationPath =
        executablePath.parent_path() / "xCliSequenceTestConfig.xml";
    const ctrl::boolean configurationLoaded = configuration.load(configurationPath,
        xwalk::agent::test::availableCliSequenceTests(), error);
