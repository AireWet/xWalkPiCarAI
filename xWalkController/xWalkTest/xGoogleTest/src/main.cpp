...
TEST(XWalkControllerGroup, Controller)
{
    ...
    const ctrl::filesystempath executablePath =
        std::filesystem::read_symlink("/proc/self/exe", pathError);
    ASSERT_FALSE(pathError) << "cannot resolve CLI GoogleTest executable: "
                            << pathError.message();
    ASSERT_FALSE(executablePath.empty());

    const ctrl::filesystempath configurationPath =
        executablePath.parent_path() /
        "xWalkController" / "xWalkApp" / "test-data" /
        "cli-central-test.conf";
    runIsolated(&xWalkControllerLegacyMain,
        {"xWalkControllerTest", configurationPath.string()});
}
...
int main(int argumentCount, char* argumentValues[])
{
    ...
    {
        std::cerr << "xCliGoogleTest path error: cannot resolve /proc/self/exe: "
                  << pathError.message() << '\n';
        return EXIT_FAILURE;
    }
    ...
    ctrl::string error;
    const ctrl::filesystempath configurationPath =
        executablePath.parent_path() / "xCliGoogleTestConfig.xml";
    const ctrl::boolean configurationLoaded = configuration.load(configurationPath,
        xwalk::agent::test::availableCliTests(), error);
    ...
    const ctrl::boolean standardFilterSelected =
        xwalk::agent::test::hasGoogleTestFilter(argumentCount, argumentValues);
    ::testing::InitGoogleTest(&argumentCount, argumentValues);
    GTEST_FLAG_SET(catch_exceptions, false);
    if (!standardFilterSelected)
    {
        GTEST_FLAG_SET(filter,
            xwalk::agent::test::configuredCliTestFilter(configuration));
    }
