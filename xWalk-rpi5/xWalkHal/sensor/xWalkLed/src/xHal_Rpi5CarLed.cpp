...
void XWalkLed::toggle() {
  stopBlinking();
  toggleFromWorker();
  XWALK_HAL_TRACE_UID1(RPI .261, "LED toggled to logical state %u",
                       static_cast<uint32>(outputValue.load()));
}
 ...
 */
void XWalkLed::blink(uint32 cycleCount, float64 toggleDelaySeconds,
                     float64 pauseSeconds) {
  if ((cycleCount == 0U) || (cycleCount > XHAL_RPI5CAR_LED_MAX_BLINK_COUNT)) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "LED blink count is outside its supported range");
  }

  const uint32 requestedToggleDelayUs =
      durationMicroseconds(toggleDelaySeconds, "LED toggle delay");
  const uint32 toggleDelayUs = XHAL_MAXIMUM_VALUE(
      requestedToggleDelayUs, XHAL_RPI5CAR_LED_STOP_POLL_INTERVAL_US);
  const uint32 pauseUs = durationMicroseconds(pauseSeconds, "LED pause");

  stopBlinking();
  blinkRunning.store(true);
  blinkThread = threadhandle(&XWalkLed::blinkLoop, this, cycleCount,
                             toggleDelayUs, pauseUs);
  XWALK_HAL_TRACE_UID1(RPI .262, "LED blinking started with %u cycle(s)",
                       cycleCount);
}
 ...
 */
boolean XWalkLed::isOn() const noexcept { return outputValue.load(); }
 ...
 */
boolean XWalkLed::isBlinking() const noexcept { return blinkRunning.load(); }
 ...
 */
void XWalkLed::blinkLoop(uint32 cycleCount, uint32 toggleDelayUs,
                         uint32 pauseUs) noexcept {
  static_cast<void>(gpioObject->off());
  outputValue.store(false);
  ...
  while (processingLoopRequested) {
    const hal::boolean blinkRequested =
        static_cast<hal::boolean>(blinkRunning.load());
    ...
    if (blinkStillActive) {
      ...
      static_cast<void>(
          waitWhileBlinking(XHAL_RPI5CAR_LED_STOP_POLL_INTERVAL_US));
    }
  }
...
boolean XWalkLed::waitWhileBlinking(uint32 durationUs) const {
  ...
  while (blinkWaitRequested) {
    const hal::boolean blinkMayContinue =
        static_cast<hal::boolean>((remainingUs > 0U) && blinkRunning.load());
...
void XWalkLed::stopWorker() {
  ...
  const hal::boolean blinkThreadJoinable =
      static_cast<hal::boolean>(blinkThread.joinable());
 ...
 */
uint32 XWalkLed::durationMicroseconds(float64 durationSeconds,
  ...
  if (durationSecondsNotFinite) {
    const std::string exceptionMessage =
        std::string(parameterName).append(" must be finite");
    XWALK_HAL_ERROR(XWALK_INVAL, exceptionMessage);
  }
  if (durationSeconds < 0.0) {
    const std::string exceptionMessage =
        std::string(parameterName).append(" must not be negative");
    XWALK_HAL_ERROR(XWALK_RANGE, exceptionMessage);
  }

  const float64 durationUs =
      durationSeconds * XHAL_RPI5CAR_LED_MICROSECONDS_PER_SECOND;
  return common::roundedValue(durationUs, parameterName, 0U,
