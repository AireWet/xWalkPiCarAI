...
XWalkRgbLed::XWalkRgbLed(XWalkPwm &red, XWalkPwm &green, XWalkPwm &blue,
    ...
    : redPwm(&red), greenPwm(&green), bluePwm(&blue),
      commonValue(validateCommon(common)) {
  XWALK_HAL_TRACE_UID1(RPI .265,
                       "RGB LED constructed with common-terminal mode %u",
                       static_cast<uint32>(commonValue));
}
...
XWalkRgbLedCommon XWalkRgbLed::validateCommon(XWalkRgbLedCommon common) {
  ...
  const boolean isCathode =
      commonSelection == XHAL_RPI5CAR_RGB_LED_COMMON_CATHODE;
  const boolean isAnode = commonSelection == XHAL_RPI5CAR_RGB_LED_COMMON_ANODE;
  if ((!isCathode) && (!isAnode)) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "RGB LED common mode must be cathode or anode");
  }
  return common;
}
