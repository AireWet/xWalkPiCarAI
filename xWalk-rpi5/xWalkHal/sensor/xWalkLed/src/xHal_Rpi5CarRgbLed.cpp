 ...
 * output fails.
 */
void XWalkRgbLed::setColor(const rgbcolor &color) { applyColor(color); }
 ...
 */
const rgbcolor &XWalkRgbLed::color() const noexcept { return colorValue; }
 ...
 */
XWalkRgbLedCommon XWalkRgbLed::common() const noexcept { return commonValue; }
...
rgbcolor XWalkRgbLed::decodeColor(uint32 packedColor) {
  if (packedColor > XHAL_RPI5CAR_RGB_LED_MAX_PACKED_COLOR) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "RGB LED packed color must be in range 0x000000..0xFFFFFF");
  }

  const uint32 redValue = (packedColor & XHAL_RPI5CAR_RGB_LED_RED_MASK) >>
                          XHAL_RPI5CAR_RGB_LED_RED_SHIFT;
  const uint32 greenValue = (packedColor & XHAL_RPI5CAR_RGB_LED_GREEN_MASK) >>
                            XHAL_RPI5CAR_RGB_LED_GREEN_SHIFT;
  const uint32 blueValue = (packedColor & XHAL_RPI5CAR_RGB_LED_BLUE_MASK) >>
                           XHAL_RPI5CAR_RGB_LED_BLUE_SHIFT;
  return {static_cast<uint8>(redValue), static_cast<uint8>(greenValue),
          static_cast<uint8>(blueValue)};
}
...
uint32 XWalkRgbLed::parseHexColor(stringview hexadecimalColor) {
  ...
  while ((endPosition > firstDigit) &&
         (hexadecimalColor[endPosition - 1U] == '#')) {
    --endPosition;
  }
  ...
  if (digitCount != XHAL_RPI5CAR_RGB_LED_HEX_DIGIT_COUNT) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "RGB LED color text must contain six hexadecimal digits");
  }

  uint32 packedColor = 0U;
  for (size index = firstDigit; index < endPosition; ++index) {
    const uint32 digitValue =
        static_cast<uint32>(hexDigitValue(hexadecimalColor[index]));
...
uint8 XWalkRgbLed::hexDigitValue(char digit) {
  ...
  if ((digit >= '0') && (digit <= '9')) {
    ...
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "RGB LED color text contains a non-hexadecimal character");
  }
  return value;
}
...
float64 XWalkRgbLed::componentToPercent(uint8 component) const noexcept {
  ...
  const float64 maximumValue =
      static_cast<float64>(XHAL_RPI5CAR_RGB_LED_MAX_COMPONENT);
...
void XWalkRgbLed::applyColor(const rgbcolor &color) {
  const float64 redPercent =
      componentToPercent(color[XHAL_RPI5CAR_RGB_LED_RED_CHANNEL]);
  const float64 greenPercent =
      componentToPercent(color[XHAL_RPI5CAR_RGB_LED_GREEN_CHANNEL]);
  const float64 bluePercent =
      componentToPercent(color[XHAL_RPI5CAR_RGB_LED_BLUE_CHANNEL]);
  ...
  XWALK_HAL_TRACE_UID3(
      RPI .266, "RGB LED color applied as red %u, green %u, and blue %u",
