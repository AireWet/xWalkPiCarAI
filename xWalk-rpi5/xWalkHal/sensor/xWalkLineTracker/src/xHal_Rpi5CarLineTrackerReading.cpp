...
int32 XWalkLineTracker::readChannel(uint32 channel, boolean raw) {
  ...
  const float64 calibratedValue =
      scaledValue + calibrationValue.offsets[channel];
 ...
 */
linetrackervalues
XWalkLineTracker::calibrateData(const linetrackervalues &data) const {
  linetrackervalues calibratedData{};
  for (uint32 channel = 0U; channel < XHAL_RPI5CAR_LINE_TRACKER_CHANNEL_COUNT;
       ++channel) {
 ...
 * `false`.
 */
boolean XWalkLineTracker::isOnCliff() { return isOnCliff(read()); }
 ...
 */
boolean
XWalkLineTracker::isOnCliff(const linetrackervalues &data) const noexcept {
  for (uint32 channel = 0U; channel < XHAL_RPI5CAR_LINE_TRACKER_CHANNEL_COUNT;
       ++channel) {
 ...
 */
float64 XWalkLineTracker::getLinePosition() { return getLinePosition(read()); }
...
float64 XWalkLineTracker::getLinePosition(const linetrackervalues &data) {
  const float64 referenceDifference =
      lineBackgroundReferenceValue - lineReferenceValue;
  ...
  const float64 middleWeight =
      weights[XHAL_RPI5CAR_LINE_TRACKER_MIDDLE_CHANNEL];
  ...
  const hal::boolean onLineNotMatched =
      static_cast<hal::boolean>(!isOnLine(data));
  if (onLineNotMatched) {
    return 0.0;
  }
  ...
  const boolean leftOnly = (leftWeight >= minimumWeight) &&
                           (middleWeight < minimumWeight) &&
                           (rightWeight < minimumWeight);
  const boolean rightOnly = (rightWeight >= minimumWeight) &&
                            (middleWeight < minimumWeight) &&
                            (leftWeight < minimumWeight);
  if (leftOnly) {
  ...
  const float64 normalizedPosition =
      position / XHAL_RPI5CAR_LINE_TRACKER_POSITION_DIVISOR;
  const float64 constrainedPosition =
      constrain(normalizedPosition, XHAL_RPI5CAR_LINE_TRACKER_MINIMUM_POSITION,
                XHAL_RPI5CAR_LINE_TRACKER_MAXIMUM_POSITION);
 ...
 */
boolean XWalkLineTracker::isOnLine() { return isOnLine(read()); }
 ...
 */
boolean
XWalkLineTracker::isOnLine(const linetrackervalues &data) const noexcept {
  const hal::boolean onCliffMatched =
      static_cast<hal::boolean>(isOnCliff(data));
  if (onCliffMatched) {
    return false;
  }
  ...
  for (uint32 channel = 1U; channel < XHAL_RPI5CAR_LINE_TRACKER_CHANNEL_COUNT;
       ++channel) {
  ...
  const float64 requiredDifference =
      static_cast<float64>(XHAL_RPI5CAR_LINE_TRACKER_LINE_DIFFERENCE);
