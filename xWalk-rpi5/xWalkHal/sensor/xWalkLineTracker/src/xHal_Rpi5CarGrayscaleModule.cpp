...
XWalkAdc &XWalkGrayscaleModule::sensorAt(uint32 channel) const {
  if (channel >= XHAL_RPI5CAR_LINE_TRACKER_CHANNEL_COUNT) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Grayscale channel must be in the range zero through two");
  }
 ...
 */
XWalkGrayscaleModule::XWalkGrayscaleModule(XWalkAdc &left, XWalkAdc &middle,
                                           XWalkAdc &right)
 ...
 */
void XWalkGrayscaleModule::setReference(
    const linetrackervalues &references) noexcept {
 ...
 */
linetrackerstatus
XWalkGrayscaleModule::readStatus(const linetrackervalues &data) const noexcept {
