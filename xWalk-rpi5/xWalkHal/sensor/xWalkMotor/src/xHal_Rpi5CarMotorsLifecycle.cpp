...
uint8 XWalkMotors::validateMotorId(uint8 motorId) {
  if ((motorId != XHAL_RPI5CAR_MOTOR_FIRST_ID) &&
      (motorId != XHAL_RPI5CAR_MOTOR_SECOND_ID)) {
 ...
 */
void XWalkMotors::validateSpeeds(float64 leftSpeedPercent,
                                 float64 rightSpeedPercent) {
  const hal::boolean leftSpeedPercentRightSpeedPercentInvalid =
      static_cast<hal::boolean>((!XHAL_IS_FINITE(leftSpeedPercent)) ||
                                (!XHAL_IS_FINITE(rightSpeedPercent)));
  if (leftSpeedPercentRightSpeedPercentInvalid) {
    XWALK_HAL_ERROR(XWALK_INVAL, "paired motor speeds must be finite");
  }
  const boolean leftOutsideRange =
      (leftSpeedPercent < XHAL_RPI5CAR_MOTOR_MIN_SPEED_PERCENT) ||
      (leftSpeedPercent > XHAL_RPI5CAR_MOTOR_MAX_SPEED_PERCENT);
  const boolean rightOutsideRange =
      (rightSpeedPercent < XHAL_RPI5CAR_MOTOR_MIN_SPEED_PERCENT) ||
      (rightSpeedPercent > XHAL_RPI5CAR_MOTOR_MAX_SPEED_PERCENT);
  if (leftOutsideRange || rightOutsideRange) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "paired motor speeds must be between -100 and 100 percent");
  }
}
 ...
 * applied.
 */
XWalkMotors::XWalkMotors(XWalkMotor &firstMotor, XWalkMotor &secondMotor,
                         const XWalkMotorsConfiguration &configuration)
    : motorOne(&firstMotor), motorTwo(&secondMotor),
  ...
  if ((configurationValue.watchdogTimeoutMilliseconds == 0U) ||
      (configurationValue.watchdogTimeoutMilliseconds >
       XHAL_RPI5CAR_MOTOR_WATCHDOG_MAXIMUM_MILLISECONDS)) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "motor.watchdog_timeout_ms must be from 1 through 60000");
  }
  ...
  if (configurationValue.watchdogWorkerEnabled) {
    if (configurationValue.beforeThreadStart != nullptr) {
      configurationValue.beforeThreadStart(
          configurationValue.threadStartContext);
    }
...
uint64 XWalkMotors::nowMilliseconds() const noexcept {
  if (configurationValue.clockMilliseconds != nullptr) {
    return configurationValue.clockMilliseconds(
        configurationValue.clockContext);
  }
