...
void testActiveControl() {
  ...
  assert(buzzer.isOn() == false);
  assert(backend.gpioValue == false);
  xwalk::hal::test::expectFailure([&]() { buzzer.setFrequency(440.0); });
  xwalk::hal::test::expectFailure([&]() { buzzer.play(440.0); });
}
/** @brief Verifies finite and non-negative playback-duration requirements. */
void testDurationValidation() {
  PassiveFixture fixture;
  XWalkHal::XWalkBuzzer buzzer(fixture.pwm);
  xwalk::hal::test::expectFailure([&]() { buzzer.play(440.0, -1.0); });
  assert(buzzer.isOn() == false);
...
void testTraceSelection() {
  ...
  XWalkHal::charpointer malformedArguments[]{executable, option,
                                             malformedSelector};
  const xwalk::hal::sim::XWalkBuzzerSimulationArguments malformed(
...
XWalkHal::int32 main() {
  xwalk::hal::XWalkTrace::configureGlobal(
      XWALK_BUZZER_SIMULATION_TRACE_CONFIG_PATH,
