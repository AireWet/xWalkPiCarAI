...
void XWalkBuzzer::on() {
  ...
  activeValue = true;
  XWALK_HAL_TRACE_UID1(RPI .277, "Buzzer activated in passive mode %u",
                       static_cast<uint32>(pwmObject != nullptr));
}
...
void XWalkBuzzer::off() {
  ...
  activeValue = false;
  XWALK_HAL_TRACE_UID1(RPI .278, "Buzzer deactivated in passive mode %u",
                       static_cast<uint32>(pwmObject != nullptr));
}
...
void XWalkBuzzer::setFrequency(float64 frequencyHz) {
  ...
  XWALK_HAL_TRACE_UID1(RPI .279, "Passive buzzer frequency set to %.2f Hz",
                       frequencyHz);
}
...
void XWalkBuzzer::play(float64 frequencyHz) {
  ...
  on();
  XWALK_HAL_TRACE_UID1(RPI .280,
                       "Continuous passive-buzzer playback started at %.2f Hz",
                       frequencyHz);
}
 ...
 */
boolean XWalkBuzzer::isOn() const noexcept { return activeValue; }
 ...
 */
boolean XWalkBuzzer::isPassive() const noexcept { return pwmObject != nullptr; }
...
void XWalkBuzzer::requirePassiveBuzzer() const {
  if (pwmObject == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Buzzer operation requires a passive PWM buzzer");
  }
}
