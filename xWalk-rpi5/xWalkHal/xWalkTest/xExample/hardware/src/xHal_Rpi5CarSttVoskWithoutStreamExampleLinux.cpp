...
void XWalkSttVoskWithoutStreamExampleLinux::run(uint32 sessionCount,
  ...
  const XWalkSttVoskWithoutStreamExampleCallbacks exampleCallbacks{&listen,
                                                                   &report};
...
XWalkSttVoskWithoutStreamExampleLinux &
XWalkSttVoskWithoutStreamExampleLinux::adapter(contextpointer context) {
  if (context == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Non-streaming Vosk Linux context must not be null");
  }
  XWalkSttVoskWithoutStreamExampleLinux &self =
      *static_cast<XWalkSttVoskWithoutStreamExampleLinux *>(context);
  if (self.speechToTextObject == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Non-streaming Vosk Linux adapter has no speech binding");
  }
  return self;
}

/** @brief Captures and recognizes one bounded utterance. */
string XWalkSttVoskWithoutStreamExampleLinux::listen(contextpointer context,
                                                     uint32 timeoutMs) {
  return adapter(context).speechToTextObject->listen(timeoutMs);
}

/** @brief Prints one literal prompt or final transcript. */
void XWalkSttVoskWithoutStreamExampleLinux::report(contextpointer context,
                                                   stringview text) {
