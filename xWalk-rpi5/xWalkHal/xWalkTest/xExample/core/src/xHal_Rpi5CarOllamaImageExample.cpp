 ...
 */
XWalkOllamaImageExample::XWalkOllamaImageExample(
  ...
  if ((callbacks.readPrompt == nullptr) || (callbacks.write == nullptr)) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Ollama image example requires complete console callbacks");
  }
}
...
void XWalkOllamaImageExample::run(uint32 maximumPrompts, stringview imagePath) {
  if ((maximumPrompts == 0U) ||
    ...
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Ollama image example prompt count is outside its range");
  }
