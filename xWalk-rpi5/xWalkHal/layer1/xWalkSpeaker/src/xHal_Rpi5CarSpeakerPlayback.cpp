...
string XWalkSpeaker::play(stringview filePath) {
  ...
  const hal::boolean audioFileUnavailable = static_cast<hal::boolean>(
      !filesystemEntryExists(audioPath) || !isRegularFile(audioPath));
  if (audioFileUnavailable) {
    XWALK_HAL_ERROR(
        XWALK_RUNTIME,
        "Speaker audio-file path must identify an existing regular file");
  }

  const XWalkSpeakerAudioHandler handler = audioHandler(filePath);
  XWalkSpeakerAudioData audioData =
      callbacks.decodeAudio(backendContext, filePath, handler);
  ...
  size selectedIndex = XHAL_RPI5CAR_SPEAKER_INVALID_TASK_INDEX;
  {
    ...
    if (selectedIndex == XHAL_RPI5CAR_SPEAKER_INVALID_TASK_INDEX) {
      XWALK_HAL_ERROR(XWALK_RUNTIME,
                      "Speaker has no available playback task slot");
    }
    ...
    task.playing = false;
    task.worker =
        threadhandle(&XWalkSpeaker::playbackLoop, this, selectedIndex);
  }
  XWALK_HAL_TRACE_UID1(RPI .310, "Speaker playback task %s started",
                       taskId.c_str());
  return taskId;
}
...
XWalkSpeakerAudioHandler XWalkSpeaker::audioHandler(stringview filePath) {
  ...
  const boolean extensionMissing = (extensionSeparator == stringview::npos) ||
                                   ((directorySeparator != stringview::npos) &&
                                    (extensionSeparator < directorySeparator));
  const hal::boolean audioExtensionInvalid = static_cast<hal::boolean>(
      extensionMissing || ((extensionSeparator + 1U) >= filePath.size()));
  if (audioExtensionInvalid) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Speaker audio file requires a supported extension");
  }
...
void XWalkSpeaker::validateAudioData(const XWalkSpeakerAudioData &audioData) {
  if ((audioData.sampleRateHz == 0U) || (audioData.channelCount == 0U)) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Speaker backend returned zero-valued audio metadata");
  }
  ...
  if (audioDataSamplesChannelCountDifferent) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Speaker backend returned incomplete interleaved frames");
  }
  for (const float64 sample : audioData.samples) {
    const hal::boolean sampleNotFinite =
        static_cast<hal::boolean>(!XHAL_IS_FINITE(sample));
    if (sampleNotFinite) {
      XWALK_HAL_ERROR(XWALK_RUNTIME,
                      "Speaker backend returned a non-finite audio sample");
    }
  }
}
