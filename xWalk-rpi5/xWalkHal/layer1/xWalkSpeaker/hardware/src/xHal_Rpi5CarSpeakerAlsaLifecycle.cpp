 ...
 * If the volume exceeds one hundred percent.
 */
XWalkSpeakerAlsa::XWalkSpeakerAlsa(
  ...
  if (operations.decodeAudio == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Speaker ALSA adapter requires a decoder callback");
  }
  if (playbackVolumePercentValue > 100U) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Speaker ALSA volume exceeds one hundred percent");
  }
}
...
XWalkSpeakerAlsa &XWalkSpeakerAlsa::adapter(contextpointer context) {
  if (context == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Speaker ALSA callback context must not be null");
  }
