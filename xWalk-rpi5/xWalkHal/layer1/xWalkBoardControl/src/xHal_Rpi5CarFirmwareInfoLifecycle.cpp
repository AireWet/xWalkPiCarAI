...
uint8 XWalkFirmwareInfo::selectAddress(XWalkI2c &i2c) {
  const hal::boolean candidateAddressAvailable = static_cast<hal::boolean>(
      i2c.probe(XHAL_RPI5CAR_FIRMWARE_INFO_ADDRESS_1));
  if (candidateAddressAvailable) {
    return XHAL_RPI5CAR_FIRMWARE_INFO_ADDRESS_1;
  }
  const hal::boolean secondaryAddressAvailable = static_cast<hal::boolean>(
...
string XWalkFirmwareInfo::formatVersion(const XWalkFirmwareVersion &version) {
  const string majorText =
      common::uint32ToString(static_cast<uint32>(version.major));
  const string minorText =
      common::uint32ToString(static_cast<uint32>(version.minor));
  const string patchText =
      common::uint32ToString(static_cast<uint32>(version.patch));
...
XWalkFirmwareInfo::XWalkFirmwareInfo(XWalkI2c &i2c)
  ...
  XWALK_HAL_TRACE_UID1(RPI .328,
                       "Robot HAT firmware interface selected I2C address %u",
                       addressValue);
}
