 ...
 */
void XWalkBoardControl::validateDependencies(
    const XWalkGpio &mcuResetGpio, const XWalkGpio &speakerEnableGpio,
    const XWalkAdc &batteryAdc,
    boardspeakerprimecallback speakerPrimeCallback) {
  const hal::boolean mcuResetGpioPinDifferent = static_cast<hal::boolean>(
  ...
  const hal::boolean batteryAdcChannelDifferent = static_cast<hal::boolean>(
  ...
  if (batteryAdcChannelDifferent) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Board control requires battery ADC channel A4");
  }
  if (speakerPrimeCallback == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Board control speaker-prime callback is null");
  }
}
...
void XWalkBoardControl::primeSpeakerOutput() {
  speakerPrimeCallbackValue(
      speakerPrimeContextPointer,
 ...
 * If a hardware object is bound to the wrong role or the callback is null.
 */
XWalkBoardControl::XWalkBoardControl(
    XWalkGpio &mcuResetGpio, XWalkGpio &speakerEnableGpio, XWalkAdc &batteryAdc,
  ...
  XWALK_HAL_TRACE_UID3(RPI .321,
                       ...
                       mcuResetGpio.pin(), speakerEnableGpio.pin(),
                       batteryAdc.channel());
}
