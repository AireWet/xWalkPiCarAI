...
XWalkHal::speechcapturehandle openCapture(XWalkHal::contextpointer context,
    XWalkHal::stringview deviceName, XWalkHal::uint32 sampleRateHz,
    XWalkHal::uint8 channelCount, XWalkHal::uint32 periodFrames)
{
...
XWalkHal::int32 readCapture(XWalkHal::contextpointer context,
    XWalkHal::speechcapturehandle captureHandle, XWalkHal::bytevector& pcmData,
...
XWalkHal::boolean recoverCapture(XWalkHal::contextpointer context,
    XWalkHal::speechcapturehandle captureHandle, XWalkHal::int32 errorValue)
{
    static_cast<void>(context);
...
void closeCapture(XWalkHal::contextpointer context,
...
{
    return {&openCapture, &readCapture, &recoverCapture, &closeCapture,
        &recognizerReady, &recognizePcm, &recognizeFile, &cancelRecognition};
}
...
void testCompletedBackendComposition()
{
    ...
    XWalkHal::XWalkGpio speakerGpio(&state, gpioCallbacks,
        XHAL_RPI5CAR_DEVICE_DEFAULT_SPEAKER_ENABLE_PIN);
    XWalkHal::XWalkI2c i2c(&state, &probeI2c, &writeI2c, &readI2c);
    XWalkHal::XWalkAdc batteryAdc(i2c, XHAL_RPI5CAR_BOARD_CONTROL_BATTERY_ADC_CHANNEL,
        XHAL_RPI5CAR_ADC_ADDRESS_1);
    XWalkHal::XWalkBoardControl boardControl(resetGpio, speakerGpio, batteryAdc,
        &state, &primeSpeaker);
    XWalkHal::XWalkAudioAlsa audio(&state, audioOperations(),
        "test-pcm", "test-mixer", "PCM");
    XWalkHal::XWalkSpeechToTextAlsa speechBackend(&state, speechOperations(),
        "test-microphone");
    ...
    XWalkHal::XWalkLanguageModelOllama modelBackend(&state, modelOperations,
    ...
    XWalkHal::XWalkTextToSpeechAlsa speechOutputBackend(audio, &state,
    ...
    const XWalkHal::XWalkVoiceAssistantConfiguration configuration{
        "Answer briefly.", {}};
    XWalkHal::XWalkVoiceAssistant assistant(speechToText, languageModel,
