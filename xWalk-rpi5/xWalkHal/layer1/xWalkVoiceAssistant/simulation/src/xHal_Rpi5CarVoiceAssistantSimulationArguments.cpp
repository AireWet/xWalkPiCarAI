...
{
XWalkVoiceAssistantSimulationArguments::XWalkVoiceAssistantSimulationArguments(
    int32 count, charpointer values[]): targetValue{}, enabledValue(true),
    updateValue(false), validValue(false), helpValue(false)
{
    if (count == 1) { validValue = true; return; }
    const boolean helpValid = (count == 2) && (values != nullptr) && (values[1] != nullptr);
    if (helpValid)
    {
        const stringview option(values[1]); helpValue = (option == "--help") || (option == "-h");
        validValue = helpValue; return;
    }
...
boolean XWalkVoiceAssistantSimulationArguments::valid() const noexcept { return validValue; }
boolean XWalkVoiceAssistantSimulationArguments::helpRequested() const noexcept { return helpValue; }
boolean XWalkVoiceAssistantSimulationArguments::applyTraceUpdate() const
{
    if (!updateValue) { return true; }
    const boolean json = (targetValue.size() > 5U) &&
        (targetValue.substr(targetValue.size() - 5U) == ".json");
    const string argument = json ? targetValue : targetValue +
...
void XWalkVoiceAssistantSimulationArguments::parseSelector(stringview selector)
{
    ...
    if (json) { targetValue = string(selector); updateValue = true; validValue = true; return; }
    ...
    targetValue = string(target); enabledValue = enabling; updateValue = true; validValue = true;
}
} /* namespace xwalk::hal::sim */
