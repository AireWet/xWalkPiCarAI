...
{
void XWalkVoiceAssistantHostStub::configureGpio(contextpointer context, uint8 pin,
...
{
    static_cast<void>(pin); return static_cast<XWalkVoiceAssistantHostStub*>(context)->gpioValue;
}
void XWalkVoiceAssistantHostStub::writeGpio(contextpointer context, uint8 pin, boolean value)
{
    static_cast<XWalkVoiceAssistantHostStub*>(context)->gpioValue = value; static_cast<void>(pin);
}
void XWalkVoiceAssistantHostStub::registerInterrupt(contextpointer context, uint8 pin,
...
void XWalkVoiceAssistantHostStub::writeI2c(contextpointer context, uint8 address,
    uint8 reg, const bytevector& data)
{
    static_cast<void>(context); static_cast<void>(address); static_cast<void>(reg);
    static_cast<void>(data);
}
bytevector XWalkVoiceAssistantHostStub::readI2c(contextpointer context, uint8 address,
    size length)
{
...
{
    static_cast<void>(context); static_cast<void>(durationMs);
}
...
string XWalkVoiceAssistantHostStub::listen(contextpointer context, uint32 timeoutMs)
{
    ...
    static_cast<void>(timeoutMs); return "simulated question";
}
string XWalkVoiceAssistantHostStub::transcribe(contextpointer context, stringview path)
{
    static_cast<void>(context); static_cast<void>(path); return "simulated transcription";
}
...
void XWalkVoiceAssistantHostStub::setInstructions(contextpointer context,
...
void XWalkVoiceAssistantHostStub::setMaximumMessages(contextpointer context,
...
string XWalkVoiceAssistantHostStub::prompt(contextpointer context, stringview text,
    stringview imagePath)
{
    XWalkVoiceAssistantHostStub& backend = *static_cast<XWalkVoiceAssistantHostStub*>(context);
    ++backend.promptCountValue; backend.promptValue = string(text);
    static_cast<void>(imagePath); return "simulated answer";
}
...
{
    XWalkVoiceAssistantHostStub& backend = *static_cast<XWalkVoiceAssistantHostStub*>(context);
    ++backend.spokenCountValue; backend.spokenValue = string(text);
}
boolean XWalkVoiceAssistantHostStub::valid() const noexcept
{
    return (instructionsValue == "Answer briefly") && (promptValue == "simulated question") &&
        (spokenValue == "simulated answer") && (listenCountValue == 1U) &&
        (promptCountValue == 1U) && (spokenCountValue == 1U) && (stopCountValue == 1U);
}
} /* namespace xwalk::hal::sim */
