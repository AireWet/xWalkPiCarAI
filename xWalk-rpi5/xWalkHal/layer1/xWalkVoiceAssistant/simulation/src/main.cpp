...
XWalkHal::int32 main(XWalkHal::int32 count, XWalkHal::charpointer values[]) {
  xwalk::hal::XWalkTrace::configureGlobal(
      XWALK_VOICE_ASSISTANT_SIMULATION_TRACE_CONFIG_PATH,
      XWALK_VOICE_ASSISTANT_SIMULATION_TRACE_LOG_PATH);
  const xwalk::hal::sim::XWalkVoiceAssistantSimulationArguments arguments(
      count, values);
  if (!arguments.valid()) {
    XWALK_HAL_ERROR(XWALK_EXCEPTION,
                    "Invalid xWalkVoiceAssistant simulation arguments");
