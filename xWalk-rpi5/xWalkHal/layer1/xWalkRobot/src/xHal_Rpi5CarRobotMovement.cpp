...
void XWalkRobot::servoMove(const float64vector &targets, float64 speedPercent,
  ...
  const hal::boolean speedPercentNotFinite =
      static_cast<hal::boolean>(!XHAL_IS_FINITE(speedPercent));
  ...
  const hal::boolean beatsPerMinuteHasValueValueInvalid =
      static_cast<hal::boolean>(beatsPerMinute.has_value() &&
                                (!XHAL_IS_FINITE(beatsPerMinute.value()) ||
                                 (beatsPerMinute.value() <= 0.0)));
  if (beatsPerMinuteHasValueValueInvalid) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Robot movement BPM must be finite and positive");
  }
  ...
  for (uint32 index = 0U; index < servoCountValue; ++index) {
    const float64 deltaDegrees =
        XHAL_ABSOLUTE_VALUE(targets[index] - servoPositionsValue[index]);
  ...
  if (maximumDeltaDegrees > maximumUint32) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Robot movement delta exceeds its supported range");
  }
  ...
  const float64 truncatedDeltaDegrees =
      static_cast<float64>(truncatedMaximumDelta);
  const float64 millisecondsPerSecond = 1'000.0;
  const float64 scaledDeltaDegrees =
      truncatedDeltaDegrees * millisecondsPerSecond;
  ...
  const float64 stepTimeMs =
      static_cast<float64>(XHAL_RPI5CAR_ROBOT_STEP_TIME_MS);
  const float64 calculatedStepCount = totalTimeMs / stepTimeMs;
  const hal::boolean calculatedStepCountMaximumUint32Invalid =
      static_cast<hal::boolean>(!XHAL_IS_FINITE(calculatedStepCount) ||
                                (calculatedStepCount > maximumUint32));
  if (calculatedStepCountMaximumUint32Invalid) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Robot movement duration exceeds its supported range");
  }
  ...
  for (uint32 stepIndex = 0U; stepIndex < stepCount; ++stepIndex) {
    ...
    for (uint32 servoIndex = 0U; servoIndex < servoCountValue; ++servoIndex) {
      const float64 deltaDegrees =
          targets[servoIndex] - startPositions[servoIndex];
      const float64 completedDeltaDegrees = deltaDegrees * completionRatio;
      servoPositionsValue[servoIndex] =
          startPositions[servoIndex] + completedDeltaDegrees;
    }
    servoWriteAll(servoPositionsValue);

    const uint64 elapsedUs = common::monotonicMicroseconds() - startTimeUs;
    const uint64 stepDurationUs =
        static_cast<uint64>(XHAL_RPI5CAR_ROBOT_STEP_TIME_MS) * 1'000U;
  ...
  XWALK_HAL_TRACE_UID1(RPI .342,
                       "Robot movement completed in %u interpolation step(s)",
                       stepCount);
}
 ...
 */
void XWalkRobot::setAction(stringview actionName,
                           const float64vectorvector &motions) {
  requireInitialized();
  const hal::boolean actionNameEmpty =
      static_cast<hal::boolean>(actionName.empty());
  if (actionNameEmpty) {
  ...
  XWALK_HAL_TRACE_UID3(RPI .343, "Robot action %.*s stored with %zu frame(s)",
                       static_cast<int32>(actionName.size()), actionName.data(),
                       motions.size());
}
 ...
 * unsupported.
 */
void XWalkRobot::doAction(stringview actionName, uint32 repetitions,
                          float64 speedPercent) {
  requireInitialized();
  const auto action = actions.find(string(actionName));
  const hal::boolean actionActionsMatched =
      static_cast<hal::boolean>(action == actions.end());
  ...
  XWALK_HAL_TRACE_UID3(RPI .344, "Robot action %.*s completed %u time(s)",
                       static_cast<int32>(actionName.size()), actionName.data(),
                       repetitions);
}
