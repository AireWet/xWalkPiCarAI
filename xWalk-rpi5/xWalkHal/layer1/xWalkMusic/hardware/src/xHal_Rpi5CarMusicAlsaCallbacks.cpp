...
uint8 XWalkMusicAlsa::volumePercent(float64 normalizedVolume) {
  const hal::boolean normalizedVolumeNotFinite =
      static_cast<hal::boolean>(!XHAL_IS_FINITE(normalizedVolume));
  ...
  if ((normalizedVolume < 0.0) || (normalizedVolume > 1.0)) {
    XWALK_HAL_ERROR(
        XWALK_RANGE,
        "Music ALSA normalized volume must be between zero and one");
  }
  const float64 scaledVolume =
      normalizedVolume * XHAL_RPI5CAR_MUSIC_VOLUME_PERCENT_DIVISOR;
  const uint32 roundedVolume =
      common::roundedValue(scaledVolume, "Music ALSA volume", 0U, 100U);
 ...
 */
void XWalkMusicAlsa::playSound(contextpointer context, stringview filename,
  ...
  const hal::boolean normalizedVolumeProvided =
      static_cast<hal::boolean>(normalizedVolume.has_value());
 ...
 * Optional normalized mixer volume from zero through one.
 */
void XWalkMusicAlsa::playSoundBackground(contextpointer context,
                                         stringview filename,
                                         optionalfloat64 normalizedVolume) {
  XWalkMusicAlsa &self = adapter(context);
 ...
 */
void XWalkMusicAlsa::playMusic(contextpointer context, stringview filename,
                               int32 loops, float64 startSeconds) {
  XWalkMusicAlsa &self = adapter(context);
  ...
  const hal::boolean startFrameMaximumStartFrameInvalid =
      static_cast<hal::boolean>(!XHAL_IS_FINITE(startFrameValue) ||
                                (startFrameValue > maximumStartFrame));
  if (startFrameMaximumStartFrameInvalid) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Music ALSA playback offset is unrepresentable");
  }
  ...
  if (startFrame >= totalFrames) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Music ALSA playback offset exceeds the decoded audio");
  }
  {
    const mutexlock lock(self.stateMutex);
 ...
 * Finite normalized volume in the inclusive range zero through one.
 */
void XWalkMusicAlsa::setMusicVolume(contextpointer context,
                                    float64 normalizedVolume) {
  XWalkMusicAlsa &self = adapter(context);
  self.audioBackend->setVolume(volumePercent(normalizedVolume));
}
 ...
 */
float64 XWalkMusicAlsa::getSoundLength(contextpointer context,
                                       stringview filename) {
  XWalkMusicAlsa &self = adapter(context);
