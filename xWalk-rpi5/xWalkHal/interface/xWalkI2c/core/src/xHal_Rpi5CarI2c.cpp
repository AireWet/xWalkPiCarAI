...
boolean XWalkI2c::probe(uint8 address) {
  ...
  if (addressInvalid) {
    XWALK_HAL_ERROR(XWALK_EXCEPTION,
                    "I2C probe address exceeds the seven-bit range: %u",
  ...
  XWALK_HAL_TRACE_UID1(RPI .001, "I2C probe requested for address %u",
                       static_cast<uint32>(address));
  const boolean deviceAvailable = probeCallback(contextValue, address);
  XWALK_HAL_TRACE_UID2(
      RPI .002, "I2C probe completed for address %u with status %u",
...
bytevector XWalkI2c::read(uint8 address, size length) {
  ...
  if (addressInvalid) {
    XWALK_HAL_ERROR(XWALK_EXCEPTION,
                    "I2C read address exceeds the seven-bit range: %u",
                    static_cast<uint32>(address));
  ...
  const mutexlock transactionLock(transactionMutexValue);
  XWALK_HAL_TRACE_UID2(
      RPI .003, "I2C sequential read requested for address %u and %zu bytes",
...
bytevector XWalkI2c::readRegister(uint8 address, uint8 reg, size length) {
  ...
  if (readRegisterCallback == nullptr) {
    XWALK_HAL_WARNING(XWALK_SYSTEM,
                      "I2C atomic register-read operation is unavailable");
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "I2C register-read callback is not configured");
  }
  const mutexlock transactionLock(transactionMutexValue);
 ...
 * If `address` exceeds the seven-bit protocol range.
 */
bytevector XWalkI2c::writeRegisterThenRead(uint8 address, uint8 reg,
  ...
  if (length == 0U) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "I2C combined-transaction read length must not be zero");
  }
  const mutexlock transactionLock(transactionMutexValue);
...
void XWalkI2c::writeRegister(uint8 address, uint8 reg, const bytevector &data) {
  ...
  writeRegisterCallback(contextValue, address, reg, data);
  XWALK_HAL_TRACE_UID2(
      RPI .008, "I2C register write completed for address %u and register %u",
      static_cast<uint32>(address), static_cast<uint32>(reg));
}
 ...
 */
boolean XWalkI2c::tryWriteRegister(uint8 address, uint8 reg,
