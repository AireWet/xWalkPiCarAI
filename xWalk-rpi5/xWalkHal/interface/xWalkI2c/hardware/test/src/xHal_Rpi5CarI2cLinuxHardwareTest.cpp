...
XWalkHal::int32 main(XWalkHal::int32 argumentCount,
  ...
  if (argumentCount != 2) {
    XWALK_HAL_ERROR(XWALK_EXCEPTION,
                    "xWalkI2c hardware test requires exactly one device path");
  ...
  xwalk::hal::XWalkI2c i2c(
      &backend, XHAL_I2C_PROBE_CALLBACK(xwalk::hal::XWalkI2cLinux),
  ...
  const hal::boolean primaryAddressAvailable =
      static_cast<hal::boolean>(i2c.probe(XHAL_RPI5CAR_I2C_ADDRESS_1));
  if (primaryAddressAvailable) {
    XWALK_HAL_TRACE_UID0(RPI .034,
                         "Robot HAT responded at primary I2C address");
    return 0;
  }
  const hal::boolean secondaryAddressAvailable =
      static_cast<hal::boolean>(i2c.probe(XHAL_RPI5CAR_I2C_ADDRESS_2));
  if (secondaryAddressAvailable) {
    XWALK_HAL_TRACE_UID0(RPI .035,
                         "Robot HAT responded at secondary I2C address");
    return 0;
  }
  const hal::boolean tertiaryAddressAvailable =
      static_cast<hal::boolean>(i2c.probe(XHAL_RPI5CAR_I2C_ADDRESS_3));
  if (tertiaryAddressAvailable) {
    XWALK_HAL_TRACE_UID0(RPI .036,
                         "Robot HAT responded at tertiary I2C address");
    return 0;
  }
  XWALK_HAL_ERROR(XWALK_EXCEPTION,
                  "No supported Robot HAT I2C address responded");
