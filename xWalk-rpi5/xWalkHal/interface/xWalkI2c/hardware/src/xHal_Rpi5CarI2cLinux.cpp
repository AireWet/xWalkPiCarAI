...
boolean XWalkI2cLinux::selectAddress(uint8 address) {
  ...
  if (addressSelected == false) {
    XWALK_HAL_TRACE_UID1(RPI .010,
                         "Linux I2C address selection failed for address %u",
...
boolean XWalkI2cLinux::probeDevice(uint8 address) {
  const boolean addressInvalid =
      address > common::I2C_MAXIMUM_SEVEN_BIT_ADDRESS;
  if (addressInvalid) {
  ...
  XWALK_HAL_TRACE_UID1(RPI .011, "Linux I2C probe started for address %u",
                       static_cast<uint32>(address));
  ...
  for (uint32 attempt = 0U; attempt < retryCountValue; ++attempt) {
    ...
    if (transferSucceeded) {
      XWALK_HAL_TRACE_UID2(
          RPI .012, "Linux I2C probe succeeded for address %u on attempt %u",
...
boolean XWalkI2cLinux::readByteOnce(uint8 &value) {
  ...
  const boolean readSucceeded =
      deviceInterfaceValue.transfer(fileDescriptor, &request);
  if (readSucceeded) {
    value = smbusData.byte;
    XWALK_HAL_TRACE_UID1(RPI .014, "Linux I2C byte read returned %u",
...
bytevector XWalkI2cLinux::readDevice(uint8 address, size length) {
  ...
  for (uint32 attempt = 0U; attempt < retryCountValue; ++attempt) {
    ...
    if (readSucceeded) {
      XWALK_HAL_TRACE_UID1(RPI .016,
                           "Linux I2C sequential read succeeded on attempt %u",
                           attempt + 1U);
      return bytes;
    }
    XWALK_HAL_TRACE_UID1(
        RPI .017, "Linux I2C sequential read attempt %u failed", attempt + 1U);
  }
 ...
 */
bytevector XWalkI2cLinux::readRegisterDevice(uint8 address, uint8 reg,
                                             size length) {
  const boolean addressInvalid =
      address > common::I2C_MAXIMUM_SEVEN_BIT_ADDRESS;
  if (addressInvalid) {
  ...
  if (length > XHAL_RPI5CAR_I2C_SMBUS_BLOCK_MAX) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "I2C register-read length exceeds SMBus block size");
  }
  ...
  for (uint32 attempt = 0U; attempt < retryCountValue; ++attempt) {
    ...
    if (readSucceeded && (returnedLength == length)) {
      ...
      XWALK_HAL_TRACE_UID1(RPI .019,
                           "Linux I2C register read succeeded on attempt %u",
                           attempt + 1U);
      return bytes;
    }
    XWALK_HAL_TRACE_UID2(
  ...
  XWALK_HAL_ERROR(XWALK_RUNTIME,
                  "Linux I2C register read failed after retries");
}
 ...
 */
void XWalkI2cLinux::writeRegisterDevice(uint8 address, uint8 reg,
  ...
  const hal::boolean payloadTooLarge = static_cast<hal::boolean>(
  ...
  const mutexlock lock(mutex);
  for (uint32 attempt = 0U; attempt < retryCountValue; ++attempt) {
    const hal::boolean registerWriteSucceeded = static_cast<hal::boolean>(
        selectAddress(address) && writeRegisterOnce(reg, payload));
    if (registerWriteSucceeded) {
      XWALK_HAL_TRACE_UID1(RPI .023,
                           "Linux I2C register write succeeded on attempt %u",
                           attempt + 1U);
      return;
    }
    XWALK_HAL_TRACE_UID1(RPI .024, "Linux I2C register-write attempt %u failed",
                         attempt + 1U);
  }

  XWALK_HAL_ERROR(XWALK_RUNTIME,
                  "Linux I2C register write failed after retries");
}
 ...
 */
boolean
XWalkI2cLinux::tryWriteRegisterDevice(uint8 address, uint8 reg,
                                      const bytevector &payload) noexcept {
  const hal::boolean addressPayloadInvalid = static_cast<hal::boolean>(
