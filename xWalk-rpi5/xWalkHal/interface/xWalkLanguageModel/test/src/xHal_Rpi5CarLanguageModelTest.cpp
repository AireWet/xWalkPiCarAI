...
void testConversationConfiguration() {
  ...
  assert(backend.maximumMessages ==
         XHAL_RPI5CAR_LANGUAGE_MODEL_DEFAULT_MAXIMUM_MESSAGES);
  languageModel.setMaximumMessages(35U);
  assert(backend.maximumMessages == 35U);
  assert(backend.limitCount == 2U);

  languageModel.addMessage(XWalkLanguageModelRole::User, "Inspect this",
                           "frame.jpg");
...
void testCallbackValidation() {
  const fixedarray<XWalkLanguageModelCallbacks, 5U> incompleteCallbacks{
      XWalkLanguageModelCallbacks{nullptr, &setWelcome, &setMaximumMessages,
                                  &addMessage, &prompt},
      XWalkLanguageModelCallbacks{&setInstructions, nullptr,
                                  &setMaximumMessages, &addMessage, &prompt},
      XWalkLanguageModelCallbacks{&setInstructions, &setWelcome, nullptr,
                                  &addMessage, &prompt},
      XWalkLanguageModelCallbacks{&setInstructions, &setWelcome,
                                  &setMaximumMessages, nullptr, &prompt},
      XWalkLanguageModelCallbacks{&setInstructions, &setWelcome,
                                  &setMaximumMessages, &addMessage, nullptr}};
...
void testSimulationArguments() {
  ...
  const sim::XWalkLanguageModelSimulationArguments malformed(3,
                                                             malformedValues);
