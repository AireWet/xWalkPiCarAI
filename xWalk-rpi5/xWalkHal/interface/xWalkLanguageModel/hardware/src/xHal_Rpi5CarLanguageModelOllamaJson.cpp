...
string XWalkLanguageModelOllama::encodeImage(stringview imagePath) {
  const hal::boolean imagePathEmpty =
      static_cast<hal::boolean>(imagePath.empty());
  if (imagePathEmpty) {
    return {};
  }
  const filesystempath path{imagePath};
  const hal::boolean imageFileTooLarge = static_cast<hal::boolean>(
      filesystemFileSize(path) >
      XHAL_RPI5CAR_LANGUAGE_MODEL_OLLAMA_MAXIMUM_IMAGE_BYTES);
  ...
  const hal::boolean contentsTooLarge = static_cast<hal::boolean>(
      contents.size() > XHAL_RPI5CAR_LANGUAGE_MODEL_OLLAMA_MAXIMUM_IMAGE_BYTES);
  if (contentsTooLarge) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Ollama image changed beyond its bounded byte count");
  }
  ...
  while (processingLoopRequested) {
    const hal::boolean imageByteAvailable =
        static_cast<hal::boolean>(byteOffset < contents.size());
    if (imageByteAvailable == false) {
      break;
    }
    const uint32 first =
        static_cast<uint8>(static_cast<unsigned char>(contents[byteOffset]));
    const boolean hasSecond = (byteOffset + 1U) < contents.size();
    const boolean hasThird = (byteOffset + 2U) < contents.size();
    const uint32 second =
        hasSecond ? static_cast<uint8>(
                        static_cast<unsigned char>(contents[byteOffset + 1U]))
                  : 0U;
    const uint32 third =
        hasThird ? static_cast<uint8>(
                       static_cast<unsigned char>(contents[byteOffset + 2U]))
                 : 0U;
...
string XWalkLanguageModelOllama::buildRequest(
  ...
  const auto appendWithSeparator =
  ...
  const hal::boolean instructionsAvailable =
      static_cast<hal::boolean>(!instructionsValue.empty());
  if (instructionsAvailable) {
    appendWithSeparator(
        {XWalkLanguageModelRole::System, instructionsValue, {}});
  }
  const hal::boolean welcomeAvailable =
      static_cast<hal::boolean>(!welcomeValue.empty());
  if (welcomeAvailable) {
  ...
  request += "]}";
  const hal::boolean requestTooLarge = static_cast<hal::boolean>(
      request.size() >
      XHAL_RPI5CAR_LANGUAGE_MODEL_OLLAMA_MAXIMUM_REQUEST_BYTES);
  if (requestTooLarge) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Ollama request exceeds its bounded byte count");
  }
  return request;
}
 ...
 * @throws std::out_of_range If serialization exceeds the request limit.
 */
void XWalkLanguageModelOllama::appendJsonString(string &output,
                                                stringview value) {
  constexpr stringview hexadecimal{"0123456789ABCDEF"};
  output.push_back('"');
  for (const char character : value) {
    const uint8 byte =
        static_cast<uint8>(static_cast<unsigned char>(character));
    ...
    const hal::boolean outputTooLarge = static_cast<hal::boolean>(
        output.size() >
        XHAL_RPI5CAR_LANGUAGE_MODEL_OLLAMA_MAXIMUM_REQUEST_BYTES);
    if (outputTooLarge) {
      XWALK_HAL_ERROR(XWALK_RANGE,
 ...
 * @throws std::out_of_range If serialization exceeds the request limit.
 */
void XWalkLanguageModelOllama::appendMessageJson(
    string &output, const XWalkLanguageModelOllamaMessage &message) const {
  output += "{\"role\":";
  appendJsonString(output, roleName(message.role));
  output += ",\"content\":";
  const hal::boolean imageBase64Empty =
      static_cast<hal::boolean>(message.imageBase64.empty());
  if (imageBase64Empty) {
 ...
 */
string
XWalkLanguageModelOllama::extractResponseContent(stringview responseJson) {
  const size messageOffset = responseJson.find("\"message\"");
  const size contentOffset =
      (messageOffset == stringview::npos)
          ? stringview::npos
          : responseJson.find("\"content\"", messageOffset + 9U);
  const size colonOffset = (contentOffset == stringview::npos)
                               ? stringview::npos
  ...
  while (whitespaceParsingRequested) {
    const hal::boolean whitespaceAvailable =
        static_cast<hal::boolean>((quoteOffset < responseJson.size()) &&
                                  ((responseJson[quoteOffset] == ' ') ||
                                   (responseJson[quoteOffset] == '\t') ||
                                   (responseJson[quoteOffset] == '\r') ||
                                   (responseJson[quoteOffset] == '\n')));
 ...
 */
string XWalkLanguageModelOllama::decodeJsonString(stringview json,
                                                  size quoteOffset) {
  string output{};
  ...
  while (jsonDecodingRequested) {
    const hal::boolean jsonByteAvailable =
        static_cast<hal::boolean>(byteOffset < json.size());
    ...
    if (character != '\\') {
      if (static_cast<uint8>(static_cast<unsigned char>(character)) < 0x20U) {
        XWALK_HAL_ERROR(XWALK_RUNTIME,
                        "Ollama response contains invalid JSON text");
      }
      output.push_back(character);
      continue;
    }
    ...
    if (jsonEscapeIncomplete) {
      XWALK_HAL_ERROR(XWALK_RUNTIME,
                      "Ollama response ends during JSON escaping");
    }
    ...
    if ((escape == '"') || (escape == '\\') || (escape == '/')) {
      ...
      const hal::boolean unicodeEscapeIncomplete =
          static_cast<hal::boolean>((json.size() - byteOffset) < 4U);
      if (unicodeEscapeIncomplete) {
        XWALK_HAL_ERROR(XWALK_RUNTIME,
                        "Ollama response has incomplete Unicode escaping");
      }
      uint32 codePoint{};
      for (size digitOffset = 0U; digitOffset < 4U; ++digitOffset) {
        codePoint = (codePoint << 4U) |
                    hexadecimalDigit(json[byteOffset + digitOffset]);
      }
      byteOffset += 4U;
      if ((codePoint >= 0xD800U) && (codePoint <= 0xDBFFU)) {
        const hal::boolean jsonByteOffsetUInvalid = static_cast<hal::boolean>(
            ((json.size() - byteOffset) < 6U) || (json[byteOffset] != '\\') ||
            (json[byteOffset + 1U] != 'u'));
        if (jsonByteOffsetUInvalid) {
          XWALK_HAL_ERROR(XWALK_RUNTIME,
                          "Ollama response has incomplete Unicode pair");
        }
        uint32 lowSurrogate{};
        for (size digitOffset = 0U; digitOffset < 4U; ++digitOffset) {
        ...
        if ((lowSurrogate < 0xDC00U) || (lowSurrogate > 0xDFFFU)) {
          XWALK_HAL_ERROR(XWALK_RUNTIME,
                          "Ollama response has invalid Unicode pair");
        }
        ...
        XWALK_HAL_ERROR(XWALK_RUNTIME,
                        "Ollama response has isolated Unicode surrogate");
      }
      appendUtf8(output, codePoint);
    } else {
      XWALK_HAL_ERROR(XWALK_RUNTIME,
                      "Ollama response has unsupported JSON escaping");
    }
  }
  XWALK_HAL_ERROR(XWALK_RUNTIME,
                  "Ollama response has an unterminated JSON string");
}
...
uint8 XWalkLanguageModelOllama::hexadecimalDigit(char value) {
  ...
  XWALK_HAL_ERROR(XWALK_RUNTIME,
                  "Ollama response has invalid Unicode hexadecimal data");
}
...
void XWalkLanguageModelOllama::appendUtf8(string &output, uint32 codePoint) {
  if (codePoint <= 0x7FU) {
    ...
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Ollama response has an invalid Unicode code point");
  }
}
