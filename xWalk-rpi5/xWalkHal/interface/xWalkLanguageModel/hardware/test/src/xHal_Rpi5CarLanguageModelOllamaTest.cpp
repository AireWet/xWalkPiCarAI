 ...
 */
XWalkHal::filesystempath
writeImageFixture(const XWalkHal::filesystempath &testDirectory) {
  const XWalkHal::filesystempath imagePath = testDirectory / "ollama-image.bin";
  XWalkHal::outputfilestream image(imagePath,
...
void testRequestAndResponse(const XWalkHal::filesystempath &imagePath) {
  ...
  assert(transport.request.find("\"model\":\"tiny\"") !=
         XWalkHal::string::npos);
  assert(transport.request.find("answer \\\"briefly\\\"") !=
         XWalkHal::string::npos);
  assert(transport.request.find("inspect\\u000Aimage") !=
         XWalkHal::string::npos);
  assert(transport.request.find("\"images\":[\"AQID\"]") !=
         XWalkHal::string::npos);
...
void testOpenAiCompatibleRequest(const XWalkHal::filesystempath &imagePath) {
  ...
  transport.response =
      ...
      "\"content\":\"cloud-ready\"}}]}";
  XWalkHal::XWalkLanguageModelHttp backend(
  ...
  assert(transport.request.find("\"model\":\"selected-model\"") !=
         XWalkHal::string::npos);
  assert(transport.request.find("\"max_tokens\":321") !=
         XWalkHal::string::npos);
  assert(transport.request.find("\"type\":\"image_url\"") !=
         XWalkHal::string::npos);
  assert(transport.request.find("data:image/jpeg;base64,AQID") !=
         XWalkHal::string::npos);
}
...
void testProviderNames() {
  ...
  for (const XWalkHal::stringview provider :
       {"openai", "chatgpt", "gemini", "grok", "xai", "claude", "anthropic",
        "openai_compatible"}) {
  ...
  xwalk::hal::test::expectFailure([]() {
    static_cast<void>(
        XWalkHal::XWalkLanguageModelHttp::dialectFromString("kiro"));
  });
  xwalk::hal::test::expectFailure([]() {
    static_cast<void>(
        XWalkHal::XWalkLanguageModelHttp::dialectFromString("unknown"));
  });
}
...
void testHistory() {
  ...
  XWalkHal::XWalkLanguageModelOllama backend(
      &transport, operations(), "http://localhost:11434/api/chat", "tiny");
  XWalkHal::XWalkLanguageModel model(&backend, backend.callbacks());
...
void testValidation() {
  ...
  xwalk::hal::test::expectFailure([&]() {
    XWalkHal::XWalkLanguageModelHttp backend(
        &transport, operations(),
        XWalkHal::XWalkLanguageModelHttpDialect::OpenAiChatCompletions,
        "https://api.example/v1/chat/completions", "model", "key", 5'000U, 0U);
  });
  ...
  xwalk::hal::test::expectFailure([&]() {
    model.addMessage(static_cast<XWalkHal::XWalkLanguageModelRole>(9U),
                     "invalid");
  });
...
XWalkHal::int32 main(XWalkHal::int32 argumentCount,
  ...
  XWalkHal::XWalkTrace::configureGlobal(
  ...
  const XWalkHal::filesystempath imagePath =
      writeImageFixture(argumentValues[1]);
  ...
  testValidation();
  testFailures();
  XWALK_HAL_TRACE_UID0(RPI .156,
                       "HTTP language-model provider tests completed");
  return 0;
}
