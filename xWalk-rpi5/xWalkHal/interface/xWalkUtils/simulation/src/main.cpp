...
XWalkHal::int32 main(XWalkHal::int32 argumentCount,
  ...
  xwalk::hal::XWalkTrace::configureGlobal(
      XWALK_UTILS_SIMULATION_TRACE_CONFIG_PATH,
      XWALK_UTILS_SIMULATION_TRACE_LOG_PATH);
  const xwalk::hal::sim::XWalkUtilsSimulationArguments arguments(
  ...
  XWALK_HAL_TRACE_UID1(
      RPI .138, "xWalkUtils simulation completed with status %d", result);
  return result;
}
