...
void XWalkUtilsLinux::writeAll(int32 descriptor, stringview bytes) {
  ...
  while (processingLoopRequested) {
    const hal::boolean bytesRemaining =
        static_cast<hal::boolean>(writtenByteCount < bytes.size());
    if (bytesRemaining == false) {
      break;
    }
    const size remainingByteCount = bytes.size() - writtenByteCount;
    const auto writeResult = ::write(
        descriptor, bytes.data() + writtenByteCount, remainingByteCount);
    if (writeResult > 0) {
...
void XWalkUtilsLinux::setSystemVolume(uint8 volumePercent) const {
  const string command =
      "amixer -M sset 'PCM' " + std::to_string(volumePercent) + "%";
  ...
  XWALK_HAL_TRACE_UID1(RPI .131, "Linux utility set PCM volume to %u percent",
...
XWalkCommandResult XWalkUtilsLinux::runCommandProcess(stringview command,
  ...
  const hal::boolean ownedUserAvailable =
      static_cast<hal::boolean>(!ownedUser.empty());
  ...
  const hal::boolean ownedGroupAvailable =
      static_cast<hal::boolean>(!ownedGroup.empty());
  ...
  if (outputPipeDifferent) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Linux utility command pipe creation failed");
  }
  ...
  if (childProcess < 0) {
    ...
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Linux utility command process creation failed");
  }

  if (childProcess == 0) {
    static_cast<void>(::close(outputPipe[0U]));
    const hal::boolean outputPipeInvalid =
        static_cast<hal::boolean>((::dup2(outputPipe[1U], STDOUT_FILENO) < 0) ||
    ...
    const hal::boolean groupChangeFailed = static_cast<hal::boolean>(
        changeGroup && (::setgid(static_cast<gid_t>(groupId)) != 0));
    if (groupChangeFailed) {
      ::_exit(127);
    }
    const hal::boolean userChangeFailed = static_cast<hal::boolean>(
  ...
  while (true) {
    const auto readResult =
        ::read(outputPipe[0U], buffer.data(), buffer.size());
    if (readResult > 0) {
      ...
      XWALK_HAL_ERROR(XWALK_RUNTIME,
                      "Linux utility command output read failed");
    }
  }
  static_cast<void>(::close(outputPipe[0U]));
  ...
  const hal::boolean childExitedNormally =
      static_cast<hal::boolean>(WIFEXITED(childStatus));
  ...
  XWALK_HAL_TRACE_UID1(
      RPI .132, "Linux utility command completed with status %d", status);
...
boolean XWalkUtilsLinux::executableExists(stringview executable) const {
  const hal::boolean executableEmpty =
      static_cast<hal::boolean>(executable.empty());
  if (executableEmpty) {
    return false;
  }
  ...
  while (pathSearchRequested) {
    ...
    const hal::boolean executableFileMatched =
        static_cast<hal::boolean>(isExecutableFile(candidate));
...
string XWalkUtilsLinux::interfaceIpv4(stringview interfaceName) const {
  ...
  const hal::boolean interfaceListDifferent =
      static_cast<hal::boolean>(::getifaddrs(&interfaceList) != 0);
  if (interfaceListDifferent) {
    return {};
  }

  string address;
  for (const ifaddrs *entry = interfaceList; entry != nullptr;
    ...
    const hal::boolean ipv4AddressSinAddrTextDifferent =
        static_cast<hal::boolean>(::inet_ntop(AF_INET, &ipv4Address->sin_addr,
                                              text.data(),
                                              text.size()) != nullptr);
...
int32 XWalkUtilsLinux::redirectStandardError() const {
  ...
  if (nullDescriptor < 0) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Linux utility could not open the null device");
  }
  ...
  if (restoreToken < 0) {
    ...
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Linux utility could not duplicate standard error");
  }

  const hal::boolean stderrRedirectFailed =
      static_cast<hal::boolean>(::dup2(nullDescriptor, STDERR_FILENO) < 0);
  if (stderrRedirectFailed) {
    static_cast<void>(::close(restoreToken));
    static_cast<void>(::close(nullDescriptor));
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Linux utility could not redirect standard error");
  }
  static_cast<void>(::close(nullDescriptor));
  return restoreToken;
}
