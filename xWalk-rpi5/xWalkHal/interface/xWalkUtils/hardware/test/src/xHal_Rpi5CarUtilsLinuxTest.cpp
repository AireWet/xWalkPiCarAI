...
void testOutput() {
  ...
  assert(output.find("\033[0;37mlinux\033[0m!") != string::npos);
}
...
void testPlatformQueries() {
  ...
  const XWalkCommandResult result =
      utilities.runCommand("printf 'standard'; printf 'error' >&2; exit 7");
...
void testStderrGuard() {
  ...
  assert(output.find("suppressed") == string::npos);
  assert(output.find("restored") != string::npos);
}
