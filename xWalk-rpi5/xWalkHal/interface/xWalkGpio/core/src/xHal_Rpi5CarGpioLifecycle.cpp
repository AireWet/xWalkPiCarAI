...
uint8 XWalkGpio::validatePin(uint32 pin) {
  const boolean supported = (pin == 4U) || (pin == 5U) || (pin == 6U) ||
                            (pin == 8U) || (pin == 12U) || (pin == 13U) ||
                            (pin == 16U) || (pin == 17U) || (pin == 19U) ||
                            (pin == 20U) || (pin == 21U) || (pin == 22U) ||
                            (pin == 23U) || (pin == 24U) || (pin == 25U) ||
                            (pin == 26U) || (pin == 27U);
  if (supported == false) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "GPIO pin is not present in the Robot HAT map");
  }
 ...
 */
XWalkGpioCallbacks
XWalkGpio::validateCallbacks(const XWalkGpioCallbacks &callbacks) {
  if ((callbacks.configure == nullptr) || (callbacks.read == nullptr) ||
      (callbacks.write == nullptr) || (callbacks.interrupt == nullptr) ||
      (callbacks.cancelInterrupt == nullptr)) {
...
XWalkGpio::XWalkGpio(contextpointer context,
                     const XWalkGpioCallbacks &callbacks, uint32 pin,
                     XWalkGpioMode mode, XWalkGpioPull pull, boolean activeHigh)
    : contextValue(context), callbacksValue(validateCallbacks(callbacks)),
      pinValue(validatePin(pin)), modeValue(mode), pullValue(pull),
      activeHighValue(activeHigh) {
  callbacksValue.configure(contextValue, pinValue, modeValue, pullValue,
 ...
 */
XWalkGpio::XWalkGpio(contextpointer context,
                     const XWalkGpioCallbacks &callbacks, stringview pinName,
                     XWalkGpioMode mode, XWalkGpioPull pull, boolean activeHigh)
    : XWalkGpio(context, callbacks, static_cast<uint32>(resolvePin(pinName)),
                mode, pull, activeHigh) {}
