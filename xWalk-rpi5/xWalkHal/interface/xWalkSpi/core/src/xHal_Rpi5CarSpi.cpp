...
bytevector XWalkSpi::transfer(const bytevector &transmitData) {
  ...
  XWALK_HAL_TRACE_UID1(RPI .045, "SPI transfer requested for %zu bytes",
                       transmitData.size());
  ...
  if (receivedDataTransmitDataDifferent) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "SPI backend returned an unexpected byte count");
  }
  XWALK_HAL_TRACE_UID1(RPI .046, "SPI transfer returned %zu bytes",
                       receivedData.size());
  return receivedData;
}
