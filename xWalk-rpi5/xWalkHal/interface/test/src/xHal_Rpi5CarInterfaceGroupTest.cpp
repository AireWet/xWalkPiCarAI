...
TEST(XWalkInterfaceGroup, ConfigurationInitializesBusAndDigitalInterfaces)
{
    ...
    const boolean configurationValid = i2cAddress == "20" && spiPayload == "9F00" &&
        gpioPin == "D2" && gpioDirection == "output";
    ...
    EXPECT_THAT(i2c.read(0x14U, 2U),
        ElementsAre(static_cast<uint8>(0x12U), static_cast<uint8>(0x34U)));

    SpiBackend spiBackend;
    XWalkSpi spi(&spiBackend, &transferSpi);
    EXPECT_THAT(spi.transfer({0x9FU, 0x00U}),
        ElementsAre(static_cast<uint8>(0xA5U), static_cast<uint8>(0x5AU)));
    ...
    EXPECT_THAT(spiBackend.transmitted,
        ElementsAre(static_cast<uint8>(0x9FU), static_cast<uint8>(0x00U)));
...
TEST(XWalkInterfaceGroup, InvalidOrFailedInitializationStopsLaterInterfaces)
{
    ...
    const boolean configurationValid = configuration.get("bus", "i2c_address") == "20" &&
        configuration.get("gpio", "direction") == "output";
    ...
    EXPECT_THROW(
        static_cast<void>(XWalkUtils::mapping(1.0, 5.0, 5.0, 0.0, 100.0)),
...
TEST(XWalkInterfaceGroup, AudioConfigurationFlowsToInjectedAlsaBoundary)
{
    ...
    XWalkConfig configuration(path.string());
    static_cast<void>(configuration.read());
    configuration.setSection("audio", {{"sample_rate_hz", "44100"}, {"channels", "2"},
        {"volume_percent", "75"}});
    ...
    XWalkAudioAlsa failingAudio(
        &failingBackend, testOperations(), "group-pcm", "group-mixer", "PCM");
...
TEST(XWalkInterfaceGroup, LanguageModelConfigurationAndBackendResultsPropagate)
{
    ...
    configuration.setSection(
        "model", {{"instructions", "Be concise"}, {"maximum_messages", "35"}});
