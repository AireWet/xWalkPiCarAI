...
audiopcmhandle XWalkAudioHostStub::openPcm(contextpointer context,
  ...
  XWALK_HAL_TRACE_UID1(RPI .090, "Host Audio opened mirrored PCM %s",
                       ownedDeviceName.c_str());
  return handle;
}

boolean XWalkAudioHostStub::configurePcm(
    contextpointer context, audiopcmhandle pcmHandle,
    const XWalkAudioStreamConfiguration &configuration) {
  static_cast<void>(context);
  return (pcmHandle != nullptr) && (configuration.sampleRateHz > 0U);
}

int32 XWalkAudioHostStub::writePcm(contextpointer context,
                                   audiopcmhandle pcmHandle,
                                   const bytevector &pcmData, size byteOffset,
                                   size frameCount) {
...
boolean XWalkAudioHostStub::recoverPcm(contextpointer context,
                                       audiopcmhandle pcmHandle,
                                       int32 errorValue) {
  static_cast<void>(context);
...
void XWalkAudioHostStub::closePcm(contextpointer context,
                                  audiopcmhandle pcmHandle) {
  static_cast<void>(context);
  static_cast<void>(pcmHandle);
}

audiomixerhandle XWalkAudioHostStub::openMixer(contextpointer context,
                                               stringview deviceName) {
  auto &stub = *static_cast<XWalkAudioHostStub *>(context);
  const boolean deviceValid = deviceName.empty() == false;
  ...
  const string ownedDeviceName(deviceName);
  XWALK_HAL_TRACE_UID1(RPI .091, "Host Audio opened mirrored mixer %s",
                       ownedDeviceName.c_str());
...
boolean XWalkAudioHostStub::setMixerVolume(contextpointer context,
                                           ...
                                           uint8 volumePercent) {
  auto &stub = *static_cast<XWalkAudioHostStub *>(context);
  const boolean requestValid = (mixerHandle != nullptr) &&
                               (elementName.empty() == false) &&
                               (volumePercent <= 100U);
  if (requestValid) {
    stub.volumePercentValue = volumePercent;
    XWALK_HAL_TRACE_UID1(RPI .092, "Host Audio mirrored volume %u percent",
                         static_cast<uint32>(volumePercent));
  }
  return requestValid;
}

void XWalkAudioHostStub::closeMixer(contextpointer context,
                                    audiomixerhandle mixerHandle) {
  static_cast<void>(context);
