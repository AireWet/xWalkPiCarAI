...
XWalkCameraLinux::XWalkCameraLinux(XWalkCameraConnection connection,
    ...
    : connectionValue(connection), executableName(executable),
      usbDevicePath(usbDevice) {
...
XWalkCameraLinux &XWalkCameraLinux::backend(contextpointer context) {
  if (context == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Camera Linux backend context must not be null");
  }
...
boolean
XWalkCameraLinux::captureImage(contextpointer context, stringview outputPath,
...
boolean XWalkCameraLinux::captureCsi(
  ...
  if (childProcess == 0) {
    ...
    ::execlp(executableName.c_str(), executableName.c_str(), "--nopreview",
             "--immediate", "--timeout", timeout.c_str(), "--width",
             width.c_str(), "--height", height.c_str(), "--encoding", "jpg",
             "--output", ownedOutput.c_str(),
             static_cast<charpointer>(nullptr));
    ::_exit(127);
  }
  return waitForProcess(static_cast<int32>(childProcess),
                        configuration.timeoutMs);
}

/** @brief Runs one USB V4L2 capture process without a shell. */
boolean XWalkCameraLinux::captureUsb(
    stringview outputPath,
    const XWalkCameraConfiguration &configuration) const {
  ...
  if (childProcess == 0) {
    ::execlp(executableName.c_str(), executableName.c_str(), "-nostdin",
             "-loglevel", "error", "-f", "v4l2", "-video_size",
             videoSize.c_str(), "-i", usbDevicePath.c_str(), "-frames:v", "1",
             "-y", "-t", "1", ownedOutput.c_str(),
             static_cast<charpointer>(nullptr));
    ::_exit(127);
  }
  return waitForProcess(static_cast<int32>(childProcess),
 ...
 */
boolean XWalkCameraLinux::waitForProcess(int32 processId,
                                         uint32 timeoutMs) noexcept {
  constexpr uint32 pollIntervalMilliseconds{10U};
  int processStatus{};
  const steadytimestamp deadline =
      steadyclock::now() + millisecondduration(timeoutMs);
...
boolean XWalkCameraLinux::validOutput(stringview outputPath) {
  ...
  const hal::boolean validFile =
      filesystemEntryExists(path) && isRegularFile(path);
  if (validFile == false) {
    return false;
  }
