...
float64 XWalkServo::angleToPulseWidth(float64 angleDegrees) const noexcept {
  const boolean lowerHalf =
      angleDegrees <= configurationValue.centreAngleDegrees;
  const float64 minimumAngleDegrees =
      lowerHalf ? configurationValue.minimumAngleDegrees
                : configurationValue.centreAngleDegrees;
  const float64 maximumAngleDegrees =
      lowerHalf ? configurationValue.centreAngleDegrees
                : configurationValue.maximumAngleDegrees;
  const float64 minimumPulseWidthUs =
      lowerHalf ? configurationValue.minimumPulseWidthUs
                : configurationValue.centrePulseWidthUs;
  const float64 maximumPulseWidthUs =
      lowerHalf ? configurationValue.centrePulseWidthUs
                : configurationValue.maximumPulseWidthUs;
...
void XWalkServo::validateConfiguration(
    const XWalkServoConfiguration &configuration) {
  const boolean finite = XHAL_IS_FINITE(configuration.minimumAngleDegrees) &&
                         XHAL_IS_FINITE(configuration.centreAngleDegrees) &&
                         XHAL_IS_FINITE(configuration.maximumAngleDegrees) &&
                         XHAL_IS_FINITE(configuration.minimumPulseWidthUs) &&
                         XHAL_IS_FINITE(configuration.centrePulseWidthUs) &&
                         XHAL_IS_FINITE(configuration.maximumPulseWidthUs);
  if (finite == false) {
    XWALK_HAL_ERROR(XWALK_INVAL, "servo.configuration values must be finite");
  }
  if (!((configuration.minimumAngleDegrees <
         configuration.centreAngleDegrees) &&
        (configuration.centreAngleDegrees <
         configuration.maximumAngleDegrees))) {
    XWALK_HAL_ERROR(
        XWALK_INVAL,
        "servo.angle limits must satisfy minimum < centre < maximum");
  }
  if (!((configuration.minimumPulseWidthUs <
         configuration.centrePulseWidthUs) &&
        (configuration.centrePulseWidthUs <
         configuration.maximumPulseWidthUs))) {
    XWALK_HAL_ERROR(
        XWALK_INVAL,
        "servo.pulse_width limits must satisfy minimum < centre < maximum");
  }
  if ((configuration.minimumPulseWidthUs < XHAL_RPI5CAR_SERVO_MIN_PULSE_US) ||
    ...
    XWALK_HAL_ERROR(
        XWALK_RANGE,
        "servo.pulse_width must remain within 500 to 2500 microseconds");
  }
}
...
void XWalkServo::requireInitialized() const {
  if (initializedValue == false) {
    XWALK_HAL_ERROR(XWALK_LOGIC,
                    "servo.initialize must succeed before an output command");
  }
}
...
void XWalkServo::setAngle(float64 angleDegrees) {
  ...
  const hal::boolean angleDegreesNotFinite =
      static_cast<hal::boolean>(!XHAL_IS_FINITE(angleDegrees));
  ...
  if (configurationValue.inverted) {
    calibratedAngle = configurationValue.centreAngleDegrees -
                      (angleDegrees - configurationValue.centreAngleDegrees);
  }
  ...
  XWALK_HAL_TRACE_UID1(RPI .184, "Servo angle configured to %.3f degrees",
                       clampedAngle);
}
...
void XWalkServo::setPulseWidthTime(float64 pulseWidthUs) {
  ...
  const hal::boolean pulseWidthUsNotFinite =
      static_cast<hal::boolean>(!XHAL_IS_FINITE(pulseWidthUs));
  ...
  const float64 servoFrameUs =
      static_cast<float64>(XHAL_RPI5CAR_SERVO_FRAME_US);
  ...
  XWALK_HAL_TRACE_UID1(RPI .185,
                       "Servo pulse width configured to %.3f microseconds",
                       clampedPulseWidth);
}
