...
void XWalkComputerVision::pollQr(XWalkComputerVisionResult &result) {
  ...
  const agent::boolean newQrCodeDetected =
      static_cast<agent::boolean>(!qrData.empty() && (qrData != lastQrData));
 ...
 */
XWalkComputerVisionResult
XWalkComputerVision::handleKey(agent::stringview keyText) {
  if (!startedValue) {
    XWALK_RPIAGENT_ERROR(
        XWALK_LOGIC, "Computer vision must be started before handling keys");
  }
  ...
  const agent::boolean keyTextMatched =
      static_cast<agent::boolean>(keyText.size() == 1U);
  if (keyTextMatched) {
    char key = keyText[0U];
    if ((key >= 'A') && (key <= 'Z')) {
