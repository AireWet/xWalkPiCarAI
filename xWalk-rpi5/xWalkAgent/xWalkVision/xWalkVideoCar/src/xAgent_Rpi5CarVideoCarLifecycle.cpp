...
XWalkVideoCar::XWalkVideoCar(
    ...
    : picarxObject(&picarx), callbackContext(context),
      callbacks(providerCallbacks), configurationValue(configuration) {
...
void XWalkVideoCar::validate(
  ...
  const agent::boolean steeringAngleDegreesNotFinite =
      static_cast<agent::boolean>(
          !std::isfinite(configuration.steeringAngleDegrees));
  if (steeringAngleDegreesNotFinite) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Video-car steering angle must be finite");
  }
  if ((configuration.speedStepPercent == 0U) ||
      ...
      (configuration.directionChangeCapPercent >
       configuration.maximumSpeedPercent) ||
      (configuration.steeringAngleDegrees <= 0.0) ||
      (configuration.steeringAngleDegrees > 45.0) ||
      (configuration.startupDelayMs > 10'000U) ||
      (configuration.keyDelayMs == 0U) || (configuration.keyDelayMs > 1'000U)) {
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "Video-car configuration is out of range");
  }
}
...
agent::boolean XWalkVideoCar::started() const noexcept { return startedValue; }
