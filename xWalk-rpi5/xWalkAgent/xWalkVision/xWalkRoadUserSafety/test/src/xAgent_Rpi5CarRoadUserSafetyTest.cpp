...
TEST(XWalkRoadUserSafety, EmptyAndDistantRoadAreSafe)
{
    ...
    distant.batch.detections.push_back(detection(
        XWalkRoadUserClass::Person, 0.95, 20.0, 0.2, 0.1, 0.01));
...
TEST(XWalkRoadUserSafety, ApproachingOrUncertainRoadUserWarns)
{
    ...
    backend.batch.detections = {
        detection(XWalkRoadUserClass::Person, 0.9, 5.0, 1.0, 0.2, 0.08),
...
TEST(XWalkRoadUserSafety, ImmediateOrAggregateDangerStopsMotion)
{
    ScenarioBackend backend;
    backend.batch.detections = {
        detection(XWalkRoadUserClass::Car, 0.98, 15.0, 0.0, 0.8, 0.1),
...
{
    const XWalkRoadSafetyStatus statuses[] = {
        XWalkRoadSafetyStatus::CameraUnavailable,
...
{
    ScenarioBackend backend;
    backend.batch.detections.push_back(detection(
        XWalkRoadUserClass::Person, std::numeric_limits<float64>::quiet_NaN(),
        4.0, 1.0, 0.0, 0.1));
...
TEST(XWalkRoadUserSafety, ProviderStatusFailuresFailSafe)
{
    ...
    classifierFailure.batch.detections.push_back(detection(
        XWalkRoadUserClass::Person, 0.9, 4.0, 1.0, 0.0, 0.1));
