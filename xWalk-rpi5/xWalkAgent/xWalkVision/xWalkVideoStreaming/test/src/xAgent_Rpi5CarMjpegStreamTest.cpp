...
TEST(XWalkMjpegStream, ValidatesConfigurationAndIdempotentLifecycle)
{
    ...
    EXPECT_EQ(validateMjpegStreamConfiguration(configuration),
        XWalkMjpegStreamStatus::InvalidConfiguration);
...
TEST(XWalkMjpegStream, DeliversOrderedMultipartFramesToMultipleClients)
{
    ...
    ASSERT_EQ(publishMjpegFrame(state, jpegFrame(0x11U, 16U)),
        XWalkMjpegStreamStatus::Ok);
    ASSERT_EQ(publishMjpegFrame(state, jpegFrame(0x22U, 20U)),
        XWalkMjpegStreamStatus::Ok);

    bytevector multipart;
    uint64 sequence{};
    EXPECT_EQ(popMjpegMultipartFrame(state, 1U, multipart, sequence),
        XWalkMjpegStreamStatus::Ok);
    ...
    EXPECT_EQ(popMjpegMultipartFrame(state, 2U, multipart, sequence),
        XWalkMjpegStreamStatus::Ok);
    EXPECT_EQ(sequence, 1U);
...
TEST(XWalkMjpegStream, DropsOldestFramesForSlowClientsWithoutBlockingOthers)
{
    ...
    {
        ASSERT_EQ(publishMjpegFrame(state, jpegFrame(marker, 12U)),
            XWalkMjpegStreamStatus::Ok);
    }
    ...
    bytevector multipart;
    uint64 sequence{};
    EXPECT_EQ(popMjpegMultipartFrame(state, 7U, multipart, sequence),
        XWalkMjpegStreamStatus::Ok);
...
TEST(XWalkMjpegStream, ConcurrentPublishersRemainBoundedAndSequenceOrdered)
{
    ...
    const auto publishFrames = [&state](uint8 marker)
    {
        ...
        {
            EXPECT_EQ(publishMjpegFrame(state, jpegFrame(marker, 12U)),
                XWalkMjpegStreamStatus::Ok);
        }
    };
    ...
    {
        uint64 sequence{};
        ASSERT_EQ(popMjpegMultipartFrame(state, 9U, multipart, sequence),
            XWalkMjpegStreamStatus::Ok);
...
{
    XWalkMjpegStreamState state;
    EXPECT_EQ(publishMjpegFrame(state, jpegFrame(1U, 8U)),
        XWalkMjpegStreamStatus::NotStarted);
    XWalkMjpegStreamConfiguration configuration;
    ASSERT_EQ(startMjpegStream(state, configuration), XWalkMjpegStreamStatus::Ok);
    ASSERT_EQ(addMjpegStreamClient(state, 3U), XWalkMjpegStreamStatus::Ok);
    EXPECT_EQ(publishMjpegFrame(state, bytevector{1U, 2U, 3U, 4U}),
        XWalkMjpegStreamStatus::InvalidFrame);
    ASSERT_EQ(publishMjpegFrame(state, jpegFrame(3U, 8U)),
        XWalkMjpegStreamStatus::Ok);
    EXPECT_EQ(reportMjpegCameraLoss(state), XWalkMjpegStreamStatus::CameraUnavailable);
    EXPECT_TRUE(state.clients[0U].pending.empty());
    EXPECT_EQ(publishMjpegFrame(state, jpegFrame(4U, 8U)),
        XWalkMjpegStreamStatus::CameraUnavailable);
