...
agent::int32 XWalkStorytellingRobot::run()
{
    ...
    const agent::boolean firstLegRequested =
    ...
    const agent::boolean firstLegCompleted =
        driveForwardAndNarrate(configuration.firstJoke);
    if (firstLegCompleted == false)
    {
        stop();
        return 0;
    }
    const agent::boolean secondLegRequested =
        callbacks.shouldContinue(callbackContext);
    ...
    const agent::boolean secondLegCompleted =
        driveForwardAndNarrate(configuration.secondJoke);
...
agent::boolean XWalkStorytellingRobot::driveForwardAndNarrate(
    agent::stringview narration)
{
