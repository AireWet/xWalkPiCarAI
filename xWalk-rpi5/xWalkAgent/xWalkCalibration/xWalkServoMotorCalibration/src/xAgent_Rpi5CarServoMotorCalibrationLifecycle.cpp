...
XWalkServoMotorCalibration::XWalkServoMotorCalibration(
  ...
  if ((delayCallback == nullptr) || (continueCallback == nullptr)) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Servo/motor calibration requires complete callbacks");
  }
...
XWalkServoMotorCalibration::~XWalkServoMotorCalibration() { stop(); }
 ...
 */
agent::boolean
XWalkServoMotorCalibration::wait(agent::uint32 durationMs) const {
