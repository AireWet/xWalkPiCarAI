...
void setAngle(xwalk::agent::contextpointer context, xwalk::agent::uint8 servoId,
    xwalk::agent::float64 angleDegrees)
{
...
{
    TestState state;
    const xwalk::agent::XWalkServoZeroingCallbacks callbacks{
        &setAngle, &delay, &continueOperation};
    ...
    for (xwalk::agent::uint8 servoId = 0U;
        servoId < xwalk::agent::XAGENT_RPI5CAR_SERVO_ZEROING_CHANNEL_COUNT;
        ++servoId)
    {
