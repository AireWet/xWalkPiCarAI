    ...
    bootapplicationcallback callback, hal::XWalkConfigStore& config)
{
    ...
    cameraConfiguration.widthPixels = parseUnsigned(
        config.get("text_vision_width", "1280"), "text_vision_width", 7'680U);
    cameraConfiguration.heightPixels = parseUnsigned(
        config.get("text_vision_height", "720"), "text_vision_height", 4'320U);
    hal::XWalkCamera camera(&cameraBackend, cameraBackend.callback(),
        cameraConfiguration);
    XWalkCameraCapture cameraCapture(camera,
        config.get("text_vision_image_path", "/tmp/llm-img.jpg"));
