 ...
 */
agent::int32 XWalkBootRpi::runVehicleMode(
  ...
  if (selectedMode == XWALK_BOOT_FACE_TRACKING_REQ) {
    ...
    return runStorytellingRobot(context, callback, config, boardControl,
                                picarx);
    ...
    return runVoiceActiveCar(context, callback, config, boardControl, picarx,
                             gpioDevice, gpioChipName, gpioChipLabel,
                             minimumGpioLineCount, gpioCallbacks);
  } else if (selectedMode == XWALK_BOOT_VOICE_ACTIVE_CAR_GPT_REQ) {
    return runVoiceActiveCarGpt(context, callback, config, boardControl, picarx,
                                gpioDevice, gpioChipName, gpioChipLabel,
                                minimumGpioLineCount, gpioCallbacks);
