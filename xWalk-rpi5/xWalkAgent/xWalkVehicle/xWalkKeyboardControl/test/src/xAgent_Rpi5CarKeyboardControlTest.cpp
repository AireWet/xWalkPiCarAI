...
void testKeyboardControl(agent::stringview configurationPath)
{
    ...
    {
        const xwalk::agent::XWalkKeyboardControlResult result =
            keyboardControl.handleKey(key);
    ...
    assert(keyboardControl.handleKey("invalid") ==
        xwalk::agent::XWalkKeyboardControlResult::Ignored);

    schedule.queryLimit = schedule.queryCount;
    assert(keyboardControl.handleKey("w") ==
        xwalk::agent::XWalkKeyboardControlResult::Cancelled);
