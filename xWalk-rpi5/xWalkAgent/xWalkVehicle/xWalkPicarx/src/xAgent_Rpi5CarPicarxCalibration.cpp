...
void XWalkPicarx::recordCalibrationVerified(agent::boolean verified) {
  ...
  configStoreObject->set("picarx_calibration_verified",
                         verified ? "true" : "false");
  maximumMotorOutputPercentValue =
      verified ? configuredMaximumMotorOutputPercentValue
 ...
 */
void XWalkPicarx::calibrateMotorDirection(agent::uint8 motorId,
                                          agent::int32 direction) {
  previewMotorDirection(motorId, direction);
  const agent::fixedarray<agent::int32, 2U> directions = motorDirections();
  const agent::string stored =
      agent::string("[") + hal::common::int32ToString(directions[0U]) + ", " +
 ...
 * @param[in] direction Direction value equal to 1 or -1.
 */
void XWalkPicarx::previewMotorDirection(agent::uint8 motorId,
                                        agent::int32 direction) {
  ...
  if ((direction != 1) && (direction != -1)) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "PiCar-X motor direction must be 1 or -1");
  }
  if (motorId == 1U) {
...
XWalkPicarx::motorDirections() const noexcept {
  const hal::XWalkMotorsConfiguration configuration =
      motorsObject->configuration();
  return {configuration.leftReversed ? -1 : 1,
          configuration.rightReversed ? -1 : 1};
}
...
void XWalkPicarx::calibrateDirectionServo(agent::float64 value) {
  ...
  configStoreObject->set(
      "picarx_dir_servo",
      hal::common::float64ToString(directionCalibrationDegreesValue));
}
...
void XWalkPicarx::setDirectionServoAngle(agent::float64 value) {
  ...
  directionServoObject->setAngle(directionAngleDegreesValue +
...
void XWalkPicarx::calibrateCameraPanServo(agent::float64 value) {
  ...
  configStoreObject->set(
      "picarx_cam_pan_servo",
      hal::common::float64ToString(cameraPanCalibrationDegreesValue));
}
