 ...
 */
agent::float64
XWalkPicarx::calibratedMotorSpeed(agent::uint8 motorId,
                                  agent::float64 speedPercent) const {
  if ((motorId < 1U) || (motorId > 2U)) {
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "PiCar-X motor identifier must be 1 or 2");
  }
  ...
  const agent::float64 calibratedMagnitude = constrain(
      scaledMagnitude - motorSpeedCalibrationValues[index], 0.0, 100.0);
  const agent::float64 limitedMagnitude =
      constrain(calibratedMagnitude, 0.0, maximumMotorOutputPercentValue);
 ...
 * percent.
 */
void XWalkPicarx::setMotorSpeed(agent::uint8 motorId,
                                agent::float64 speedPercent) {
  const agent::boolean emergencyStopRequested =
      static_cast<agent::boolean>(emergencyStopRequestedValue.load());
  if (emergencyStopRequested) {
    return;
  }
  const agent::float64 command = calibratedMotorSpeed(motorId, speedPercent);
  const agent::float64 left =
      (motorId == 1U) ? command : motorsObject->left().speed();
  const agent::float64 right =
      (motorId == 2U) ? command : motorsObject->right().speed();
...
void XWalkPicarx::forward(agent::float64 speedPercent) {
  ...
  const agent::float64 absoluteAngle =
      XHAL_ABSOLUTE_VALUE(directionAngleDegreesValue);
  const agent::float64 powerScale = (100.0 - absoluteAngle) / 100.0;
  const agent::float64 leftRaw =
      (directionAngleDegreesValue > 0.0) ? speed * powerScale : speed;
  const agent::float64 rightRaw =
      (directionAngleDegreesValue < 0.0) ? speed * powerScale : speed;
  motorsObject->setSpeed(calibratedMotorSpeed(1U, leftRaw),
                         calibratedMotorSpeed(2U, rightRaw));
}
...
void XWalkPicarx::backward(agent::float64 speedPercent) {
  ...
  const agent::float64 powerScale = (100.0 - absoluteAngle) / 100.0;
  const agent::float64 leftRaw =
      (directionAngleDegreesValue > 0.0) ? speed : speed * powerScale;
  const agent::float64 rightRaw =
      (directionAngleDegreesValue > 0.0) ? speed * powerScale : speed;
  motorsObject->setSpeed(calibratedMotorSpeed(1U, leftRaw),
                         calibratedMotorSpeed(2U, rightRaw));
...
void XWalkPicarx::stop() { motorsObject->stop(); }
...
void XWalkPicarx::close() {
  ...
  if (stopped == false) {
    XWALK_RPIAGENT_ERROR(
        XWALK_RUNTIME, "PiCar-X shutdown could not disable every motor output");
  }
