...
namespace
{
void testStorytellingRobot(
    xwalk::agent::test::ControllerCommandTestContext& context)
{
    xwalk::agent::test::XWalkControllerSequence sequence(
        *context.storytellingController);
    assert(sequence.run({{"storytelling-robot", "start"},
        {"storytelling-robot", "stop"}}) == 0);
    assert(context.state->spokenText == ctrl::stringvector({
        "Hello! I'm PiCar-X speaking with Piper.",
    ...
    assert(std::all_of(context.state->delays.begin(),
        context.state->delays.end(), [](ctrl::uint32 delayMs)
        {
            return delayMs == 20U;
        }));
    ...
    assert(std::all_of(context.state->leftSpeeds.begin(),
        ...
        [](ctrl::float64 speed) { return speed > 0.0; }));
