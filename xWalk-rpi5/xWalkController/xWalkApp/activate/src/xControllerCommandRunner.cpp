 ...
 */
::ctrl::int32 XWALK_runControllerCommand(XWalkController& controller,
    const ::ctrl::stringvector& arguments)
{
    const XWalkControllerCommandRequest request =
        XWALK_parseControllerCommand(arguments);
    ...
    {
        return controller.XWALK_handlerSpi(
            XWALK_parseSpiRequest(request.arguments));
    }
    else if (request.command == XWALK_CNTRL_DOCTOR_REQ)
    {
        return controller.XWALK_handlerDoctor(XWALK_parseNoArgumentRequest(
            request.arguments, "doctor accepts no additional arguments"));
    }
    else if (request.command == XWALK_CNTRL_SERVO_ZEROING_REQ)
    {
        return controller.XWALK_handlerServoZeroing(XWALK_parseNoArgumentRequest(
            request.arguments, "servo-zeroing does not accept arguments"));
    }
    else if (request.command == XWALK_CNTRL_COMPUTER_VISION_REQ)
    {
        return controller.XWALK_handlerComputerVision(XWALK_parseNoArgumentRequest(
            request.arguments, "computer-vision accepts no arguments"));
    }
    else if (request.command == XWALK_CNTRL_RECORD_VIDEO_REQ)
    {
        return controller.XWALK_handlerVideoRecording(XWALK_parseNoArgumentRequest(
            request.arguments, "record-video accepts no arguments"));
    }
    else if (request.command == XWALK_CNTRL_SOUND_BACKGROUND_MUSIC_REQ)
    {
        return controller.XWALK_handlerSoundBackgroundMusic(
            XWALK_parseNoArgumentRequest(request.arguments,
                "sound-background-music accepts no arguments"));
    }
    else if (request.command == XWALK_CNTRL_TEXT_VISION_TALK_REQ)
    {
        return controller.XWALK_handlerTextVisionTalk(XWALK_parseLifecycleRequest(
            request.arguments, "text-vision-talk requires exactly start or stop"));
    }
    else if (request.command == XWALK_CNTRL_ONLINE_LLM_TEST_REQ)
    {
        return controller.XWALK_handlerOnlineLlmTest(XWALK_parseLifecycleRequest(
            request.arguments, "online-llm-test requires exactly start or stop"));
    }
