...
::ctrl::int32 XWALK_runController(::ctrl::contextpointer context,
                                  agent::XWalkBootServices &services) {
  ...
  const XWalkControllerCallbacks callbacks{
      &XWALK_outputLine, &XWALK_inputLine, &XWALK_delayMilliseconds,
      &XWALK_continueOperation, &XWALK_performSound};
  ...
  if (services.computerVision != nullptr) {
    XWalkController cli(*services.computerVision, &applicationContext,
                        callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }
  ...
  if ((services.languageModel != nullptr) &&
    ...
    xwalk::agent::XWalkTextVisionTalk textVisionTalk(
        *services.languageModel, *services.cameraCapture, &applicationContext,
        talkCallbacks);
  ...
  if (services.videoRecording != nullptr) {
    XWalkController cli(*services.videoRecording, &applicationContext,
                        callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }

  if (services.soundBackgroundMusic != nullptr) {
    XWalkController cli(*services.soundBackgroundMusic, &applicationContext,
                        callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }
  ...
  if (services.faceTracking != nullptr) {
    if (services.picarx == nullptr) {
      XWALK_CTRL_ERROR(XWALK_LOGIC,
                       "xWalkBoot did not provide PiCar-X for face tracking");
    }

    XWalkController cli(*services.picarx, *services.faceTracking,
  ...
  if (services.bullFight != nullptr) {
    if (services.picarx == nullptr) {
      XWALK_CTRL_ERROR(XWALK_LOGIC,
                       "xWalkBoot did not provide PiCar-X for bull fight");
    }

    XWalkController cli(*services.picarx, *services.bullFight,
                        &applicationContext, callbacks);
  ...
  if (services.treasureHunt != nullptr) {
    if (services.picarx == nullptr) {
      XWALK_CTRL_ERROR(XWALK_LOGIC,
                       "xWalkBoot did not provide PiCar-X for treasure hunt");
    }

    XWalkController cli(*services.picarx, *services.treasureHunt,
                        &applicationContext, callbacks);
  ...
  if (services.videoCar != nullptr) {
    if (services.picarx == nullptr) {
      XWALK_CTRL_ERROR(XWALK_LOGIC,
                       "xWalkBoot did not provide PiCar-X for video car");
    }

    XWalkController cli(*services.picarx, *services.videoCar,
                        &applicationContext, callbacks);
  ...
  if (services.appControl != nullptr) {
    if (services.picarx == nullptr) {
      XWALK_CTRL_ERROR(XWALK_LOGIC,
                       "xWalkBoot did not provide PiCar-X for app control");
    }

    XWalkController cli(*services.picarx, *services.appControl,
                        &applicationContext, callbacks);
  ...
  if (services.selfDrive != nullptr) {
    services.selfDrive->setCancellation(&applicationContext,
                                        &XWALK_continueOperation);
  }

  if (services.lineTracking != nullptr) {
    XWalkController cli(*services.picarx, *services.lineTracking,
                        &applicationContext, callbacks);
  ...
  if (services.voiceAssistant != nullptr) {
    const ::ctrl::boolean commandArgumentsValid = static_cast<::ctrl::boolean>(
        !commandArguments.empty() && (commandArguments[0U] == "voice-chat"));

    if (commandArgumentsValid) {
      const xwalk::agent::XWalkLocalVoiceChatbotCallbacks voiceCallbacks{
          &XWALK_outputLine, &XWALK_continueOperation,
          &XWALK_delayMilliseconds};
      xwalk::agent::XWalkLocalVoiceChatbot chatbot(*services.voiceAssistant,
                                                   nullptr, voiceCallbacks);

      XWalkController cli(*services.picarx, chatbot, &applicationContext,
                          callbacks);
    ...
    if ((services.selfDrive == nullptr) ||
        (services.voiceStatusLed == nullptr) ||
        (services.cameraCapture == nullptr)) {
      XWALK_CTRL_ERROR(
          XWALK_LOGIC,
    ...
    if (voiceActiveCarGptRequested) {
      ...
      const ::ctrl::boolean gptCarRequested = static_cast<::ctrl::boolean>(
          !commandArguments.empty() && (commandArguments[0U] == "gpt-car"));
      if (gptCarRequested) {
    ...
    xwalk::agent::XWalkVoiceActiveCar voiceActiveCar(
        ...
        *services.voiceStatusLed, services.cameraCapture, voiceCallbacks,
        voiceConfiguration);
    ...
    if (gptCarRequested) {
      ...
      XWalkController cli(*services.picarx, gptCar, &applicationContext,
                          callbacks);
      return XWALK_runControllerCommand(cli, commandArguments);
    }

    XWalkController cli(*services.picarx, voiceActiveCar, &applicationContext,
                        callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }

  if (services.selfDrive != nullptr) {
    XWalkController cli(*services.picarx, *services.selfDrive,
                        &applicationContext, callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }

  if (services.localVoiceChatbot != nullptr) {
    XWalkController cli(*services.picarx, *services.localVoiceChatbot,
                        &applicationContext, callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }

  if (services.voiceActiveCar != nullptr) {
    XWalkController cli(*services.picarx, *services.voiceActiveCar,
                        &applicationContext, callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }

  if (services.voiceControlledCar != nullptr) {
    XWalkController cli(*services.picarx, *services.voiceControlledCar,
                        &applicationContext, callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }
  ...
  if (services.speechToText != nullptr) {
    ...
    XWalkController cli(*services.picarx, voiceControlledCar,
                        &applicationContext, callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }

  if (services.voicePromptCar != nullptr) {
    XWalkController cli(*services.picarx, *services.voicePromptCar,
                        &applicationContext, callbacks);
    return XWALK_runControllerCommand(cli, commandArguments);
  }

  if (services.textToSpeech != nullptr) {
    const ::ctrl::boolean storytellingRobotRequested =
        static_cast<::ctrl::boolean>(
            !commandArguments.empty() &&
            (commandArguments[0U] == "storytelling-robot"));

    if (storytellingRobotRequested) {
      const xwalk::agent::XWalkStorytellingRobotCallbacks storyCallbacks{
          &XWALK_delayMilliseconds, &XWALK_continueOperation};

      xwalk::agent::XWalkStorytellingRobot storytellingRobot(
          *services.picarx, *services.textToSpeech, nullptr, storyCallbacks);
      XWalkController cli(*services.picarx, storytellingRobot,
                          &applicationContext, callbacks);
    ...
    XWalkController cli(*services.picarx, voicePromptCar, &applicationContext,
                        callbacks);

    return XWALK_runControllerCommand(cli, commandArguments);
  }
  xwalk::agent::XWalkGrayscaleCalibration grayscaleCalibration(
      *services.picarx, &applicationContext, &XWALK_delayMilliseconds,
      &XWALK_continueOperation);
  ...
  xwalk::agent::XWalkCliffDetection cliffDetection(
      *services.picarx, &applicationContext, &XWALK_delayMilliseconds,
      &XWALK_continueOperation);
  XWalkController cli(*services.picarx, grayscaleCalibration,
                      servoMotorCalibration, moveExample, keyboardControl,
                      obstacleAvoidance, cliffDetection, &applicationContext,
                      callbacks);
  return XWALK_runControllerCommand(cli, commandArguments);
}
