 ...
 */
::ctrl::boolean
XWalkController::calibrateServoMotor(::ctrl::boolean configureMotorDirections) {
  if (servoMotorCalibrationObject == nullptr) {
    XWALK_CTRL_ERROR(XWALK_EXCEPTION,
                     "Servo/motor-calibration backend unavailable");
    return false;
  }
  const ::ctrl::boolean servosReset =
      servoMotorCalibrationObject->resetServos();
  if (servosReset == false) {
    return false;
  }
  const ::ctrl::string servoTest =
      input("Type 'r' to test all servos, or 'skip': ");
  if ((servoTest == "r") || (servoTest == "R")) {
    const ::ctrl::boolean servosTested =
        servoMotorCalibrationObject->testServos();
  ...
  const XWalkServoCalibrationConfig steeringConfig{
      "--- Steering Servo ---",
      "Enter steering offset (-20 to 20, or 'skip'): ", -20.0, 20.0, 0U};
  const XWalkServoCalibrationConfig cameraPanConfig{
      "--- Camera Pan Servo ---",
      "Enter camera pan offset (-20 to 20, or 'skip'): ", -20.0, 20.0, 1U};
  const XWalkServoCalibrationConfig cameraTiltConfig{
      "--- Camera Tilt Servo ---",
      "Enter camera tilt offset (-20 to 20, or 'skip'): ", -20.0, 20.0, 2U};
  ...
  const ::ctrl::string motorCorrection =
      input("Enter motor balance correction (-100 to 100, positive reduces "
            "left, or 'skip'): ");
  ...
  const ::ctrl::string accepted =
      input("Persist servo offsets and motor directions? (y/n): ");
...
void XWalkController::calibrateMotorDirection(::ctrl::uint8 motorId,
                                              ::ctrl::stringview motorName) {
  const ::ctrl::string direction =
      input(::ctrl::string("Enter ") + ::ctrl::string(motorName) +
            " motor direction (1, -1, or 'skip'): ");
  if ((direction == "skip") || (direction == "SKIP")) {
    return;
  }
  const ::ctrl::float64 parsed =
      XWALK_parseNumber(direction, "motor direction", -1.0, 1.0);
  if ((parsed != -1.0) && (parsed != 1.0)) {
    XWALK_CTRL_ERROR(XWALK_INVAL, "motor direction must be 1 or -1");
  }
  servoMotorCalibrationObject->setMotorDirection(
      motorId, static_cast<::ctrl::int32>(parsed));
}
...
::ctrl::boolean XWalkController::calibrateGrayscaleReferences() {
  if (grayscaleCalibrationObject == nullptr) {
    XWALK_CTRL_ERROR(XWALK_EXCEPTION,
                     "Grayscale-calibration backend unavailable");
    return false;
  }
  picarxObject->stop();
  XWALK_CTRL_TRACE_UID0(CTRL .009,
                        "--- Automatic Grayscale and Cliff Calibration ---");
  XWALK_CTRL_TRACE_UID0(
      CTRL .010,
      "Clear the steering mechanism and provide a clear calibration surface.");
  const ::ctrl::string ready =
      input("Type 'ready' to run the steering check, or 'skip': ");
  ...
  const ::ctrl::string lineRequest =
      input("Place the car on the line calibration surface and type 'q', or "
            "'skip': ");
  if ((lineRequest != "q") && (lineRequest != "Q")) {
    return false;
  }
  const ::ctrl::boolean lineCalibrated =
      grayscaleCalibrationObject->calibrateLine();
  if (lineCalibrated == false) {
    return false;
  }
  XWALK_CTRL_TRACE_UID1(
      CTRL .011, "Pending line reference: %s",
      XWALK_FORMAT_VALUES(grayscaleCalibrationObject->result().lineReference)
          .c_str());
  ...
  const ::ctrl::boolean cliffCalibrated =
      grayscaleCalibrationObject->calibrateCliff();
  if (cliffCalibrated == false) {
    return false;
  }
  XWALK_CTRL_TRACE_UID1(
      CTRL .012, "Pending cliff reference: %s",
      XWALK_FORMAT_VALUES(grayscaleCalibrationObject->result().cliffReference)
          .c_str());
...
::ctrl::boolean XWalkController::calibrateServo(
  ...
  while (true) {
    ...
    const ::ctrl::float64 angle = XWALK_parseNumber(
        valueText, "servo calibration", configuration.minimumAngleDegrees,
        configuration.maximumAngleDegrees);
    const ::ctrl::boolean offsetApplied =
        servoMotorCalibrationObject->setServoOffset(configuration.servoId,
                                                    angle);
    if (offsetApplied == false) {
      return false;
    }
    const ::ctrl::string response = input("Is it centered? (y/n/skip): ");
    if ((response == "y") || (response == "Y") || (response == "skip") ||
        (response == "SKIP")) {
      return true;
    }
  }
