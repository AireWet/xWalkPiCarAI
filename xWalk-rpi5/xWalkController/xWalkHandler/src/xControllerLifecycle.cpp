 ...
 */
XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, ::ctrl::contextpointer context,
 ...
 */
XWalkController::XWalkController(
    agent::XWalkServoZeroing &servoZeroing, ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : servoZeroingObject(&servoZeroing), callbackContext(context),
      callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
  servoZeroingObject->setCancellation(callbackContext,
                                      callbacks.continueOperation);
}
...
XWalkController::XWalkController(
    ...
    agent::XWalkCliffDetection &cliffDetection, ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : picarxObject(&picarx), grayscaleCalibrationObject(&grayscaleCalibration),
      servoMotorCalibrationObject(&servoMotorCalibration),
      moveExampleObject(&moveExample), keyboardControlObject(&keyboardControl),
      obstacleAvoidanceObject(&obstacleAvoidance),
      cliffDetectionObject(&cliffDetection), callbackContext(context),
      callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}

XWalkController::XWalkController(
    agent::XWalkVideoRecording &videoRecording, ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : videoRecordingObject(&videoRecording), callbackContext(context),
      callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}

XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkVideoCar &videoCar,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : picarxObject(&picarx), videoCarObject(&videoCar),
      callbackContext(context), callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}

XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkAppControl &appControl,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : picarxObject(&picarx), appControlObject(&appControl),
      callbackContext(context), callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}

XWalkController::XWalkController(
    agent::XWalkSoundBackgroundMusic &soundBackgroundMusic,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : soundBackgroundMusicObject(&soundBackgroundMusic),
      callbackContext(context), callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}
 ...
 */
XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkLineTracking &lineTracking,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
 ...
 */
XWalkController::XWalkController(
    agent::XWalkComputerVision &computerVision, ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : computerVisionObject(&computerVision), callbackContext(context),
      callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}

XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkFaceTracking &faceTracking,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : picarxObject(&picarx), faceTrackingObject(&faceTracking),
      callbackContext(context), callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}

XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkBullFight &bullFight,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : picarxObject(&picarx), bullFightObject(&bullFight),
      callbackContext(context), callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}
 ...
 */
XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkTreasureHunt &treasureHunt,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : picarxObject(&picarx), treasureHuntObject(&treasureHunt),
      callbackContext(context), callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}
 ...
 */
XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkSelfDrive &selfDrive,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : picarxObject(&picarx), selfDriveObject(&selfDrive),
      callbackContext(context), callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
  selfDriveObject->setCancellation(callbackContext,
                                   callbacks.continueOperation);
}
 ...
 */
XWalkController::XWalkController(
    agent::XWalkSpiTransfer &spiTransfer, ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
 ...
 */
XWalkController::XWalkController(
    const ::ctrl::stringvector &doctorLines, ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : doctorLinesObject(&doctorLines), callbackContext(context),
      callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}
...
XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkVoicePromptCar &voicePromptCar,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
...
XWalkController::XWalkController(
    ...
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : picarxObject(&picarx), storytellingRobotObject(&storytellingRobot),
      callbackContext(context), callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}
...
XWalkController::XWalkController(
    agent::XWalkTextVisionTalk &textVisionTalk, ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : textVisionTalkObject(&textVisionTalk), callbackContext(context),
      callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}
...
XWalkController::XWalkController(
    agent::XWalkOnlineLlmTest &onlineLlmTest, ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : onlineLlmTestObject(&onlineLlmTest), callbackContext(context),
      callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}
...
XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkGptCar &gptCar,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
    : picarxObject(&picarx), gptCarObject(&gptCar), callbackContext(context),
      callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
}
...
XWalkController::XWalkController(
    agent::XWalkPicarx &picarx, agent::XWalkVoiceActiveCar &voiceActiveCar,
    ::ctrl::contextpointer context,
    const XWalkControllerCallbacks &backendCallbacks)
 ...
 */
void XWalkController::validateCallbacks(
    const XWalkControllerCallbacks &backendCallbacks) {
