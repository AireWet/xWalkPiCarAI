...
::ctrl::int32 XWalkController::XWALK_handlerComputerVision(
  ...
  if (started == false) {
    XWALK_CTRL_ERROR(XWALK_EXCEPTION,
                     "Computer-vision camera could not be started");
    return 2;
  }

  XWALK_CTRL_TRACE_UID0(
      CTRL .045, "Computer vision keys: q photo; 1-6 colors; 0 color off; "
  ...
  while (processingLoopRequested) {
    ...
    if ((key == "x") || (key == "X") || (key == "exit") || (key == "stop") ||
        (key == "skip")) {
      break;
    }
    const agent::XWalkComputerVisionResult result =
        computerVisionObject->handleKey(key);
    if (result.event == agent::XWalkComputerVisionEvent::PhotoCaptured) {
      XWALK_CTRL_TRACE_UID1(CTRL .046, "photo save as %s",
                            result.photoPath.c_str());
    } else if (result.event == agent::XWalkComputerVisionEvent::ColorChanged) {
      XWALK_CTRL_TRACE_UID1(
          CTRL .047, "Color detect : %s",
          agent::XWalkComputerVision::colorName(result.color).c_str());
    } else if (result.event == agent::XWalkComputerVisionEvent::FaceChanged) {
      XWALK_CTRL_TRACE_UID1(CTRL .048, "Face Detect:%s",
                            result.faceEnabled ? "True" : "False");
    } else if (result.event == agent::XWalkComputerVisionEvent::QrChanged) {
      XWALK_CTRL_TRACE_UID1(CTRL .049, "%s",
                            result.qrEnabled ? "Waitting for QR code"
                                             : "QRcode Detect: close");
      ...
      if (result.color != agent::XWalkComputerVisionColor::Close) {
        XWALK_CTRL_TRACE_UID2(
            CTRL .050, "%s%s",
            result.observation.color.count == 0U ? "Color Detect: "
                                                 : "[Color Detect] ",
      ...
      if (result.faceEnabled) {
        XWALK_CTRL_TRACE_UID2(
            CTRL .051, "%s%s",
            result.observation.face.count == 0U ? "Face Detect: "
                                                : "[Face Detect] ",
    ...
    if (result.qrChanged) {
      XWALK_CTRL_TRACE_UID1(CTRL .052, "QR code:%s",
                            result.observation.qrData.c_str());
    }
