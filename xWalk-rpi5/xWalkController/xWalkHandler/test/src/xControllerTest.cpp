...
void writeRegister(ctrl::contextpointer context, ctrl::uint8 address, ctrl::uint8 reg,
...
ctrl::boolean tryWriteRegister(ctrl::contextpointer context, ctrl::uint8 address,
    ctrl::uint8 reg, const ctrl::bytevector& data) noexcept
{
    static_cast<void>(context);
    static_cast<void>(address);
    static_cast<void>(reg);
...
ctrl::bytevector read(ctrl::contextpointer context, ctrl::uint8 address,
    ctrl::size length)
{
    static_cast<void>(address);
...
void configureGpio(ctrl::contextpointer context, ctrl::uint8 pin,
    XWalkHal::XWalkGpioMode mode, XWalkHal::XWalkGpioPull pull, ctrl::boolean initialValue)
{
...
ctrl::string inputLine(ctrl::contextpointer context, ctrl::stringview prompt)
{
    ...
    const ::ctrl::boolean inputUnavailable =
        static_cast<::ctrl::boolean>(
            backend.inputIndex >= backend.inputLines.size());
    if (inputUnavailable)
    {
        return "skip";
    }
...
ctrl::boolean selfDriveDelayMilliseconds(ctrl::contextpointer context,
...
ctrl::boolean soundOperation(ctrl::contextpointer context,
 ...
 */
void playMusicSound(ctrl::contextpointer context, ctrl::stringview filename,
    ctrl::optionalfloat64 normalizedVolume)
{
 ...
 */
void playMusicFile(ctrl::contextpointer context, ctrl::stringview filename,
 ...
 */
void playMusicTone(ctrl::contextpointer context, const ctrl::bytevector& pcmData,
...
void testCommands(ctrl::stringview configPath)
{
    ...
    const xwalk::ctrl::XWalkControllerCallbacks callbacks{&outputLine, &inputLine,
    ...
    xwalk::agent::XWalkCliffDetection cliffDetection(
        picarx, &backend, &delayMilliseconds, &continueOperation);
    xwalk::ctrl::XWalkController cli(
        picarx, grayscaleCalibration, servoMotorCalibration, moveExample,
        keyboardControl, obstacleAvoidance,
        cliffDetection, &backend, callbacks);
    ...
    const ctrl::stringvector passingDoctorReport{
    ...
    const ctrl::stringvector selfDriveActions{
    ...
    assert(xwalk::ctrl::XWALK_runControllerCommand(
        cli, {"move", "forward", "--speed", "40", "--duration=0.25"}) == 0);
    ...
    delayStart = backend.delays.size();
    assert(xwalk::ctrl::XWALK_runControllerCommand(
        cli, {"move", "forward", "--speed", "40", "--duration", "1"}) == 0);
    delayedMs = 0U;
    for (ctrl::size index = delayStart; index < backend.delays.size(); ++index)
    {
        delayedMs += backend.delays[index];
    ...
    assert(backend.musicSoundFile ==
        (ctrl::filesystempath(XWALK_TEST_SOUND_DIRECTORY) /
    ...
    assert(xwalk::ctrl::XWALK_runControllerCommand(cli,
        {"sound", "play", "../xWalk-rpi5/xWalkAudioResources/sounds/car-double-horn.wav", "--volume", "80"}) == 0);
    ...
    assert(backend.soundRequest.volumePercent.has_value() &&
        (*backend.soundRequest.volumePercent == 80.0));
    assert(xwalk::ctrl::XWALK_runControllerCommand(cli, {"sound", "music",
        "../xWalk-rpi5/xWalkAudioResources/music/slow-trail-Ahjay_Stelino.mp3"}) == 0);
    assert(backend.soundRequest.operation == xwalk::ctrl::XWalkSoundOperation::Music);
    assert(backend.soundRequest.filePath ==
        "../xWalk-rpi5/xWalkAudioResources/music/slow-trail-Ahjay_Stelino.mp3");
    assert(backend.soundRequest.volumePercent.has_value() &&
        (*backend.soundRequest.volumePercent == 20.0));
    assert(xwalk::ctrl::XWALK_runControllerCommand(
        cli, {"sound", "volume", "0"}) == 0);
    assert(backend.soundRequest.operation == xwalk::ctrl::XWalkSoundOperation::Volume);
    assert(backend.soundRequest.volumePercent.has_value() &&
        (*backend.soundRequest.volumePercent == 0.0));
    assert(xwalk::ctrl::XWALK_runControllerCommand(
        cli, {"sound", "volume", "100"}) == 0);
    assert(backend.soundRequest.volumePercent.has_value() &&
    ...
    backend.inputLines = {"skip", "5", "y", "-2", "y", "3", "y", "10", "1", "-1",
        "raised", "y", "y", "y", "y", "y"};
    backend.inputIndex = 0U;
    ...
    backend.inputLines = {"skip", "skip", "skip", "skip", "10", "raised", "y", "y", "y", "y",
        "y", "ready", "q", "e", "y"};
    backend.inputIndex = 0U;
    ...
    {
        static_cast<void>(xwalk::ctrl::XWALK_runControllerCommand(
            cli, {"move", "forward", "--speed", "101"}));
    });
    xwalk::hal::test::expectFailure([&]()
    {
        static_cast<void>(xwalk::ctrl::XWALK_runControllerCommand(
