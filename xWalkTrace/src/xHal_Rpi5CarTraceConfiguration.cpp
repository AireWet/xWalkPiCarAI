...
void XWalkTrace::loadConfiguration(const filesystempath& configurationPath)
{
        ...
        module = module->NextSiblingElement("module"))
    {
        ...
        const boolean moduleAttributesPresent =
            (moduleName != nullptr) && (moduleDefault != nullptr);
        const stringview moduleState = moduleDefault == nullptr ? "" : moduleDefault;
        const boolean moduleStateValid =
            (moduleState == "enable") || (moduleState == "disable");
            ...
            trace = trace->NextSiblingElement("trace"))
        {
            ...
            const boolean traceStateValid =
                (traceState == "enable") || (traceState == "disable");
            ...
            const string normalizedNumericId = numericIdIsZero ?
                "0" : numericIdText.substr(firstNonZero);
            const string scopedNumericId = string(moduleName) + "." + normalizedNumericId;
            const boolean numericIdUnique = (numericId != nullptr) &&
                (loadedScopedNumericIds.find(scopedNumericId) ==
                    loadedScopedNumericIds.end());
            const boolean attributesValid = (uid != nullptr) &&
                (numericId != nullptr) && (defaultState != nullptr) &&
                (sourceFile != nullptr) && isValidUid(uid) &&
                traceStateValid && numericIdUnique &&
                ...
                (trace->QueryUnsignedAttribute("priority", &priority) ==
