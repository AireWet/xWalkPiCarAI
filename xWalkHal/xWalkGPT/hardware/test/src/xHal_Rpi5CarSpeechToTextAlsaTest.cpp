...
XWalkHal::speechcapturehandle openCapture(XWalkHal::contextpointer context,
    XWalkHal::stringview device, XWalkHal::uint32 rate, XWalkHal::uint8 channels,
...
XWalkHal::int32 readCapture(XWalkHal::contextpointer context,
    XWalkHal::speechcapturehandle handle, XWalkHal::bytevector& data,
...
XWalkHal::boolean recoverCapture(XWalkHal::contextpointer context,
    XWalkHal::speechcapturehandle handle, XWalkHal::int32 error)
{
...
XWalkHal::XWalkSpeechToTextAlsaOperations operations()
{
    return {&openCapture, &readCapture, &recoverCapture, &closeCapture,
        &ready, &recognizePcm, &recognizeFile, &cancel};
}
...
void testCancellation()
{
    ...
    for (XWalkHal::uint32 waitCount{};
        !backend.readStarted.load() && (waitCount < 1'000U); ++waitCount)
    {
