...
XWalkTextToSpeechPiper& XWalkTextToSpeechPiper::provider(
    contextpointer context)
{
    if (context == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Piper provider context must not be null");
    }
...
void XWalkTextToSpeechPiper::speak(contextpointer context, stringview text)
{
    ...
    if (textTooLarge)
    {
        XHAL_THROW_OUT_OF_RANGE("Piper text exceeds the bounded input length");
    }
...
void XWalkTextToSpeechPiper::execute(stringview text) const
{
    ...
    if (descriptor < 0)
    {
        XHAL_THROW_RUNTIME_ERROR("Piper temporary file creation failed");
    }
    ...
    if (synthesisProcess == 0)
    {
        ...
        ::execlp(executableName.c_str(), executableName.c_str(),
            "-m", modelName.c_str(), "-f", temporaryPath, "--",
    ...
    if (playbackProcess == 0)
    {
        ::execlp(playbackExecutableName.c_str(),
            playbackExecutableName.c_str(), temporaryPath,
