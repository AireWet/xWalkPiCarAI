...
XWalkTextToSpeechAlsa::XWalkTextToSpeechAlsa(XWalkAudioAlsa& sharedAudioBackend,
    contextpointer context, const XWalkTextToSpeechAlsaOperations& backendOperations,
    ...
    playbackVolumePercentValue(playbackVolumePercent)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Text-to-speech ALSA adapter requires synthesis");
    }
    if (playbackVolumePercentValue > 100U)
    {
        XHAL_THROW_OUT_OF_RANGE("Text-to-speech ALSA volume exceeds one hundred percent");
    }
}
...
void XWalkTextToSpeechAlsa::validateAudioData(const XWalkTextToSpeechPcmData& audioData)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Text-to-speech PCM metadata is invalid");
    }
...
XWalkTextToSpeechAlsa& XWalkTextToSpeechAlsa::adapter(contextpointer context)
{
    if (context == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Text-to-speech ALSA context must not be null");
    }
