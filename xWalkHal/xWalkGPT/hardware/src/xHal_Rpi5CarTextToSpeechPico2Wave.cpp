 ...
 */
XWalkTextToSpeechPico2Wave::XWalkTextToSpeechPico2Wave(
    stringview executable, stringview playbackExecutable, stringview language):
    ...
    languageName(language)
{
    ...
    if (ttsConfigurationInvalid)
    {
        XHAL_THROW_INVALID_ARGUMENT(
            "Pico2Wave executable, playback executable, and language are required");
    }
}
 ...
 * @param[in,out] context Non-null pointer to a provider that outlives this call.
 * @return Referenced provider.
 * @throws std::invalid_argument If `context` is null.
 */
XWalkTextToSpeechPico2Wave& XWalkTextToSpeechPico2Wave::provider(
    contextpointer context)
{
    if (context == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Pico2Wave provider context must not be null");
    }
...
void XWalkTextToSpeechPico2Wave::speak(contextpointer context, stringview text)
{
    ...
    if (textTooLarge)
    {
        XHAL_THROW_OUT_OF_RANGE("Pico2Wave text exceeds the bounded input length");
    }
 ...
 * @throws std::runtime_error If file, process, synthesis, playback, or cleanup fails.
 */
void XWalkTextToSpeechPico2Wave::execute(stringview text) const
{
    ...
    if (descriptor < 0)
    {
        XHAL_THROW_RUNTIME_ERROR("Pico2Wave temporary file creation failed");
    }
    ...
    {
        static_cast<void>(::unlink(temporaryPath));
        XHAL_THROW_RUNTIME_ERROR("Pico2Wave process creation failed");
    }
    if (synthesisProcess == 0)
    {
        ...
        ::execlp(executableName.c_str(), executableName.c_str(),
            "-l", languageName.c_str(), "-w", temporaryPath,
            ownedText.c_str(), static_cast<charpointer>(nullptr));
        ::_exit(127);
    }
    ...
    {
        static_cast<void>(::unlink(temporaryPath));
        XHAL_THROW_RUNTIME_ERROR("Pico2Wave playback process creation failed");
    }
    if (playbackProcess == 0)
    {
