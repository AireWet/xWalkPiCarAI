 ...
 */
XWalkTextToSpeechEspeak::XWalkTextToSpeechEspeak(
...
XWalkTextToSpeechEspeak& XWalkTextToSpeechEspeak::provider(contextpointer context)
{
    if (context == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Espeak provider context must not be null");
    }
...
bytevector XWalkTextToSpeechEspeak::execute(stringview text) const
{
    ...
    while (processingLoopRequested)
    {
        ...
        else
        {
            ...
            XHAL_THROW_RUNTIME_ERROR("Espeak input staging failed");
        }
    }
    static_cast<void>(::close(inputPipe[1U]));
    ...
    if (childProcess < 0)
    {
        ...
        static_cast<void>(::close(outputPipe[0U]));
        static_cast<void>(::close(outputPipe[1U]));
        XHAL_THROW_RUNTIME_ERROR("Espeak process creation failed");
    }
    if (childProcess == 0)
    {
        static_cast<void>(::close(outputPipe[0U]));
        ...
        ::execlp(executableName.c_str(), executableName.c_str(), "--stdout",
            "-v", voiceName.c_str(), static_cast<charpointer>(nullptr));
        ::_exit(127);
    }
    ...
    while (true)
    {
        ...
        if (result > 0)
        {
            ...
            if (waveDataTooLarge)
            {
                ...
                XHAL_THROW_OUT_OF_RANGE("Espeak WAV output exceeds its bounded size");
            }
        }
        else if (result == 0)
        {
        ...
        else if (errno != EINTR)
        {
            ...
            static_cast<void>(::kill(childProcess, SIGTERM));
            static_cast<void>(::waitpid(childProcess, nullptr, 0));
            XHAL_THROW_RUNTIME_ERROR("Espeak output read failed");
        }
    }
    static_cast<void>(::close(outputPipe[0U]));
    int processStatus{};
 ...
 * @throws std::runtime_error If required PCM WAV chunks are malformed or absent.
 */
    ...
    const bytevector& waveData)
{
    ...
    if ((audioFormat != 1U) || (bitsPerSample != 16U) ||
        (channelCount == 0U) || (channelCount > 8U) ||
        (sampleRateHz == 0U) || (dataByteCount == 0U))
    {
        XHAL_THROW_RUNTIME_ERROR("Espeak WAV does not contain supported PCM data");
    }
