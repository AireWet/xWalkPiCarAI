    ...
    stringview libraryName, stringview modelPath)
{
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("Vosk shared library could not be loaded");
    }
        ...
        (api.finalResult == nullptr) || (api.recognizerFree == nullptr))
    {
        ...
        XHAL_THROW_RUNTIME_ERROR("Required Vosk API symbol is unavailable");
    }
    ...
    if (modelHandle == nullptr)
    {
        static_cast<void>(::dlclose(libraryHandle));
        libraryHandle = nullptr;
        XHAL_THROW_RUNTIME_ERROR("Vosk model could not be loaded");
    }
}
...
XWalkSpeechRecognizerVosk& XWalkSpeechRecognizerVosk::provider(contextpointer context)
{
    if (context == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Vosk provider context must not be null");
    }
...
FunctionType XWalkSpeechRecognizerVosk::loadFunction(
    contextpointer library, cstring symbolName)
{
...
/** @brief Reports model readiness. @param[in,out] context Provider context. @return Readiness state. */
...
string XWalkSpeechRecognizerVosk::recognizePcm(contextpointer context,
    const bytevector& pcmData, uint32 sampleRateHz, uint8 channelCount)
{
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("Vosk recognizer could not be created");
    }
 ...
 */
string XWalkSpeechRecognizerVosk::recognizeFile(
    contextpointer context, stringview filePath)
{
    static_cast<void>(provider(context));
    static_cast<void>(filePath);
    XHAL_THROW_RUNTIME_ERROR("Vosk provider does not own an audio-file decoder");
}

/** @brief Requests recognition cancellation. @param[in,out] context Provider context. */
