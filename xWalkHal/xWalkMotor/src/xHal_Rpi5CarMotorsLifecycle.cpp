...
uint8 XWalkMotors::validateMotorId(uint8 motorId)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("motor identifier must be 1 or 2");
    }
    return motorId;
}
...
void XWalkMotors::validateSpeeds(float64 leftSpeedPercent, float64 rightSpeedPercent)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("paired motor speeds must be between -100 and 100 percent");
    }
}
 ...
 *
 * @post
 * Both role identifiers are valid and their configured reversal states are applied.
 */
