 ...
 * @return Zero after one successful final response, including an empty response.
 * @warning Run only after approving the endpoint, model, prompt, and network policy.
 */
    ...
    XWalkHal::charpointer argumentValues[])
{
    if (argumentCount != 4)
    {
        XHAL_THROW_INVALID_ARGUMENT("Ollama hardware test requires endpoint, model, and prompt");
    }
