    ...
    uint32 timeoutMs, size maximumResponseBytes)
{
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("Ollama libcurl initialization failed");
    }
    ...
    if (request == nullptr)
    {
        XHAL_THROW_RUNTIME_ERROR("Ollama libcurl request allocation failed");
    }
    ...
    if (!authorizationHeaderCopy.empty())
    {
        ...
        if (authenticatedHeaders == nullptr)
        {
            ...
            ::curl_easy_cleanup(request);
            XHAL_THROW_RUNTIME_ERROR("Language-model authorization header allocation failed");
        }
    ...
    if (optionResult != CURLE_OK)
    {
        ::curl_slist_free_all(headers);
        ::curl_easy_cleanup(request);
        XHAL_THROW_RUNTIME_ERROR("Ollama libcurl option configuration failed");
    }
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("Ollama HTTP request failed");
    }
    if ((statusResult != CURLE_OK) || (statusCode < 200L) || (statusCode >= 300L))
    {
        XHAL_THROW_RUNTIME_ERROR("Ollama HTTP status is unsuccessful");
    }
