...
{
    return {&systemOpenPcm, &systemConfigurePcm, &systemWritePcm,
        &systemRecoverPcm, &systemClosePcm, &systemOpenMixer,
        &systemSetMixerVolume, &systemCloseMixer};
}
...
boolean XWalkAudioAlsa::systemConfigurePcm(contextpointer context,
    audiopcmhandle pcmHandle, const XWalkAudioStreamConfiguration& configuration)
{
    static_cast<void>(context);
    ...
    const boolean parametersAccepted =
        ...
        (::snd_pcm_hw_params_set_access(nativeHandle, hardwareParameters,
            SND_PCM_ACCESS_RW_INTERLEAVED) >= 0) &&
        (::snd_pcm_hw_params_set_format(nativeHandle, hardwareParameters, nativeFormat) >= 0) &&
        (::snd_pcm_hw_params_set_channels(nativeHandle, hardwareParameters,
            configuration.channelCount) >= 0) &&
        (::snd_pcm_hw_params_set_rate_near(nativeHandle, hardwareParameters,
            &negotiatedRateHz, &direction) >= 0) &&
        (::snd_pcm_hw_params_set_period_size_near(nativeHandle, hardwareParameters,
            &negotiatedPeriodFrames, &direction) >= 0) &&
        (::snd_pcm_hw_params_set_buffer_time_near(nativeHandle, hardwareParameters,
            &negotiatedLatencyUs, &direction) >= 0) &&
        (::snd_pcm_hw_params(nativeHandle, hardwareParameters) >= 0) &&
        (::snd_pcm_prepare(nativeHandle) >= 0);
 ...
 */
int32 XWalkAudioAlsa::systemWritePcm(contextpointer context, audiopcmhandle pcmHandle,
    const bytevector& pcmData, size byteOffset, size frameCount)
{
    static_cast<void>(context);
    snd_pcm_t* const nativeHandle = static_cast<snd_pcm_t*>(pcmHandle);
    const void* const framePointer = pcmData.data() + byteOffset;
    const snd_pcm_sframes_t writeResult = ::snd_pcm_writei(nativeHandle,
        framePointer, static_cast<snd_pcm_uframes_t>(frameCount));
 ...
 */
boolean XWalkAudioAlsa::systemRecoverPcm(contextpointer context,
 ...
 */
audiomixerhandle XWalkAudioAlsa::systemOpenMixer(contextpointer context,
    stringview deviceName)
{
    ...
    const boolean attached = opened &&
        (::snd_mixer_attach(mixerHandle, ownedDeviceName.c_str()) >= 0);
    const boolean registered = attached &&
        (::snd_mixer_selem_register(mixerHandle, nullptr, nullptr) >= 0);
 ...
 * Non-null handle that becomes invalid after this call.
 */
void XWalkAudioAlsa::systemCloseMixer(contextpointer context,
