...
void XWalkAudioAlsa::validateConstruction(const XWalkAudioAlsaOperations& backendOperations,
    stringview pcmDeviceName, stringview mixerDeviceName, stringview mixerElementName)
{
    ...
    const XWalkAudioStreamConfiguration& configuration)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("ALSA audio stream values must be greater than zero");
    }
    if (configuration.channelCount > XHAL_RPI5CAR_AUDIO_MAXIMUM_CHANNEL_COUNT)
    {
        XHAL_THROW_OUT_OF_RANGE("ALSA audio channel count exceeds its supported maximum");
    }
    if (configuration.periodFrames > XHAL_RPI5CAR_AUDIO_MAXIMUM_PERIOD_FRAMES)
    {
        XHAL_THROW_OUT_OF_RANGE("ALSA audio period exceeds its supported maximum");
    }
...
size XWalkAudioAlsa::bytesPerSample(XWalkAudioSampleFormat format)
{
    switch (format)
    {
        ...
        case XWalkAudioSampleFormat::Float32LittleEndian:
            return 4U;
        default:
            XHAL_THROW_OUT_OF_RANGE("ALSA audio sample format is not supported");
    }
}
...
size XWalkAudioAlsa::availableStreamIndex() const
{
    ...
    XHAL_THROW_OUT_OF_RANGE("ALSA audio stream limit has been reached");
}
...
size XWalkAudioAlsa::streamIndex(audiopcmhandle streamHandle) const
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("ALSA audio stream handle must not be null");
    }
    for (size index = 0U; index < pcmHandles.size(); ++index)
    {
    ...
    XHAL_THROW_INVALID_ARGUMENT("ALSA audio stream is not owned by this backend");
}
      ...
      mixerElementNameValue(mixerElementName)
{
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("ALSA audio mixer could not be opened");
    }
}
