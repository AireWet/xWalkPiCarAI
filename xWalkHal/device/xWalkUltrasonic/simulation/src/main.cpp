...
XWalkHal::int32 main(XWalkHal::int32 argumentCount,
  ...
  const xwalk::hal::sim::XWalkUltrasonicSimulationArguments arguments(
      argumentCount, argumentValues);
  if (arguments.valid() == false) {
    XWALK_HAL_ERROR(XWALK_EXCEPTION,
                    "Invalid xWalkUltrasonic simulation arguments");
  ...
  XWALK_HAL_TRACE_UID1(
      RPI .209, "xWalkUltrasonic simulation completed with status %d", result);
  return result;
}
