...
float64 XWalkUltrasonic::readOnce() {
  ...
  const uint64 timeoutLimitMicroseconds =
      static_cast<uint64>(timeoutMicrosecondsValue);
  ...
  while (processingLoopRequested) {
    ...
    const uint64 elapsedMicroseconds =
        pulseStartMicroseconds - timeoutStartMicroseconds;
  ...
  while (echoCompletionWaitRequested) {
    const hal::boolean readSucceeded =
        static_cast<hal::boolean>(echoPin->read());
    if (readSucceeded == false) {
      break;
    }
    pulseEndMicroseconds = common::monotonicMicroseconds();
    const uint64 elapsedMicroseconds =
        pulseEndMicroseconds - timeoutStartMicroseconds;
    if (elapsedMicroseconds > timeoutLimitMicroseconds) {
      return XHAL_RPI5CAR_ULTRASONIC_TIMEOUT_RESULT_CM;
    }
  }
  ...
  const uint64 durationMicroseconds =
      pulseEndMicroseconds - pulseStartMicroseconds;
  ...
  const float64 soundSpeedMps =
      static_cast<float64>(XHAL_RPI5CAR_ULTRASONIC_SOUND_SPEED_MPS);
  ...
  const float64 distanceCentimeters =
      oneWayDistanceMeters * centimetersPerMeter;
  ...
  XWALK_HAL_TRACE_UID1(RPI .203,
                       "Ultrasonic pulse measured at %.2f centimeters", result);
  return result;
}
  ...
  for (uint32 attempt = 0U; attempt < attempts; ++attempt) {
    ...
    if (distanceCentimeters != XHAL_RPI5CAR_ULTRASONIC_TIMEOUT_RESULT_CM) {
      XWALK_HAL_TRACE_UID2(RPI .204,
                           ...
                           attempt + 1U, distanceCentimeters);
