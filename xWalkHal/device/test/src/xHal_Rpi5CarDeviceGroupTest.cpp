...
TEST_P(ServoAngleGroupTest, MapsAngleThroughPwmToBigEndianRegisterData)
{
    ...
    XWalkI2c i2c(&bus, &xwalk::hal::test::XWalkPwmTestI2c::probeCallback,
    ...
    const bytevector expectedBytes{
        static_cast<uint8>((angleCase.pulseWidth >> 8U) & 0xFFU),
        static_cast<uint8>(angleCase.pulseWidth & 0xFFU)};
    EXPECT_EQ(bus.writeData(0U), expectedBytes);
}

INSTANTIATE_TEST_SUITE_P(BoundaryAngles, ServoAngleGroupTest,
    testing::Values(
        ServoAngleCase{-120.0, 102U}, ServoAngleCase{0.0, 307U}, ServoAngleCase{120.0, 511U}));
...
TEST(XWalkDeviceGroup, ServoPropagatesPwmI2cWriteFailure)
{
    ...
    backend.failingWrite = 0U;
    servo.setAngle(0.0);
    EXPECT_EQ(pwm.pulseWidth(), 307U)
        << "Servo must remain usable after the propagated bus failure is cleared";
}
...
TEST(XWalkDeviceGroup, AdcAndAccelerometerPreserveBusEncoding)
{
    ...
    XWalkI2c adcI2c(&adcBus, &xwalk::hal::test::adc::probe,
        &xwalk::hal::test::adc::writeRegister, &xwalk::hal::test::adc::read);
    ...
    XWalkI2c accelerometerI2c(&accelerometerBus, &xwalk::hal::test::adxl345::probe,
        &xwalk::hal::test::adxl345::writeRegister, &xwalk::hal::test::adxl345::read,
    ...
    EXPECT_THROW(
        static_cast<void>(accelerometer.read(XWalkAdxl345Axis::X)), std::runtime_error);
}
...
TEST(XWalkDeviceGroup, UserButtonPreservesActiveLowGpioAndThresholdBoundaries)
{
    ...
    EXPECT_THROW(
        button.setOnLongPress(nullptr, nullptr, XHAL_POSITIVE_INFINITY(float64)),
