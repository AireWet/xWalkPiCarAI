...
uint16 XWalkAdc::read() {
  ...
  const bytevector sampleBytes = i2cObject->writeRegisterThenRead(
      addressValue, commandValue, commandPayload, XHAL_RPI5CAR_ADC_READ_LENGTH);
  const hal::boolean sampleBytesDifferent = static_cast<hal::boolean>(
  ...
  const uint16 shiftedMostSignificantByte =
      static_cast<uint16>(mostSignificantByte << 8U);
  const uint16 sample =
      static_cast<uint16>(shiftedMostSignificantByte | leastSignificantByte);
  XWALK_HAL_TRACE_UID2(RPI .175, "ADC channel %u read sample %u", channelValue,
                       sample);
  return sample;
}
...
float64 XWalkAdc::readVoltage() {
  ...
  const float64 referenceVoltage =
      static_cast<float64>(XHAL_RPI5CAR_ADC_REFERENCE_VOLTAGE);
  const float64 maximumAdcCount =
      static_cast<float64>(XHAL_RPI5CAR_ADC_MAX_COUNT);
  const float64 scaledVoltage = adcCountValue * referenceVoltage;
  const float64 voltage = scaledVoltage / maximumAdcCount;
  XWALK_HAL_TRACE_UID2(RPI .176,
                       "ADC channel %u converted sample to %.6f volts",
                       channelValue, voltage);
  return voltage;
}
 ...
 */
uint8 XWalkAdc::address() const noexcept { return addressValue; }
 ...
 */
uint8 XWalkAdc::channel() const noexcept { return channelValue; }
 ...
 */
uint8 XWalkAdc::command() const noexcept { return commandValue; }
