...
void XWalkAdxl345::configureMeasurement() {
  ...
  i2cObject->writeRegister(
      addressValue, XHAL_RPI5CAR_ADXL345_DATA_FORMAT_REGISTER, formatData);
  const bytevector powerData{XHAL_RPI5CAR_ADXL345_MEASUREMENT_MODE_VALUE};
  i2cObject->writeRegister(
      addressValue, XHAL_RPI5CAR_ADXL345_POWER_CONTROL_REGISTER, powerData);
...
float64 XWalkAdxl345::convertSample(const bytevector &sampleBytes) {
  ...
  const uint16 packedSample =
      static_cast<uint16>(lowByteValue | shiftedHighByte);
  ...
  const float64 countsPerGravity =
      static_cast<float64>(XHAL_RPI5CAR_ADXL345_COUNTS_PER_G);
...
float64 XWalkAdxl345::readAxis(uint8 axis) {
  ...
  const bytevector discardedSample = i2cObject->readRegister(
      addressValue, dataRegister, XHAL_RPI5CAR_ADXL345_SAMPLE_LENGTH);
  static_cast<void>(convertSample(discardedSample));
  const bytevector sampleBytes = i2cObject->readRegister(
      addressValue, dataRegister, XHAL_RPI5CAR_ADXL345_SAMPLE_LENGTH);
  const float64 acceleration = convertSample(sampleBytes);
  XWALK_HAL_TRACE_UID2(RPI .194, "ADXL345 axis %u read %.6f g", axis,
                       acceleration);
  return acceleration;
}
 ...
 */
uint8 XWalkAdxl345::address() const noexcept { return addressValue; }
