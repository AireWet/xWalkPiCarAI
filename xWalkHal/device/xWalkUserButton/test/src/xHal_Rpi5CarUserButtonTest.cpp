...
void testShortPress() {
  ...
  XWalkHal::XWalkGpio gpio(&backend, callbacks, "USER",
...
void testFailures() {
  ...
  xwalk::hal::test::expectFailure([&]() {
    ...
    XWalkHal::XWalkGpio failingGpio(&failingBackend, callbacks, "USER",
                                    XWalkHal::XWalkGpioMode::Input,
                                    XWalkHal::XWalkGpioPull::Up);
...
XWalkHal::int32 main() {
  xwalk::hal::XWalkTrace::configureGlobal(
      XWALK_USER_BUTTON_SIMULATION_TRACE_CONFIG_PATH,
