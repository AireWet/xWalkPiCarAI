...
XWalkHal::int32 main(XWalkHal::int32 argumentCount,
  ...
  const xwalk::hal::sim::XWalkUserButtonSimulationArguments arguments(
      argumentCount, argumentValues);
  if (arguments.valid() == false) {
    XWALK_HAL_ERROR(XWALK_EXCEPTION,
                    "Invalid xWalkUserButton simulation arguments");
  ...
  XWALK_HAL_TRACE_UID1(
      RPI .229, "xWalkUserButton simulation completed with status %d", result);
  return result;
}
