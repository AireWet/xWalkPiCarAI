...
boolean validCameraSourceString(stringview source) noexcept
{
  ...
  if (source.empty() || (source.size() > MAXIMUM_SOURCE_BYTES) ||
      (source.find('\0') != stringview::npos) ||
      (source.find('\r') != stringview::npos) ||
      (source.find('\n') != stringview::npos)) {
    return false;
  }
  ...
  if ((source.size() <= DEVICE_PREFIX.size()) ||
      (source.substr(0U, DEVICE_PREFIX.size()) != DEVICE_PREFIX)) {
    return false;
  }
...
XWalkCamera::XWalkCamera(contextpointer context,
    ...
    : backendContext(context), captureCallback(captureOperation),
      configurationValue(configuration) {
  validate(captureCallback, configurationValue);
  XWALK_HAL_TRACE_UID3(
      RPI .212, "Camera constructed for %u by %u pixels with timeout %u ms",
      configurationValue.widthPixels, configurationValue.heightPixels,
...
string XWalkCamera::capture(stringview outputPath) {
  ...
  XWALK_HAL_TRACE_UID2(RPI .213, "Camera capture completed at %u by %u pixels",
...
void XWalkCamera::validate(cameracapturecallback capture,
  ...
  if ((configuration.widthPixels < 16U) ||
      (configuration.widthPixels > 7'680U) ||
      (configuration.heightPixels < 16U) ||
      (configuration.heightPixels > 4'320U) ||
      (configuration.timeoutMs == 0U) || (configuration.timeoutMs > 300'000U)) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Camera capture settings are outside supported bounds");
  }
}
