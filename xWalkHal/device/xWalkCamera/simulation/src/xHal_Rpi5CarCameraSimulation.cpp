...
int32 runCameraSimulation() {
  ...
  const boolean valid = (outputPath == "simulation.jpg") &&
                        (backend.captureCount() == 1U) &&
                        (backend.outputPath() == "simulation.jpg") &&
                        (backend.configuration().widthPixels == 1'280U) &&
                        (backend.configuration().heightPixels == 720U) &&
                        (backend.configuration().timeoutMs == 2'000U);
