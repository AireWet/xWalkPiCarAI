...
```bash
cmake -S xWalkHal/device/xWalkCamera -B xWalkHal/device/xWalkCamera/build-host -DXWALK_CAMERA_BUILD_HOST_TESTS=ON
cmake --build xWalkHal/device/xWalkCamera/build-host --parallel
ctest --test-dir xWalkHal/device/xWalkCamera/build-host --output-on-failure
```
...
```bash
cmake -S xWalkHal/device/xWalkCamera/simulation -B build-camera-simulation -DCMAKE_BUILD_TYPE=Debug
...
```bash
cmake -S xWalkHal/device/xWalkCamera -B xWalkHal/device/xWalkCamera/build-rpi -DXWALK_CAMERA_BUILD_HARDWARE_TESTS=ON
cmake --build xWalkHal/device/xWalkCamera/build-rpi --parallel
ctest --test-dir xWalkHal/device/xWalkCamera/build-rpi -N -L hardware
```
