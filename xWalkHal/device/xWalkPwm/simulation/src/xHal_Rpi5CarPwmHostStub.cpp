...
void XWalkPwmHostStub::writeRegister(contextpointer context, uint8 address,
...
bytevector XWalkPwmHostStub::read(contextpointer context, uint8 address,
                                  size length) {
  static_cast<void>(context);
  static_cast<void>(address);
...
size XWalkPwmHostStub::writeCount() const noexcept { return writeCountValue; }
