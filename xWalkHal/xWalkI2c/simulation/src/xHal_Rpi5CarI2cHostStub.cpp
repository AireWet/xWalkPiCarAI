...
boolean XWalkI2cHostStub::selectAddress(int32 fileDescriptor, uint8 address)
{
    ...
    XWALK_HAL_TRACE_UID3(RPI.037, "Host I2C selected address %u with status %u",
        static_cast<uint32>(address), static_cast<uint32>(descriptorValid));
...
boolean XWalkI2cHostStub::transfer(int32 fileDescriptor, contextpointer request)
{
    ...
    if ((smbusRequest->read_write == I2C_SMBUS_READ) &&
        (smbusRequest->size == I2C_SMBUS_BYTE) && (smbusRequest->data != nullptr))
    {
        ...
        (smbusRequest->data != nullptr))
    {
        ...
        XWALK_HAL_TRACE_UID3(RPI.040,
            "Host I2C mirrored address %u, register %u, and %zu register-read bytes",
            static_cast<uint32>(lastAddressValue),
            static_cast<uint32>(lastRegisterValue), requestedLength);
        return true;
    }
    ...
    XWALK_HAL_TRACE_UID3(RPI.038,
        ...
        static_cast<uint32>(lastAddressValue), static_cast<uint32>(lastRegisterValue),
        lastDataValue.size());
    return true;
}
