 ...
 * arguments or trace selection, and another non-zero status for operation failure.
 */
XWalkHal::int32 main(XWalkHal::int32 argumentCount, XWalkHal::charpointer argumentValues[])
{
    const xwalk::hal::sim::XWalkI2cSimulationArguments arguments(
    ...
    if (helpRequested)
    {
        std::fprintf(stdout,
            ...
            "Options:\n"
            "  -h, --help                 Show this help without opening I2C hardware.\n"
            ...
            "Trace IDs use TAG.<digits> and must be unique across the compiled project.\n"
            "A duplicate trace ID fails metadata generation and compilation.\n"
            "All normal traces are disabled by default.\n",
            argumentValues[0]);
        return 0;
    }
    ...
    XWALK_HAL_TRACE_UID1(RPI.043, "xWalkI2c trace boot completed");
    XWALK_HAL_TRACE_UID1(RPI.031, "xWalkI2c simulation started");

    xwalk::hal::owningpointer<xwalk::hal::XWalkI2cDevice> device =
        xwalk::hal::sim::createI2cDevice();
    xwalk::hal::XWalkI2cLinux linuxBackend(
        *device, XWALK_I2C_SIMULATION_DEVICE_PATH, 1U);

    const XWalkHal::int32 simulationResult = runSimulation(linuxBackend);
    XWALK_HAL_TRACE_UID1(RPI.032, "xWalkI2c simulation completed with status %d",
        simulationResult);
