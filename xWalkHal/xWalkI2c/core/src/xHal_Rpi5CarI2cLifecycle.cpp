 ...
 *
 * @pre
 * The context satisfies the requirements of every configured callback and remains valid
 * for every operation performed through this object.
 *
 * @post
 * Required callback members are non-null when construction succeeds. The
 * register-read and fail-safe-write callbacks remain nullable for compatible consumers.
 *
 * @throws std::invalid_argument
 * If a required probe, write-register, or sequential-read callback is null.
 */
XWalkI2c::XWalkI2c(contextpointer context, i2cprobecallback probeOperation,
