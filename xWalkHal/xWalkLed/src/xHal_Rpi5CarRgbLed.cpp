...
rgbcolor XWalkRgbLed::decodeColor(uint32 packedColor)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("RGB LED packed color must be in range 0x000000..0xFFFFFF");
    }

    const uint32 redValue =
        (packedColor & XHAL_RPI5CAR_RGB_LED_RED_MASK) >> XHAL_RPI5CAR_RGB_LED_RED_SHIFT;
    const uint32 greenValue =
        (packedColor & XHAL_RPI5CAR_RGB_LED_GREEN_MASK) >> XHAL_RPI5CAR_RGB_LED_GREEN_SHIFT;
    const uint32 blueValue =
        (packedColor & XHAL_RPI5CAR_RGB_LED_BLUE_MASK) >> XHAL_RPI5CAR_RGB_LED_BLUE_SHIFT;
    return {
        static_cast<uint8>(redValue),
        static_cast<uint8>(greenValue),
...
uint32 XWalkRgbLed::parseHexColor(stringview hexadecimalColor)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("RGB LED color text must contain six hexadecimal digits");
    }
...
uint8 XWalkRgbLed::hexDigitValue(char digit)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("RGB LED color text contains a non-hexadecimal character");
    }
    return value;
}
