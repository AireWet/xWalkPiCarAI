...
XWalkRobotHatSimulation &XWalkRobotHatSimulation::from(contextpointer context) {
  if (context == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Robot HAT simulation context must not be null");
  }
...
boolean
XWalkRobotHatSimulation::consumeFailure(XWalkRobotHatOperation operation,
                                        uint32 target) noexcept {
  for (Failure &failure : failuresValue) {
    if ((failure.operation == operation) && (failure.target == target) &&
        (failure.remaining > 0U)) {
      --failure.remaining;
      return true;
    }
  }
...
void XWalkRobotHatSimulation::record(XWalkRobotHatOperation operation,
                                     uint32 target, uint32 value,
                                     boolean succeeded, const bytevector &data,
                                     stringview text) {
  ...
  eventsValue.push_back({nextSequenceValue, logicalTimeValue, operation, target,
                         value, succeeded, data, string(text)});
  ++nextSequenceValue;
  ++logicalTimeValue;
}
...
void XWalkRobotHatSimulation::setAdcValue(uint8 channel, uint16 value) {
  if (channel >= adcValuesValue.size()) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "simulated ADC channel must be between zero and seven");
  }
  if (value > 4095U) {
...
void XWalkRobotHatSimulation::setGrayscaleValues(
    const std::array<uint16, 3U> &values) {
...
void XWalkRobotHatSimulation::setBatteryVoltage(float64 voltage) {
  ...
  if ((voltage < 0.0) || (voltage > maximumBatteryVoltage)) {
    XWALK_HAL_ERROR(
        XWALK_RANGE,
        "simulated battery voltage must be between zero and 9.9 volts");
  }
  const uint16 sample = static_cast<uint16>(
      std::lround((voltage / maximumBatteryVoltage) * maximumAdcCount));
  setAdcValue(4U, sample);
}

void XWalkRobotHatSimulation::setUltrasonicDistance(
    float64 distanceCentimeters) {
  if (!std::isfinite(distanceCentimeters)) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "simulated ultrasonic distance must be finite");
  }
  if ((distanceCentimeters < 0.0) || (distanceCentimeters > 500.0)) {
    XWALK_HAL_ERROR(XWALK_RANGE, "simulated ultrasonic distance must be "
...
void XWalkRobotHatSimulation::setCameraFrames(
    const std::vector<bytevector> &frames) {
...
boolean XWalkRobotHatSimulation::nextCameraFrame(bytevector &frame) {
  ...
  const boolean failed =
      consumeFailure(XWalkRobotHatOperation::CameraCapture, 0U) ||
      !cameraAvailableValue;
  if (failed) {
    record(XWalkRobotHatOperation::CameraCapture, 0U, 0U, false);
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "injected or unavailable simulated camera frame");
  }
  if (nextCameraFrameValue >= cameraFramesValue.size()) {
    frame.clear();
    record(XWalkRobotHatOperation::CameraCapture, 0U, 0U, true, {},
           "end-of-sequence");
    return false;
  }
  frame = cameraFramesValue[nextCameraFrameValue];
  ++nextCameraFrameValue;
  record(XWalkRobotHatOperation::CameraCapture, 0U,
         static_cast<uint32>(frame.size()), true, frame);
  return true;
}

void XWalkRobotHatSimulation::setLogicalDelay(XWalkRobotHatOperation operation,
                                              uint32 target, uint32 ticks) {
  const std::lock_guard<std::mutex> lock(mutexValue);
  delaysValue[{operation, target}] = ticks;
}

void XWalkRobotHatSimulation::failNext(XWalkRobotHatOperation operation,
                                       uint32 target, uint32 count) {
  if (count == 0U) {
...
bytevector XWalkRobotHatSimulation::registerValue(uint8 address,
                                                  uint8 reg) const {
  const std::lock_guard<std::mutex> lock(mutexValue);
...
boolean XWalkRobotHatSimulation::probe(contextpointer context, uint8 address) {
  ...
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::I2cProbe, address);
  const boolean present =
      !failed && (simulation.presentAddressesValue.count(address) != 0U);
  simulation.record(XWalkRobotHatOperation::I2cProbe, address, 0U, present);
  return present;
}

void XWalkRobotHatSimulation::writeRegister(contextpointer context,
                                            uint8 address, uint8 reg,
                                            const bytevector &data) {
  XWalkRobotHatSimulation &simulation = from(context);
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::I2cWrite, reg);
  simulation.record(XWalkRobotHatOperation::I2cWrite, reg, address, !failed,
                    data);
  if (failed) {
  ...
  if (simulation.presentAddressesValue.count(address) == 0U) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "simulated Robot HAT I2C device is unavailable");
  }
...
boolean
XWalkRobotHatSimulation::tryWriteRegister(contextpointer context, uint8 address,
                                          uint8 reg,
                                          const bytevector &data) noexcept {
  if (context == nullptr) {
    return false;
  }
  XWalkRobotHatSimulation &simulation =
      *static_cast<XWalkRobotHatSimulation *>(context);
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::I2cWrite, reg) ||
...
bytevector XWalkRobotHatSimulation::read(contextpointer context, uint8 address,
                                         size length) {
  XWalkRobotHatSimulation &simulation = from(context);
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::I2cRead, address);
  if (failed || (simulation.presentAddressesValue.count(address) == 0U)) {
    simulation.record(XWalkRobotHatOperation::I2cRead, address,
                      static_cast<uint32>(length), false);
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "injected or unavailable Robot HAT I2C read");
  }
  ...
  simulation.record(XWalkRobotHatOperation::I2cRead, address,
                    static_cast<uint32>(length), true, result);
  return result;
}

bytevector XWalkRobotHatSimulation::readRegister(contextpointer context,
                                                 uint8 address, uint8 reg,
                                                 size length) {
  XWalkRobotHatSimulation &simulation = from(context);
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::I2cReadRegister, reg);
  if (failed) {
    simulation.record(XWalkRobotHatOperation::I2cReadRegister, reg,
                      static_cast<uint32>(length), false);
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "injected Robot HAT I2C register read failure");
  }
  bytevector result(length, 0U);
  for (size index = 0U; index < length; ++index) {
    const uint8 current = static_cast<uint8>(reg + static_cast<uint8>(index));
    const auto value =
        simulation.registersValue.find(registerKey(address, current));
...
void XWalkRobotHatSimulation::configureGpio(contextpointer context, uint8 pin,
                                            XWalkGpioMode mode,
                                            XWalkGpioPull pull,
  ...
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::GpioConfigure, pin);
  simulation.record(XWalkRobotHatOperation::GpioConfigure, pin,
                    static_cast<uint32>(mode), !failed);
  if (failed) {
...
boolean XWalkRobotHatSimulation::readGpio(contextpointer context, uint8 pin) {
  ...
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::GpioRead, pin);
  simulation.record(XWalkRobotHatOperation::GpioRead, pin,
                    simulation.gpioValuesValue[pin], !failed);
  if (failed) {
...
void XWalkRobotHatSimulation::writeGpio(contextpointer context, uint8 pin,
                                        boolean value) {
  XWalkRobotHatSimulation &simulation = from(context);
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::GpioWrite, pin);
...
void XWalkRobotHatSimulation::interruptGpio(contextpointer context, uint8 pin,
                                            XWalkGpioEdge edge,
                                            uint32 debounceMs,
  ...
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::GpioInterrupt, pin);
  simulation.record(XWalkRobotHatOperation::GpioInterrupt, pin, debounceMs,
                    !failed);
  if (failed) {
...
void XWalkRobotHatSimulation::cancelInterruptGpio(contextpointer context,
                                                  uint8 pin) {
  XWalkRobotHatSimulation &simulation = from(context);
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed = simulation.consumeFailure(
      XWalkRobotHatOperation::GpioCancelInterrupt, pin);
  simulation.record(XWalkRobotHatOperation::GpioCancelInterrupt, pin, 0U,
                    !failed);
  if (failed) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "injected Robot HAT GPIO interrupt cancellation failure");
  }
}

boolean XWalkRobotHatSimulation::capture(
    contextpointer context, stringview outputPath,
    const XWalkCameraConfiguration &configuration) {
  XWalkRobotHatSimulation &simulation = from(context);
  const std::lock_guard<std::mutex> lock(simulation.mutexValue);
  const boolean failed =
      simulation.consumeFailure(XWalkRobotHatOperation::CameraCapture, 0U) ||
      !simulation.cameraAvailableValue;
  simulation.record(XWalkRobotHatOperation::CameraCapture, 0U,
                    configuration.widthPixels, !failed, {}, outputPath);
  return !failed;
}
