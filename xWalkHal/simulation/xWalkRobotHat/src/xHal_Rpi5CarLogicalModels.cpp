...
void recordEvent(XWalkLogicalModelState& state, XWalkLogicalModelEventId identifier,
    float64 firstValue = 0.0, float64 secondValue = 0.0) noexcept
{
    if (state.eventCount < state.events.size())
    {
        state.events[state.eventCount] = {state.nextEventSequence, state.logicalTime,
            identifier, firstValue, secondValue};
        ++state.eventCount;
    }
...
float64 moveToward(float64 current, float64 target, float64 increase,
    float64 decrease) noexcept
{
...
XWalkLogicalModelStatus validateLogicalModelConfiguration(
    const XWalkLogicalModelConfiguration& configuration) noexcept
{
    if (!positiveFinite(configuration.accelerationPerTick) ||
        !positiveFinite(configuration.decelerationPerTick) ||
        !std::isfinite(configuration.steeringMinimum) ||
        !std::isfinite(configuration.steeringMaximum) ||
        !std::isfinite(configuration.servoCentre) ||
        !positiveFinite(configuration.servoTravel) ||
        (configuration.steeringMinimum >= configuration.steeringMaximum) ||
        (configuration.servoCentre - configuration.servoTravel <
            configuration.steeringMinimum) ||
        (configuration.servoCentre + configuration.servoTravel >
            configuration.steeringMaximum) ||
        ...
        !std::isfinite(configuration.batteryReductionPerTick) ||
        (configuration.batteryReductionPerTick < 0.0) ||
        (configuration.grayscaleSequence.size() >
            XWALK_LOGICAL_MODEL_MAXIMUM_SEQUENCE_VALUES) ||
        (configuration.ultrasonicSequence.size() >
            XWALK_LOGICAL_MODEL_MAXIMUM_SEQUENCE_VALUES))
    {
    ...
    {
        if ((sample[0U] > 4'095U) || (sample[1U] > 4'095U) ||
            (sample[2U] > 4'095U))
        {
            return XWalkLogicalModelStatus::InvalidConfiguration;
        }
...
XWalkLogicalModelStatus commandLogicalMotors(XWalkLogicalModelState& state,
    float64 leftSpeed, float64 rightSpeed) noexcept
{
    ...
    state.commandedLeftSpeed = clampSpeed(state.configuration.invertLeftDirection ?
        -leftSpeed : leftSpeed);
    state.commandedRightSpeed = clampSpeed(state.configuration.invertRightDirection ?
        -rightSpeed : rightSpeed);
    recordEvent(state, XWalkLogicalModelEventId::MotorCommand,
        state.commandedLeftSpeed, state.commandedRightSpeed);
    return XWalkLogicalModelStatus::Ok;
}

XWalkLogicalModelStatus advanceLogicalModel(XWalkLogicalModelState& state,
    uint64 ticks) noexcept
{
    if (!state.initialized)
    {
        return XWalkLogicalModelStatus::NotInitialized;
    ...
    for (uint64 tick = 0U; tick < ticks; ++tick)
    {
        ...
        state.simulatedLeftSpeed = moveToward(state.simulatedLeftSpeed,
            state.commandedLeftSpeed, state.configuration.accelerationPerTick,
            state.configuration.decelerationPerTick);
        state.simulatedRightSpeed = moveToward(state.simulatedRightSpeed,
            state.commandedRightSpeed, state.configuration.accelerationPerTick,
            state.configuration.decelerationPerTick);
        state.batteryVoltage = std::max(0.0, state.batteryVoltage -
            state.configuration.batteryReductionPerTick);
        if (!state.batteryWarning &&
            (state.batteryVoltage <= state.configuration.batteryWarningVoltage))
        {
            state.batteryWarning = true;
            recordEvent(state, XWalkLogicalModelEventId::BatteryWarning,
                state.batteryVoltage);
        }
        if (!state.batteryCritical &&
            (state.batteryVoltage <= state.configuration.batteryCriticalVoltage))
        {
            state.batteryCritical = true;
            recordEvent(state, XWalkLogicalModelEventId::BatteryCritical,
                state.batteryVoltage);
            enterLogicalSafeState(state);
        }
    }
    recordEvent(state, XWalkLogicalModelEventId::MotorState,
        state.simulatedLeftSpeed, state.simulatedRightSpeed);
    return XWalkLogicalModelStatus::Ok;
}

XWalkLogicalModelStatus commandLogicalSteering(XWalkLogicalModelState& state,
    float64 angle) noexcept
{
    if (!state.initialized)
    {
        return XWalkLogicalModelStatus::NotInitialized;
    ...
    const float64 minimum = state.configuration.servoCentre -
        state.configuration.servoTravel;
    const float64 maximum = state.configuration.servoCentre +
        state.configuration.servoTravel;
    ...
    std::array<uint16, 3U>& sample) noexcept
{
    ...
    recordEvent(state, XWalkLogicalModelEventId::GrayscaleSample,
        sample[0U], sample[2U]);
    return XWalkLogicalModelStatus::Ok;
}

XWalkLogicalModelStatus nextLogicalUltrasonic(XWalkLogicalModelState& state,
    float64& distanceCentimeters) noexcept
{
    if (!state.initialized)
    {
        return XWalkLogicalModelStatus::NotInitialized;
    ...
    recordEvent(state, XWalkLogicalModelEventId::UltrasonicSample,
        distanceCentimeters);
    return XWalkLogicalModelStatus::Ok;
}

XWalkLogicalModelStatus nextLogicalCameraFrame(XWalkLogicalModelState& state,
    uint64& frameIdentifier) noexcept
{
    if (!state.initialized)
    {
        return XWalkLogicalModelStatus::NotInitialized;
    }
    if (std::numeric_limits<uint64>::max() - state.logicalTime <
        state.configuration.cameraDelayTicks)
    {
    ...
    if (!state.configuration.freezeCamera &&
        (state.cameraFrameIdentifier != std::numeric_limits<uint64>::max()))
    {
        ++state.cameraFrameIdentifier;
    }
    frameIdentifier = state.cameraFrameIdentifier;
    recordEvent(state, state.configuration.freezeCamera ?
        XWalkLogicalModelEventId::CameraFrozenFrame :
        XWalkLogicalModelEventId::CameraFrame, static_cast<float64>(frameIdentifier));
    return XWalkLogicalModelStatus::Ok;
}
...
boolean logicalI2cOperationFails(XWalkLogicalModelState& state) noexcept
{
    ...
    if (failed)
    {
        recordEvent(state, XWalkLogicalModelEventId::I2cFailure,
            state.i2cOperationCount);
        enterLogicalSafeState(state);
    }
    return failed;
