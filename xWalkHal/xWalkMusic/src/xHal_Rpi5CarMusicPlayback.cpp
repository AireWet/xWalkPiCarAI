 ...
 */
void XWalkMusic::musicPlay(stringview filename, int32 loops, float64 startSeconds,
...
float64 XWalkMusic::soundLength(stringview filename)
{
    ...
    return roundedHundredths(durationSeconds);
}
...
float64 XWalkMusic::normalizedVolume(float64 volumePercent)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("Music volume must be between zero and one hundred percent");
    }
