...
void XWalkMusic::setTimeSignature(uint32 top, uint32 bottom)
{
    if ((top == 0U) || (bottom == 0U))
    {
        XHAL_THROW_OUT_OF_RANGE("Music time-signature values must be non-zero");
    }
...
void XWalkMusic::setKeySignature(stringview keySignature)
{
    ...
    if ((marker != '#') && (marker != 'b'))
    {
        XHAL_THROW_INVALID_ARGUMENT("Music key signature must contain only sharp or flat markers");
    }
    for (const char character : keySignature)
    {
        if (character != marker)
        {
            XHAL_THROW_INVALID_ARGUMENT("Music key signature must not mix marker types");
        }
    }

    const int32 markerCount = static_cast<int32>(keySignature.size());
    const int32 displacementDirection = (marker == '#') ?
        XHAL_RPI5CAR_MUSIC_KEY_SIGNATURE_SHARP : XHAL_RPI5CAR_MUSIC_KEY_SIGNATURE_FLAT;
...
void XWalkMusic::setTempo(float64 beatsPerMinute, float64 noteValue)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Music tempo values must be greater than zero");
    }
...
float64 XWalkMusic::beatDurationSeconds(float64 beatValue) const
{
    ...
    if (beatValue < 0.0)
    {
        XHAL_THROW_OUT_OF_RANGE("Music beat value must not be negative");
    }
...
float64 XWalkMusic::noteFrequencyHz(int32 noteIndex, boolean natural) const
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("Music note index is outside the supported MIDI range");
    }
...
int32 XWalkMusic::parseNoteIndex(stringview noteName)
{
    ...
    switch (noteName[0U])
    {
        ...
        case 'B':
            ...
            break;
        default:
            XHAL_THROW_INVALID_ARGUMENT("Music note name requires an uppercase pitch letter");
    }
    ...
    if (noteName.size() == 3U)
    {
        if (noteName[1U] != '#')
        {
            XHAL_THROW_INVALID_ARGUMENT("Music note names support only sharp accidentals");
        }
        ++pitchClass;
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Music note octave must be in the range zero through eight");
    }
    ...
    if ((noteIndex < XHAL_RPI5CAR_MUSIC_FIRST_NAMED_NOTE_INDEX) ||
        (noteIndex > XHAL_RPI5CAR_MUSIC_MAXIMUM_NOTE_INDEX))
    {
        XHAL_THROW_INVALID_ARGUMENT("Music note is outside the named range A0 through C8");
    }
    return noteIndex;
}
...
void XWalkMusic::validateKeySignature(int32 keySignature)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("Music key signature must be between minus seven and seven");
    }
}
