 ...
 */
XWalkMusicAlsaAudioData XWalkMusicSndFileDecoder::decodeAudio(
    contextpointer context, stringview filename)
{
    ...
    std::array<short, decoderReadFrames * XHAL_RPI5CAR_AUDIO_MAXIMUM_CHANNEL_COUNT>
        sampleBuffer{};
    ...
    while (true)
    {
        const sf_count_t frameCount = ::sf_readf_short(file.get(), sampleBuffer.data(),
            static_cast<sf_count_t>(decoderReadFrames));
        if (frameCount < 0)
        {
            XHAL_THROW_RUNTIME_ERROR("Music audio file decoding failed");
        }
        if (frameCount == 0)
        {
            break;
