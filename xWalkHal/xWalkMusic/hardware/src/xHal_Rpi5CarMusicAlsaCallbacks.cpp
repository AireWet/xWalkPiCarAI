...
uint8 XWalkMusicAlsa::volumePercent(float64 normalizedVolume)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("Music ALSA normalized volume must be between zero and one");
    }
    const float64 scaledVolume =
        normalizedVolume * XHAL_RPI5CAR_MUSIC_VOLUME_PERCENT_DIVISOR;
    const uint32 roundedVolume = common::roundedValue(scaledVolume,
        "Music ALSA volume", 0U, 100U);
...
void XWalkMusicAlsa::playSoundBackground(contextpointer context,
    stringview filename, optionalfloat64 normalizedVolume)
{
    XWalkMusicAlsa& self = adapter(context);
    ...
    int32 loops, float64 startSeconds)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("Music ALSA playback offset exceeds the decoded audio");
    }
    {
        const mutexlock lock(self.stateMutex);
