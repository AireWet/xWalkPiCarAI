...
void XWalkDevice::refresh() {
  ...
  if (hatPathEmpty) {
    resetInformation();
    XWALK_HAL_TRACE_UID0(RPI .327,
                         "Robot HAT discovery selected legacy board defaults");
    return;
  }
  ...
  detectedInformation.productVersion = parseHexProperty(
      productVersionText, XHAL_RPI5CAR_DEVICE_PRODUCT_VERSION_PROPERTY);
  ...
  detectedInformation.speakerEnablePin =
  ...
  XWALK_HAL_TRACE_UID2(
      RPI .326, "Robot HAT v5 detected with product ID %u and version %u",
      informationValue.productId, informationValue.productVersion);
}
...
filesystempath XWalkDevice::findSupportedHatPath() const {
  ...
  for (const string &entryName : entryNames) {
    const hal::boolean entryNameMatched = static_cast<hal::boolean>(
    ...
    const filesystempath uuidPath =
        candidatePath / XHAL_RPI5CAR_DEVICE_UUID_PROPERTY;
    const hal::boolean uuidFileUnavailable = static_cast<hal::boolean>(
...
uint32 XWalkDevice::parseHexProperty(stringview text, cstring propertyName) {
  ...
  const hal::boolean textXInvalid =
      static_cast<hal::boolean>((text.size() >= 2U) && (text[0U] == '0') &&
                                ((text[1U] == 'x') || (text[1U] == 'X')));
  if (textXInvalid) {
    firstDigit = 2U;
  }
  ...
  if ((digitCount == 0U) ||
      (digitCount >
       static_cast<size>(XHAL_RPI5CAR_DEVICE_MAXIMUM_HEX_DIGITS))) {
    const std::string exceptionMessage =
        std::string(propertyName).append(" is not a 32-bit hexadecimal value");
