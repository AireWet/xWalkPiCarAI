...
void XWalkBoardControl::setPin(XWalkGpio &gpio, boolean value) {
  ...
  XWALK_HAL_TRACE_UID2(RPI .322,
                       "Board-control GPIO %u set to logical value %u",
                       gpio.pin(), static_cast<uint32>(value));
}
...
float64 XWalkBoardControl::batteryVoltage() {
  ...
  const float64 dividerRatio =
      static_cast<float64>(XHAL_RPI5CAR_BOARD_CONTROL_BATTERY_DIVIDER_RATIO);
  const float64 batteryVoltageValue = measuredVoltage * dividerRatio;
  XWALK_HAL_TRACE_UID1(RPI .324, "Robot HAT battery voltage is %.3f volts",
                       batteryVoltageValue);
