...
void writeSimulationProperty(const filesystempath &path, stringview value,
                             boolean appendNull) {
...
void createSimulationDeviceTree(const filesystempath &root,
                                const filesystempath &node) {
  static_cast<void>(createDirectories(node));
  writeSimulationProperty(node / XHAL_RPI5CAR_DEVICE_PRODUCT_PROPERTY,
                          "Robot HAT 5", false);
  writeSimulationProperty(node / XHAL_RPI5CAR_DEVICE_PRODUCT_ID_PROPERTY,
                          "00001902", true);
  writeSimulationProperty(node / XHAL_RPI5CAR_DEVICE_PRODUCT_VERSION_PROPERTY,
                          "0x00000050", true);
  writeSimulationProperty(node / XHAL_RPI5CAR_DEVICE_UUID_PROPERTY,
                          XHAL_RPI5CAR_DEVICE_ROBOT_HAT_V5_UUID, true);
  writeSimulationProperty(node / XHAL_RPI5CAR_DEVICE_VENDOR_PROPERTY,
                          "SunFounder", false);
  static_cast<void>(root);
}
void removeSimulationDeviceTree(const filesystempath &root,
                                const filesystempath &node) {
  static_cast<void>(
      removeFilesystemEntry(node / XHAL_RPI5CAR_DEVICE_PRODUCT_PROPERTY));
  static_cast<void>(
      removeFilesystemEntry(node / XHAL_RPI5CAR_DEVICE_PRODUCT_ID_PROPERTY));
  static_cast<void>(removeFilesystemEntry(
      node / XHAL_RPI5CAR_DEVICE_PRODUCT_VERSION_PROPERTY));
  static_cast<void>(
      removeFilesystemEntry(node / XHAL_RPI5CAR_DEVICE_UUID_PROPERTY));
  static_cast<void>(
      removeFilesystemEntry(node / XHAL_RPI5CAR_DEVICE_VENDOR_PROPERTY));
...
int32 runBoardControlSimulation() {
  ...
  const XWalkGpioCallbacks callbacks =
      XWalkBoardControlHostStub::gpioCallbacks();
  ...
  XWalkI2c i2c(&busBackend, &XWalkBoardControlHostStub::probeI2c,
  ...
  XWalkBoardControl control(resetGpio, speakerGpio, adc, &speakerBackend,
                            &XWalkBoardControlHostStub::primeSpeaker);
  control.resetMcu();
  ...
  const boolean valid =
      resetBackend.gpioValue() && !speakerBackend.gpioValue() &&
      (speakerBackend.primeCount() == 1U) && (voltage > 9.8) &&
      (voltage < 10.0) && (version.major == 2U) && (version.minor == 5U) &&
      (version.patch == 5U) && detected;
