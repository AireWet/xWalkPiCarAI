...
XWalkHal::int32 main(XWalkHal::int32 count, XWalkHal::charpointer values[]) {
  ...
  const xwalk::hal::sim::XWalkBoardControlSimulationArguments arguments(count,
                                                                        values);
  if (!arguments.valid()) {
    XWALK_HAL_ERROR(XWALK_EXCEPTION,
                    "Invalid xWalkBoardControl simulation arguments");
  ...
  XWALK_HAL_TRACE_UID1(RPI .332,
                       "xWalkBoardControl simulation completed with status %d",
                       result);
  return result;
}
