...
bytevector readFirmwareRegister(contextpointer context, uint8 address,
                                uint8 reg, size length) {
  TestFirmwareBus &bus = *static_cast<TestFirmwareBus *>(context);
...
void writeProperty(const filesystempath &path, stringview value,
                   boolean appendNull) {
...
void createSupportedNode(const filesystempath &nodePath) {
  ...
  writeProperty(nodePath / XHAL_RPI5CAR_DEVICE_PRODUCT_PROPERTY, "Robot HAT 5",
                false);
  writeProperty(nodePath / XHAL_RPI5CAR_DEVICE_PRODUCT_ID_PROPERTY, "00001902",
                true);
...
void removeNode(const filesystempath &nodePath) {
  static_cast<void>(
      removeFilesystemEntry(nodePath / XHAL_RPI5CAR_DEVICE_PRODUCT_PROPERTY));
  ...
  static_cast<void>(
      removeFilesystemEntry(nodePath / XHAL_RPI5CAR_DEVICE_UUID_PROPERTY));
  static_cast<void>(
