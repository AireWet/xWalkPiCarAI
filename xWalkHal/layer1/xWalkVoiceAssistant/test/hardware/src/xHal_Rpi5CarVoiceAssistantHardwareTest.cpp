...
XWalkHal::boolean probeI2c(XWalkHal::contextpointer context,
 ...
 * smoke test. */
void primeSpeaker(XWalkHal::contextpointer context,
                  XWalkHal::uint32 durationMs) {
  static_cast<void>(context);
...
XWalkHal::string recognizePcm(XWalkHal::contextpointer context,
  ...
  const hal::boolean pcmDataSampleRateHzChannelCountInvalid =
      static_cast<hal::boolean>(pcmData.empty() || (sampleRateHz != 16'000U) ||
                                (channelCount != 1U));
  if (pcmDataSampleRateHzChannelCountInvalid) {
    XWALK_HAL_ERROR(
        XWALK_RUNTIME,
        "Voice-assistant smoke test received invalid microphone PCM");
  }
...
synthesizeFixture(XWalkHal::contextpointer context, XWalkHal::stringview text) {
  ...
  if (textEmpty) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Voice-assistant smoke response must not be empty");
  }
  const SmokeProviders &providers = *static_cast<SmokeProviders *>(context);
  const XWalkHal::string contents =
      XWalkHal::readFileContents(providers.synthesisFixture);
  const hal::boolean contentsInvalid = static_cast<hal::boolean>(
...
XWalkHal::int32 main(XWalkHal::int32 argumentCount,
  ...
  if (argumentCount != 9) {
    XWALK_HAL_ERROR(
        XWALK_INVAL,
        "Voice-assistant hardware test requires eight explicit settings");
  }

  SmokeProviders providers{argumentValues[7], argumentValues[8], false};
  const hal::boolean promptEmpty =
      static_cast<hal::boolean>(providers.prompt.empty());
  if (promptEmpty) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Voice-assistant hardware prompt must not be empty");
  }
  ...
  XWalkHal::XWalkGpio speakerGpio(
      &providers, gpioCallbacks,
  ...
  XWalkHal::XWalkBoardControl boardControl(resetGpio, speakerGpio, batteryAdc,
                                           nullptr, &primeSpeaker);
  XWalkHal::XWalkAudioAlsa audio(argumentValues[2], argumentValues[3],
                                 argumentValues[4]);
  ...
  XWalkHal::XWalkSpeechToTextAlsa speechBackend(argumentValues[1], &providers,
                                                recognizerOperations);
  XWalkHal::XWalkSpeechToText speechToText(&speechBackend,
                                           speechBackend.callbacks());
  XWalkHal::XWalkLanguageModelOllama modelBackend(argumentValues[5],
                                                  argumentValues[6]);
  XWalkHal::XWalkLanguageModel languageModel(&modelBackend,
                                             modelBackend.callbacks());
  const XWalkHal::XWalkTextToSpeechAlsaOperations synthesisOperations{
      &synthesizeFixture};
  XWalkHal::XWalkTextToSpeechAlsa speechOutputBackend(audio, &providers,
                                                      synthesisOperations, 15U);
  XWalkHal::XWalkTextToSpeech textToSpeech(boardControl, &speechOutputBackend,
                                           speechOutputBackend.callback());
  XWalkHal::XWalkVoiceAssistant assistant(
      speechToText, languageModel, textToSpeech, {"Answer briefly.", {}});
  ...
  if (responseEmpty) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Voice-assistant hardware response is empty");
  }
  return 0;
}
