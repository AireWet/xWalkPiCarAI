    ...
    optionalfloat64 normalizedVolume)
{
    ...
    ++backend.soundCount; backend.filename = string(filename);
...
{
    TestBackend& backend = *static_cast<TestBackend*>(context);
    ++backend.backgroundSoundCount; backend.filename = string(filename);
    backend.soundVolume = normalizedVolume;
}
...
{
    TestBackend& backend = *static_cast<TestBackend*>(context);
    ++backend.musicCount; backend.filename = string(filename);
    backend.loops = loops; backend.startSeconds = startSeconds;
}
void setMusicVolume(contextpointer context, float64 normalizedVolume)
{
    TestBackend& backend = *static_cast<TestBackend*>(context);
    ++backend.volumeCount; backend.musicVolume = normalizedVolume;
}
...
{
    TestBackend& backend = *static_cast<TestBackend*>(context);
    backend.filename = string(filename); return backend.soundDurationSeconds;
}
void playTone(contextpointer context, const bytevector& pcmData,
    uint32 sampleRateHz, uint8 channelCount)
{
    TestBackend& backend = *static_cast<TestBackend*>(context);
    ++backend.toneCount; backend.pcmData = pcmData;
    backend.sampleRateHz = sampleRateHz; backend.channelCount = channelCount;
}
