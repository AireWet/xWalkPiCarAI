...
XWalkMusicAlsa::XWalkMusicAlsa(
  ...
  if (operations.decodeAudio == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Music ALSA adapter requires a decoder callback");
  }
}
...
XWalkMusicAlsa &XWalkMusicAlsa::adapter(contextpointer context) {
  if (context == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Music ALSA callback context must not be null");
  }
...
void XWalkMusicAlsa::stopSoundWorker() {
  ...
  const hal::boolean soundWorkerJoinable =
      static_cast<hal::boolean>(soundWorker.joinable());
...
void XWalkMusicAlsa::stopMusicWorker() {
  ...
  const hal::boolean musicWorkerJoinable =
      static_cast<hal::boolean>(musicWorker.joinable());
