...
{
    /** @brief Fake MCU-reset GPIO state. */
    test::gpt::TestGpioBackend resetBackend{};
    /** @brief Fake speaker-enable GPIO state. */
    test::gpt::TestGpioBackend speakerBackend{};
    /** @brief Fake board I2C state. */
    test::gpt::TestI2cBackend i2cBackend{};
    /** @brief Fake speaker-prime state. */
    test::gpt::TestSpeakerPrime primeBackend{};
    /** @brief Fake speech-recognition state. */
    test::gpt::TestRecognitionBackend recognitionBackend{};
    /** @brief Fake language-model state. */
    test::language_model::TestLanguageModelBackend modelBackend{};
    /** @brief Fake speech-output state. */
    test::gpt::TestSpeechBackend outputBackend{};
    /** @brief MCU-reset GPIO interface. */
    XWalkGpio resetGpio;
    /** @brief Speaker-enable GPIO interface. */
    XWalkGpio speakerGpio;
    /** @brief Board I2C interface. */
    XWalkI2c i2c;
    /** @brief Battery ADC required by board control. */
    XWalkAdc batteryAdc;
    /** @brief Real board-control coordinator. */
    XWalkBoardControl boardControl;
    /** @brief Real speech-to-text coordinator from xWalkGPT. */
    XWalkSpeechToText speechToText;
    /** @brief Real provider-neutral language-model coordinator. */
    XWalkLanguageModel languageModel;
    /** @brief Real text-to-speech coordinator from xWalkGPT. */
    XWalkTextToSpeech textToSpeech;
    /** @brief Real Layer 1 voice-assistant coordinator. */
    XWalkVoiceAssistant assistant;

    /** @brief Constructs dependencies in their required lifetime order. */
    VoicePipelineFixture()
        : resetGpio(&resetBackend, test::gpt::gpioCallbacks(), "MCURST"),
          speakerGpio(&speakerBackend, test::gpt::gpioCallbacks(),
              XHAL_RPI5CAR_DEVICE_DEFAULT_SPEAKER_ENABLE_PIN),
          i2c(&i2cBackend, &test::gpt::probeI2c, &test::gpt::writeI2c, &test::gpt::readI2c),
          batteryAdc(i2c, XHAL_RPI5CAR_BOARD_CONTROL_BATTERY_ADC_CHANNEL,
              XHAL_RPI5CAR_ADC_ADDRESS_1),
          boardControl(resetGpio, speakerGpio, batteryAdc, &primeBackend,
              &test::gpt::primeSpeaker),
          speechToText(&recognitionBackend, test::gpt::recognitionCallbacks()),
          languageModel(&modelBackend, test::language_model::backendCallbacks()),
          textToSpeech(boardControl, &outputBackend, &test::gpt::speakText),
          assistant(speechToText, languageModel, textToSpeech,
              XWalkVoiceAssistantConfiguration{"Answer briefly", "Ready"})
    {
    }
};
...
TEST(XWalkLayer1Group, BoardControlAndRobotInitializationUseInjectedDependencies)
{
    ...
    XWalkGpio speakerGpio(&speakerBackend, gpioCallbackTable,
        XHAL_RPI5CAR_DEVICE_DEFAULT_SPEAKER_ENABLE_PIN);
    XWalkI2c boardI2c(
        &boardBus, &boardcontrol::probeI2c, &boardcontrol::writeI2c, &boardcontrol::readI2c);
    XWalkAdc batteryAdc(boardI2c, XHAL_RPI5CAR_BOARD_CONTROL_BATTERY_ADC_CHANNEL,
        XHAL_RPI5CAR_ADC_ADDRESS_1);
    XWalkBoardControl boardControl(
        resetGpio, speakerGpio, batteryAdc, &primeBackend, &boardcontrol::primeSpeaker);
...
TEST(XWalkLayer1Group, VoiceAssistantStopsDataFlowAfterCriticalFailures)
{
    ...
    const uint32 outputCountBeforeRecognitionFailure =
        recognitionFailureFixture.outputBackend.callCount;
    EXPECT_THROW(
        static_cast<void>(recognitionFailureFixture.assistant.runRound()), std::runtime_error);
    EXPECT_EQ(recognitionFailureFixture.modelBackend.promptCount, 0U);
    EXPECT_EQ(recognitionFailureFixture.outputBackend.callCount,
        outputCountBeforeRecognitionFailure);
