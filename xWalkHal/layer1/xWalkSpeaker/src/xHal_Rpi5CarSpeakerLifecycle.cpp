 ...
 */
XWalkSpeaker::XWalkSpeaker(contextpointer context,
                           const XWalkSpeakerCallbacks &backendCallbacks)
    : backendContext(context), callbacks(backendCallbacks) {
  validateCallbacks(callbacks);
  enableSpeaker();
  XWALK_HAL_TRACE_UID0(RPI .308,
                       "Speaker controller constructed with output enabled");
}
 ...
 */
void XWalkSpeaker::validateCallbacks(
    const XWalkSpeakerCallbacks &backendCallbacks) {
  const boolean outputMissing = (backendCallbacks.enableOutput == nullptr) ||
                                (backendCallbacks.disableOutput == nullptr);
  ...
  const boolean dataMissing = (backendCallbacks.decodeAudio == nullptr) ||
                              (backendCallbacks.createTaskId == nullptr);
  if (outputMissing || streamMissing || dataMissing) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Speaker backend requires a complete callback table");
  }
}
