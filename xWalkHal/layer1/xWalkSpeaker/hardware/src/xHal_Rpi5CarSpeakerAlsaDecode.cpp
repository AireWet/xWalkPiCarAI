...
uint16 XWalkSpeakerAlsa::readUint16(stringview data, size byteOffset) {
  ...
  if (byteOffsetInvalid) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Speaker WAVE file contains a truncated value");
  }
...
XWalkSpeakerAlsa::decodeWave(contextpointer context, stringview filePath,
                             XWalkSpeakerAudioHandler handler) {
  static_cast<void>(context);
  if (handler != XWalkSpeakerAudioHandler::SoundFile) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Speaker compressed format requires an optional decoder");
  }
  ...
  if (fileBytes > XHAL_RPI5CAR_SPEAKER_MAXIMUM_INPUT_BYTES) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Speaker audio file exceeds the decoder input bound");
  }
  ...
  if (predicateInvalid) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Speaker built-in decoder requires a PCM WAVE file");
  }
  ...
  while (processingLoopRequested) {
    ...
    if (payloadOffsetChunkBytesInvalid) {
      XWALK_HAL_ERROR(XWALK_RUNTIME,
                      "Speaker WAVE file contains a truncated chunk");
    }
    if (chunkName == "fmt ") {
      if (chunkBytes < 16U) {
        XWALK_HAL_ERROR(XWALK_RUNTIME,
                        "Speaker WAVE format chunk is too short");
      }
    ...
    const size maximumChunkBytes =
        std::numeric_limits<size>::max() - payloadOffset;
    if ((chunkBytes > maximumChunkBytes) ||
        (paddingBytes > (maximumChunkBytes - chunkBytes))) {
      XWALK_HAL_ERROR(XWALK_RUNTIME,
                      "Speaker WAVE chunk size is unrepresentable");
    }
  ...
  if (formatTagSampleBitsChannelCountInvalid) {
    XWALK_HAL_ERROR(
        XWALK_RUNTIME,
        "Speaker WAVE decoder requires non-empty sixteen-bit PCM audio");
  }

  const size sampleCount = pcmBytes.size() / 2U;
  if (sampleCount > XHAL_RPI5CAR_SPEAKER_MAXIMUM_DECODED_SAMPLE_COUNT) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Speaker decoded sample count exceeds its bound");
  }
  ...
  for (size sampleIndex = 0U; sampleIndex < sampleCount; ++sampleIndex) {
    ...
    const int32 signedSample = (packedSample <= 0x7FFFU)
                                   ? static_cast<int32>(packedSample)
 ...
 */
void XWalkSpeakerAlsa::validateDecodedAudio(
  ...
  if ((audioData.sampleRateHz == 0U) || (audioData.channelCount == 0U) ||
    ...
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Speaker decoder returned invalid audio metadata");
  }
  const hal::boolean samplesEmpty =
      static_cast<hal::boolean>(audioData.samples.empty());
  if (samplesEmpty) {
    XWALK_HAL_ERROR(XWALK_RUNTIME, "Speaker decoder returned no audio samples");
  }
  const hal::boolean samplesTooLarge = static_cast<hal::boolean>(
      audioData.samples.size() >
      XHAL_RPI5CAR_SPEAKER_MAXIMUM_DECODED_SAMPLE_COUNT);
  if (samplesTooLarge) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Speaker decoder returned an invalid bounded sample count");
  }
  const size channelCount = static_cast<size>(audioData.channelCount);
  const hal::boolean audioDataSamplesChannelCountDifferent =
      static_cast<hal::boolean>((audioData.samples.size() % channelCount) !=
                                0U);
  if (audioDataSamplesChannelCountDifferent) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Speaker decoder returned incomplete interleaved frames");
  }
  for (const float64 sampleValue : audioData.samples) {
    const hal::boolean sampleInvalid =
        static_cast<hal::boolean>(!XHAL_IS_FINITE(sampleValue) ||
                                  (sampleValue < -1.0) || (sampleValue > 1.0));
    if (sampleInvalid) {
      XWALK_HAL_ERROR(
          XWALK_RUNTIME,
          "Speaker decoder returned a sample outside minus one through one");
    }
  }
}
