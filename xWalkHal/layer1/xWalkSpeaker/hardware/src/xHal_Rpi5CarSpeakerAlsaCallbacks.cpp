...
XWalkSpeakerAlsa::decodeAudio(contextpointer context, stringview filePath,
...
speakerstreamhandle XWalkSpeakerAlsa::openStream(contextpointer context,
  ...
  const XWalkAudioStreamConfiguration configuration{
      sampleRateHz, channelCount, XWalkAudioSampleFormat::Float32LittleEndian,
...
void XWalkSpeakerAlsa::writeStream(contextpointer context,
  ...
  static_assert(sizeof(float32) == 4U,
                "Speaker ALSA requires four-byte float32 samples");
  if ((frameCount == 0U) ||
      (frameCount > XHAL_RPI5CAR_SPEAKER_CHUNK_FRAME_COUNT) ||
      (audioData.channelCount == 0U) ||
      (audioData.channelCount > XHAL_RPI5CAR_AUDIO_MAXIMUM_CHANNEL_COUNT)) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Speaker ALSA write has invalid frame metadata");
  }
  const size channelCount = static_cast<size>(audioData.channelCount);
  const hal::boolean firstFrameOffsetOverflow = static_cast<hal::boolean>(
  ...
  const hal::boolean sampleCountOverflow = static_cast<hal::boolean>(
  ...
  if (firstSampleAudioDataSamplesInvalid) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Speaker ALSA write exceeds decoded audio data");
  }
  ...
  for (size sampleOffset = 0U; sampleOffset < sampleCount; ++sampleOffset) {
    ...
    if (sampleInvalid) {
      XWALK_HAL_ERROR(XWALK_INVAL,
                      "Speaker ALSA sample is outside minus one through one");
    }
...
string XWalkSpeakerAlsa::createTaskId(contextpointer context) {
  ...
  const hal::boolean selfTaskIdentifierMatched = static_cast<hal::boolean>(
      self.taskIdentifierValue == std::numeric_limits<uint64>::max());
  if (selfTaskIdentifierMatched) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Speaker ALSA task identifier sequence is exhausted");
  }
