...
void writeSilentWave(const XWalkHal::filesystempath &filePath) {
  ...
  if (fixtureWriteFailed) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Speaker hardware test could not write its WAVE fixture");
  }
}
...
XWalkHal::int32 main(XWalkHal::int32 argumentCount,
  ...
  if ((argumentCount != 1) && (argumentCount != 4)) {
    XWALK_HAL_ERROR(
        XWALK_INVAL,
        "Speaker hardware test accepts PCM, mixer, and element names");
  }
  ...
  const XWalkHal::filesystempath fixturePath(
      "/tmp/xwalk-speaker-alsa-hardware-test.wav");
  ...
  for (XWalkHal::uint32 attempt = 0U; attempt < 200U; ++attempt) {
    const hal::boolean listTasksEmpty =
        static_cast<hal::boolean>(speaker.listTasks().empty());
    if (listTasksEmpty) {
  ...
  if (!playbackComplete) {
    ...
    XWALK_HAL_ERROR(
        XWALK_RUNTIME,
        "Speaker hardware test playback did not complete in two seconds");
  }
