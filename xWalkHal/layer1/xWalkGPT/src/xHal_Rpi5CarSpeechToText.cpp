...
boolean XWalkSpeechToText::isReady() const {
  ...
  XWALK_HAL_TRACE_UID1(RPI .358, "Speech-to-text readiness is %u",
                       static_cast<uint32>(readyValue));
  return readyValue;
}
...
string XWalkSpeechToText::listen(uint32 timeoutMs) {
  ...
  XWALK_HAL_TRACE_UID1(
      RPI .359,
      "Speech-to-text listening completed after a %u millisecond request",
      timeoutMs);
  return result;
}
...
string XWalkSpeechToText::transcribeFile(stringview filePath) {
  ...
  const string result =
      callbacks.transcribeFile(backendContextPointer, filePath);
  XWALK_HAL_TRACE_UID2(RPI .360, "Speech-to-text transcribed file %.*s",
                       static_cast<int32>(filePath.size()), filePath.data());
  return result;
}
 ...
 * Any exception raised by the injected callback is propagated.
 */
void XWalkSpeechToText::stop() { callbacks.stop(backendContextPointer); }
