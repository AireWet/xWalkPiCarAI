...
void testInputValidation() {
  ...
  xwalk::hal::test::expectFailure([&]() {
    static_cast<void>(
        speech.listen(XHAL_RPI5CAR_SPEECH_TO_TEXT_MAXIMUM_TIMEOUT_MS + 1U));
  });
...
void testBackendFailures() {
  ...
  xwalk::hal::test::expectFailure([&]() { speech.stop(); });
  backend.failStop = false;
}
void testCallbackValidation() {
  TestRecognitionBackend backend;
  const fixedarray<XWalkSpeechToTextCallbacks, 4U> incomplete{
      {{nullptr, &listen, &transcribeFile, &stop},
