 ...
 */
XWalkTextToSpeechAlsa::XWalkTextToSpeechAlsa(
    XWalkAudioAlsa &sharedAudioBackend, contextpointer context,
    const XWalkTextToSpeechAlsaOperations &backendOperations,
    uint8 playbackVolumePercent)
    : audioBackend(&sharedAudioBackend), providerContext(context),
  ...
  if (operations.synthesize == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Text-to-speech ALSA adapter requires synthesis");
  }
  if (playbackVolumePercentValue > 100U) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Text-to-speech ALSA volume exceeds one hundred percent");
  }
}
 ...
 */
void XWalkTextToSpeechAlsa::validateAudioData(
  ...
  if (pcmDataTooLarge) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Text-to-speech PCM exceeds bounded byte count");
  }
  const hal::boolean pcmDataEmpty =
      static_cast<hal::boolean>(audioData.pcmData.empty());
  if (pcmDataEmpty) {
    return;
  }
  ...
  const size bytesPerFrame =
      channelCount * XHAL_RPI5CAR_TEXT_TO_SPEECH_SAMPLE_BYTES;
  const hal::boolean audioDataPcmDataBytesPerFrameDifferent =
      static_cast<hal::boolean>((audioData.pcmData.size() % bytesPerFrame) !=
                                0U);
...
XWalkTextToSpeechAlsa &XWalkTextToSpeechAlsa::adapter(contextpointer context) {
  if (context == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Text-to-speech ALSA context must not be null");
  }
...
void XWalkTextToSpeechAlsa::speak(contextpointer context, stringview text) {
  ...
  const XWalkAudioStreamConfiguration configuration{
      audioData.sampleRateHz, audioData.channelCount,
  ...
  while (framePosition < totalFrames) {
    ...
    const size periodFrames =
        static_cast<size>(XHAL_RPI5CAR_TEXT_TO_SPEECH_PERIOD_FRAMES);
    ...
    const auto firstIterator =
        audioData.pcmData.begin() +
        static_cast<bytevector::difference_type>(firstByte);
    const auto finalIterator =
        firstIterator + static_cast<bytevector::difference_type>(byteCount);
