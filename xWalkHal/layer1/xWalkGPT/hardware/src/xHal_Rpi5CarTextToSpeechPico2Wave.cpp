 ...
 */
texttospeechspeakcallback
XWalkTextToSpeechPico2Wave::callback() const noexcept {
  return &speak;
}
...
XWalkTextToSpeechPico2Wave::provider(contextpointer context) {
  if (context == nullptr) {
 ...
 */
void XWalkTextToSpeechPico2Wave::speak(contextpointer context,
                                       stringview text) {
  ...
  if (textTooLarge) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Pico2Wave text exceeds the bounded input length");
  }
...
void XWalkTextToSpeechPico2Wave::execute(stringview text) const {
  ...
  if (synthesisProcess == 0) {
    ...
    ::execlp(executableName.c_str(), executableName.c_str(), "-l",
             languageName.c_str(), "-w", temporaryPath, ownedText.c_str(),
