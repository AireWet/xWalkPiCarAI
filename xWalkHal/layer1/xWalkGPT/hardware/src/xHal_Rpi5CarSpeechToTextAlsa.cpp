...
XWalkSpeechToTextAlsa::XWalkSpeechToTextAlsa(
    stringview deviceName, contextpointer recognizerContext,
    const XWalkSpeechToTextAlsaOperations &recognizerOperations)
    : XWalkSpeechToTextAlsa(recognizerContext,
                            systemCaptureOperations(recognizerOperations),
                            deviceName) {}
...
XWalkSpeechToTextAlsa::XWalkSpeechToTextAlsa(
    ...
    : operationContext(context), operations(backendOperations),
 ...
 */
XWalkSpeechToTextAlsa::~XWalkSpeechToTextAlsa() { stopCallback(this); }
...
void XWalkSpeechToTextAlsa::validateOperations(
  ...
  const boolean captureMissing =
      (backendOperations.openCapture == nullptr) ||
...
XWalkSpeechToTextAlsa &XWalkSpeechToTextAlsa::adapter(contextpointer context) {
  if (context == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Speech ALSA callback context must not be null");
  }
 ...
 * fails.
 */
string XWalkSpeechToTextAlsa::listenCallback(contextpointer context,
                                             uint32 timeoutMs) {
  XWalkSpeechToTextAlsa &self = adapter(context);
  self.cancellationRequested.store(false);
  speechcapturehandle capture = self.operations.openCapture(
      self.operationContext, self.microphoneDeviceName,
  ...
  if (capture == nullptr) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Speech ALSA microphone could not be opened");
  }
  ...
  const uint64 maximumBytes =
      maximumFrames * XHAL_RPI5CAR_SPEECH_CAPTURE_SAMPLE_BYTES;
  ...
  while (processingLoopRequested) {
    const hal::boolean captureMayContinue = static_cast<hal::boolean>(
    ...
    const size readFrames =
        static_cast<size>(std::min(remainingFrames, periodFrames));
    ...
    if (result > 0) {
      ...
      const size expectedBytes =
          completedFrames * XHAL_RPI5CAR_SPEECH_CAPTURE_SAMPLE_BYTES;
      const hal::boolean completedFramesReadFramesPeriodDataInvalid =
          static_cast<hal::boolean>((completedFrames > readFrames) ||
                                    (periodData.size() != expectedBytes));
      if (completedFramesReadFramesPeriodDataInvalid) {
        self.operations.closeCapture(self.operationContext, capture);
        XWALK_HAL_ERROR(XWALK_RUNTIME,
                        "Speech ALSA capture returned malformed PCM");
      }
      capturedPcm.insert(capturedPcm.end(), periodData.begin(),
                         periodData.end());
      ...
      const hal::boolean resultInvalid = static_cast<hal::boolean>(
          ...
          !self.operations.recoverCapture(self.operationContext, capture,
                                          result));
      if (resultInvalid) {
        self.operations.closeCapture(self.operationContext, capture);
  ...
  const hal::boolean selfCancellationRequestedCapturedPcmInvalid =
      static_cast<hal::boolean>(self.cancellationRequested.load() ||
                                capturedPcm.empty());
  if (selfCancellationRequestedCapturedPcmInvalid) {
    return {};
  }
  return self.operations.recognizePcm(
      self.operationContext, capturedPcm,
      XHAL_RPI5CAR_SPEECH_CAPTURE_SAMPLE_RATE_HZ,
      XHAL_RPI5CAR_SPEECH_CAPTURE_CHANNEL_COUNT);
}
 ...
 */
string XWalkSpeechToTextAlsa::fileCallback(contextpointer context,
                                           stringview filePath) {
  XWalkSpeechToTextAlsa &self = adapter(context);
  self.cancellationRequested.store(false);
