...
configurePcm(XWalkHal::contextpointer context,
  ...
  assert(configuration.periodFrames ==
...
XWalkHal::int32 writePcm(XWalkHal::contextpointer context,
  ...
  backend.writtenPcm.insert(backend.writtenPcm.end(), pcmData.begin(),
                            pcmData.end());
...
void testValidation() {
  ...
  backend.audioData = {
      XWalkHal::bytevector(XHAL_RPI5CAR_TEXT_TO_SPEECH_MAXIMUM_PCM_BYTES + 1U,
                           0U),
      16'000U, 1U};
  xwalk::hal::test::expectFailure(
