 ...
 *
 * @throws outofrange
 * If `channel` exceeds two or the calibrated result exceeds the signed count range.
 *
...
float64 XWalkLineTracker::getLinePosition(const linetrackervalues& data)
{
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("Line-tracker references must not be equal");
    }
    ...
    const boolean leftOnly = (leftWeight >= minimumWeight) &&
        (middleWeight < minimumWeight) && (rightWeight < minimumWeight);
    const boolean rightOnly = (rightWeight >= minimumWeight) &&
        (middleWeight < minimumWeight) && (leftWeight < minimumWeight);
    if (leftOnly)
    {
    ...
    const float64 constrainedPosition = constrain(normalizedPosition,
        XHAL_RPI5CAR_LINE_TRACKER_MINIMUM_POSITION,
        XHAL_RPI5CAR_LINE_TRACKER_MAXIMUM_POSITION);
    return roundTwoDecimals(constrainedPosition);
}
