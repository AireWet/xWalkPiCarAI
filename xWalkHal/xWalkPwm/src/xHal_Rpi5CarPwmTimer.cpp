...
void XWalkPwm::setFrequency(float64 frequencyHz)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("frequency exceeds the uint32 range");
    }

    const uint32 requestedFrequencyHzInteger = static_cast<uint32>(frequencyHz);
    if (requestedFrequencyHzInteger == 0U)
    {
        XHAL_THROW_INVALID_ARGUMENT("frequency is too small");
    }
    ...
    if (bestPrescaler == 0U)
    {
        XHAL_THROW_OUT_OF_RANGE("frequency cannot be represented");
    }
