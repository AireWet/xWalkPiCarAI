...
XWalkAdc &XWalkLineTracker::sensorAt(uint32 channel) const {
  if (channel >= XHAL_RPI5CAR_LINE_TRACKER_CHANNEL_COUNT) {
    XWALK_HAL_ERROR(
        XWALK_RANGE,
        "Line-tracker channel must be in the range zero through two");
  }
 ...
 */
void XWalkLineTracker::validateCalibration(
  ...
  for (uint32 channel = 0U; channel < XHAL_RPI5CAR_LINE_TRACKER_CHANNEL_COUNT;
    ...
    if (calibrationSlopesChannelInvalid) {
      XWALK_HAL_ERROR(XWALK_INVAL,
                      "Line-tracker calibration values must be finite");
    }
  }
}
...
int32 XWalkLineTracker::roundReading(float64 value) {
  const hal::boolean valueNotFinite =
      static_cast<hal::boolean>(!XHAL_IS_FINITE(value));
  if (valueNotFinite) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Calibrated line-tracker value must be finite");
  }
  ...
  if ((roundedValue < minimumValue) || (roundedValue > maximumValue)) {
    XWALK_HAL_ERROR(
        XWALK_RANGE,
        "Calibrated line-tracker value exceeds the signed count range");
  }
 ...
 * Restricted value.
 */
float64 XWalkLineTracker::constrain(float64 value, float64 minimum,
                                    float64 maximum) noexcept {
 ...
 */
XWalkLineTracker::XWalkLineTracker(XWalkAdc &left, XWalkAdc &middle,
                                   XWalkAdc &right)
