...
void testPwmAndDirectionMode() {
  ...
  xwalk::hal::test::expectFailure([&motor]() { motor.setSpeed(1.0); });
  assert(bus.writeCount == writesBeforeMotorConstruction);
...
void testDualPwmMode() {
  ...
  assert(bus.writeCount == writesBeforeMotorConstruction);
  xwalk::hal::test::expectFailure([&motor]() { motor.brake(); });
  assert(bus.writeCount == writesBeforeMotorConstruction);
  assert(motor.initialize());
  motor.setSpeed(40.0);
...
void testValidation() {
  ...
  motors.arm();
  xwalk::hal::test::expectFailure([&]() { motor.setSpeed(101.0); });
  xwalk::hal::test::expectFailure(
      [&]() { motor.setSpeed(XHAL_POSITIVE_INFINITY(XWalkHal::float64)); });
  xwalk::hal::test::expectFailure([&]() { motor.brake(); });
  xwalk::hal::test::expectFailure([&]() {
    xwalk::hal::XWalkMotor invalidFrequencyMotor(pwm, direction, false, 0.0);
  });
  xwalk::hal::test::expectFailure([&]() { motors.setLeftMotorId(0U); });
  motors.setSpeed(12.0, 13.0);
  xwalk::hal::test::expectFailure([&]() { motors.setSpeed(20.0, 101.0); });
...
void testDeterministicWatchdog() {
  ...
  xwalk::hal::test::expectFailure([&motors]() { motors.setSpeed(101.0, 0.0); });
  clock.milliseconds = 100U;
  assert(motors.checkWatchdog());
  assert(!motors.isArmed());
  assert((firstMotor.speed() == 0.0) && (secondMotor.speed() == 0.0));
  xwalk::hal::test::expectFailure([&motors]() { motors.forward(10.0); });
...
void testWatchdogConfigurationBounds() {
  ...
  configuration.watchdogTimeoutMilliseconds =
      xwalk::hal::XHAL_RPI5CAR_MOTOR_WATCHDOG_MAXIMUM_MILLISECONDS;
  ...
  configuration.watchdogTimeoutMilliseconds =
      xwalk::hal::XHAL_RPI5CAR_MOTOR_WATCHDOG_MAXIMUM_MILLISECONDS + 1U;
  xwalk::hal::test::expectFailure([&]() {
    xwalk::hal::XWalkMotors invalid(firstMotor, secondMotor, configuration);
  });
}
...
void testPartialInitializationFailure() {
  ...
  xwalk::hal::test::expectFailure(
      [&motor]() { static_cast<void>(motor.initialize()); });
  assert(!motor.initialized());
  assert(motor.speed() == 0.0);
  bus.failingWrites.clear();
  assert(motor.initialize());
...
void testTraceSelection() {
  ...
  XWalkHal::charpointer malformedArguments[]{executable, option,
                                             malformedSelector};
  const xwalk::hal::sim::XWalkMotorSimulationArguments malformed(
...
XWalkHal::int32 main() {
  xwalk::hal::XWalkTrace::configureGlobal(
      XWALK_MOTOR_SIMULATION_TRACE_CONFIG_PATH,
