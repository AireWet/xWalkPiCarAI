...
float64 XWalkMotor::validateSpeed(float64 speedPercent) {
  ...
  if ((speedPercent < XHAL_RPI5CAR_MOTOR_MIN_SPEED_PERCENT) ||
    ...
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "motor speed must be between -100 and 100 percent");
  }
  return speedPercent;
}
...
float64 XWalkMotor::validateFrequency(float64 frequencyHz) {
  ...
  if (frequencyHzInvalid) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "motor frequency must be finite and greater than zero");
  }
  return frequencyHz;
}
 ...
 */
XWalkMotor::XWalkMotor(XWalkPwm &pwm, XWalkGpio &direction, boolean reversed,
                       float64 frequencyHz)
    : pwmA(&pwm), directionPin(&direction),
      modeValue(XWalkMotorMode::PwmAndDirection),
      frequencyHzValue(validateFrequency(frequencyHz)),
      reversedValue(reversed) {
  XWALK_HAL_TRACE_UID1(
      RPI .245, "Uninitialized PWM-and-direction motor constructed at %.2f Hz",
      frequencyHzValue);
}
 ...
 */
XWalkMotor::XWalkMotor(XWalkPwm &forwardPwm, XWalkPwm &reversePwm,
                       boolean reversed, float64 frequencyHz)
    : pwmA(&forwardPwm), pwmB(&reversePwm), modeValue(XWalkMotorMode::DualPwm),
      frequencyHzValue(validateFrequency(frequencyHz)),
      reversedValue(reversed) {
  XWALK_HAL_TRACE_UID1(RPI .246,
                       "Uninitialized dual-PWM motor constructed at %.2f Hz",
                       frequencyHzValue);
}
 ...
 * dependency.
 */
XWalkMotor::~XWalkMotor() noexcept { static_cast<void>(stopSafely()); }
