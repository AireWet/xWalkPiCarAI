...
void XWalkMotor::setSpeed(float64 speedPercent) {
  ...
  if (initializedValue == false) {
    XWALK_HAL_ERROR(XWALK_LOGIC,
                    "motor movement requires explicit initialization");
  }
  ...
  XWALK_HAL_TRACE_UID1(RPI .247, "Motor speed applied at %.2f percent",
                       speedPercentValue);
}
...
void XWalkMotor::stop() {
  ...
  if (stopped == false) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "motor stop could not disable every PWM output");
  }
...
void XWalkMotor::brake() {
  ...
  if (initializedValue == false) {
    XWALK_HAL_ERROR(XWALK_LOGIC,
                    "motor braking requires explicit initialization");
  }
 ...
 */
float64 XWalkMotor::speed() const noexcept { return speedPercentValue; }
 ...
 */
boolean XWalkMotor::reversed() const noexcept { return reversedValue; }
 ...
 */
XWalkMotorMode XWalkMotor::mode() const noexcept { return modeValue; }
 ...
 */
float64 XWalkMotor::frequency() const noexcept { return frequencyHzValue; }
 ...
 */
boolean XWalkMotor::initialized() const noexcept { return initializedValue; }
