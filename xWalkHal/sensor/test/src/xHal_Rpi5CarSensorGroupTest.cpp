 ...
 */
void applyLineResponse(XWalkLineTracker& tracker, const linetrackervalues& values,
    XWalkMotors& motors, XWalkLed& statusLed, XWalkBuzzer& buzzer)
{
...
TEST_P(LineResponseGroupTest, TrackerObservationControlsMotorsLedAndBuzzer)
{
    ...
    XWalkI2c trackerI2c(
        &trackerBus, &linetracker::probe, &linetracker::writeRegister, &linetracker::read);
    ...
    XWalkI2c motorI2c(&motorBus, &motor::probe, &motor::writeRegister, &motor::read,
        nullptr, &motor::tryWriteRegister);
    ...
    EXPECT_THAT(values, testing::ElementsAre(
        responseCase.samples[0U], responseCase.samples[1U], responseCase.samples[2U]));
...
INSTANTIATE_TEST_SUITE_P(LinePositions, LineResponseGroupTest,
    testing::Values(
        LineResponseCase{{1'000U, 200U, 1'000U}, 35.0, 35.0, false},
...
TEST(XWalkSensorGroup, MotorWatchdogUsesInjectedClockAndStopsDeterministically)
{
    ...
    configuration.watchdogTimeoutMilliseconds =
        XHAL_RPI5CAR_MOTOR_WATCHDOG_MAXIMUM_MILLISECONDS + 1U;
    EXPECT_THROW(static_cast<void>(XWalkMotors(leftMotor, rightMotor, configuration)),
        std::out_of_range);
}
