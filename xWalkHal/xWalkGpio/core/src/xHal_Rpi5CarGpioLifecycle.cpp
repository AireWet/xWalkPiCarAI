 ...
 *
 * @details
 * Validates callback bindings and Robot HAT pins, performs initial configuration, and cancels handlers.
 *
...
uint8 XWalkGpio::resolvePin(stringview pinName)
{
    ...
    XHAL_THROW_INVALID_ARGUMENT("GPIO pin name is not supported");
}
...
{
    const boolean supported = (pin == 4U) || (pin == 5U) || (pin == 6U) || (pin == 8U) ||
        (pin == 12U) || (pin == 13U) || (pin == 16U) || (pin == 17U) || (pin == 19U) ||
        (pin == 20U) || (pin == 21U) || (pin == 22U) || (pin == 23U) || (pin == 24U) ||
        (pin == 25U) || (pin == 26U) || (pin == 27U);
    if (!supported)
    {
        XHAL_THROW_OUT_OF_RANGE("GPIO pin is not present in the Robot HAT map");
    }
...
XWalkGpioCallbacks XWalkGpio::validateCallbacks(const XWalkGpioCallbacks& callbacks)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("GPIO callbacks must not be null");
    }
    return callbacks;
}
 ...
 */
XWalkGpio::XWalkGpio(contextpointer context, const XWalkGpioCallbacks& callbacks, uint32 pin,
 ...
 */
XWalkGpio::XWalkGpio(contextpointer context, const XWalkGpioCallbacks& callbacks, stringview pinName,
    XWalkGpioMode mode, XWalkGpioPull pull, boolean activeHigh):
