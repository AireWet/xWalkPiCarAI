 ...
 * component object by value; create that object in `main()` and cache a pointer.
 */
template<typename ValueType>
class XWalkLazyReader final
{
    private:
        /**************************************************************************
    ...
    protected:
            ...
            utilityclockcallback clock)
        {
            ...
            {
                XHAL_THROW_INVALID_ARGUMENT("Lazy reader callbacks must not be null");
            }
        }
    ...
    public:
        ...
        XWalkLazyReader(contextpointer backendContext,
            utilityreadcallback<ValueType> readOperation, utilityclockcallback clockOperation,
            uint32 intervalMs = XHAL_RPI5CAR_UTILS_DEFAULT_LAZY_INTERVAL_MS)
            : backendContextPointer(backendContext),
              readCallback(readOperation),
