...
string XWalkConfig::parseSectionHeader(stringview line) {
  const hal::boolean lineInvalid = static_cast<hal::boolean>(
...
void XWalkConfig::ensureFileExists(stringview description) const {
  ...
  while (processingLoopRequested) {
    const hal::boolean descriptionTextAvailable =
        static_cast<hal::boolean>(lineStart < description.size());
    ...
    const hal::boolean carriageReturnPresent = static_cast<hal::boolean>(
  ...
  const hal::boolean descriptionAvailable =
      static_cast<hal::boolean>(!description.empty());
  ...
  XWALK_HAL_TRACE_UID1(RPI .108, "Configuration file created: %s",
                       ownedFilePath.c_str());
}
...
XWalkConfig::XWalkConfig(stringview filePath, stringview description)
  ...
  const string ownedFilePath = filePathValue.string();
  XWALK_HAL_TRACE_UID1(RPI .100, "Configuration loaded from %s",
                       ownedFilePath.c_str());
}
