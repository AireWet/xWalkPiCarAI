...
void XWalkConfigStore::appendConfigurationLines(const filesystempath &filePath,
  ...
  if (depth > maximumIncludeDepth) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Configuration include depth exceeds eight files");
  }
  ...
  const hal::boolean activePathsActivePathDifferent = static_cast<hal::boolean>(
      std::find(activePaths.begin(), activePaths.end(), activePath) !=
      activePaths.end());
  ...
  const hal::boolean regularFileNotMatched =
      static_cast<hal::boolean>(!isRegularFile(normalizedPath));
  if (regularFileNotMatched) {
    XWALK_HAL_ERROR(XWALK_RUNTIME,
                    "Configuration include is not a regular file");
  }
  ...
  for (const string &line : fileLines) {
    ...
    const filesystempath includePath(
...
void XWalkConfigStore::writeLines(const stringvector &lines) const {
  ...
  const filesystempermissions originalPermissions =
      originalStatus.permissions();
  errorcode permissionError;
  replaceFilesystemPermissions(replacementPath, originalPermissions,
                               permissionError);
  if (permissionError) {
    errorcode removeError;
    static_cast<void>(removeFilesystemEntry(replacementPath, removeError));
...
string XWalkConfigStore::get(stringview name, stringview defaultValue) const {
  ...
  for (const string &line : lines) {
    ...
    const hal::boolean targetOptionMatched = static_cast<hal::boolean>(
        (separator != string::npos) &&
        (trim(stringview(line).substr(0U, separator)) == name));
  ...
  XWALK_HAL_TRACE_UID1(RPI .106, "Configuration-store value read: %s",
                       ownedName.c_str());
  return result;
}
...
void XWalkConfigStore::set(stringview name, stringview value) {
  ...
  writeLines(lines);
  const string ownedName(name);
  XWALK_HAL_TRACE_UID1(RPI .107, "Configuration-store value persisted: %s",
                       ownedName.c_str());
}
 ...
 */
string XWalkConfigStore::filePath() const { return filePathValue.string(); }
