...
XWalkHal::int32 main(XWalkHal::int32 argumentCount,
  ...
  const xwalk::hal::sim::XWalkI2cSimulationArguments arguments(argumentCount,
                                                               argumentValues);
  ...
  if (traceUpdateApplied == false) {
    std::fprintf(stderr, "The requested trace identifier is not present in the "
                         "trace inventory.\n");
    return 2;
  }
  ...
  xwalk::hal::XWalkI2cLinux linuxBackend(*device,
                                         XWALK_I2C_SIMULATION_DEVICE_PATH, 1U);
