...
void XWalkGpio::setup(XWalkGpioMode mode, XWalkGpioPull pull) {
  close();
  callbacksValue.configure(contextValue, pinValue, mode, pull,
                           physicalValue(outputValue));
  modeValue = mode;
  pullValue = pull;
  XWALK_HAL_TRACE_UID3(RPI .062,
                       "GPIO line %u configured with mode %u and pull %u",
                       static_cast<uint32>(pinValue), static_cast<uint32>(mode),
...
boolean XWalkGpio::read() {
  ...
  const boolean value =
      physicalValue(callbacksValue.read(contextValue, pinValue));
  XWALK_HAL_TRACE_UID2(RPI .063, "GPIO line %u read logical value %u",
...
boolean XWalkGpio::write(boolean value) {
  ...
  outputValue = value;
  XWALK_HAL_TRACE_UID2(RPI .064, "GPIO line %u wrote logical value %u",
                       static_cast<uint32>(pinValue),
                       static_cast<uint32>(value));
  return value;
}
 ...
 */
boolean XWalkGpio::on() { return write(true); }
 ...
 */
boolean XWalkGpio::off() { return write(false); }
 ...
 * Always `true` after a successful write.
 */
boolean XWalkGpio::high() { return on(); }
 ...
 * Always `false` after a successful write.
 */
boolean XWalkGpio::low() { return off(); }
 ...
 * If `handler` is null.
 */
void XWalkGpio::irq(contextpointer handlerContext, gpiointerrupthandler handler,
                    XWalkGpioEdge edge, uint32 debounceMs, XWalkGpioPull pull) {
  if (handler == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL, "GPIO interrupt handler must not be null");
  }
  close();
  setup(XWalkGpioMode::Input, pull);
  callbacksValue.interrupt(contextValue, pinValue, edge, debounceMs,
                           handlerContext, handler);
  interruptActive = true;
  XWALK_HAL_TRACE_UID3(
      RPI .065, "GPIO line %u registered edge %u with %u milliseconds debounce",
      static_cast<uint32>(pinValue), static_cast<uint32>(edge), debounceMs);
}
...
void XWalkGpio::close() {
  if (interruptActive) {
    ...
    XWALK_HAL_TRACE_UID1(RPI .066, "GPIO line %u interrupt cancelled",
                         static_cast<uint32>(pinValue));
  }
}
 ...
 */
uint8 XWalkGpio::pin() const noexcept { return pinValue; }
 ...
 */
XWalkGpioMode XWalkGpio::mode() const noexcept { return modeValue; }
 ...
 */
XWalkGpioPull XWalkGpio::pull() const noexcept { return pullValue; }
 ...
 */
string XWalkGpio::name() const { return common::createGpioName(pinValue); }
