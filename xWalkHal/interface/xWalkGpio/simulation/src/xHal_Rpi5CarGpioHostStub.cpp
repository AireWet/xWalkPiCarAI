...
boolean XWalkGpioHostStub::readChipInformation(int32 chipDescriptor,
                                               contextpointer information) {
  const boolean requestValid =
      (chipDescriptor == 201) && (information != nullptr);
...
boolean XWalkGpioHostStub::requestLine(int32 chipDescriptor,
  ...
  const boolean inputRequested =
      (lineRequest->flags & static_cast<uint32>(GPIOHANDLE_REQUEST_INPUT)) !=
      0U;
  modeValue = inputRequested ? XWalkGpioMode::Input : XWalkGpioMode::Output;
  const boolean pullUpRequested =
      (lineRequest->flags &
       static_cast<uint32>(GPIOHANDLE_REQUEST_BIAS_PULL_UP)) != 0U;
  const boolean pullDownRequested =
      (lineRequest->flags &
       static_cast<uint32>(GPIOHANDLE_REQUEST_BIAS_PULL_DOWN)) != 0U;
  pullValue = pullUpRequested ? XWalkGpioPull::Up
                              : (pullDownRequested ? XWalkGpioPull::Down
                                                   : XWalkGpioPull::None);
  if (inputRequested == false) {
    lineValue = lineRequest->default_values[0U] != 0U;
  }
  ++configureCountValue;
  XWALK_HAL_TRACE_UID1(RPI .077, "Host GPIO mirrored line %u configuration",
...
boolean XWalkGpioHostStub::writeLine(int32 lineDescriptor,
                                     contextpointer data) {
  const boolean requestValid = (lineDescriptor == 202) && (data != nullptr);
  if (requestValid == false) {
    return false;
...
boolean XWalkGpioHostStub::requestEvent(int32 chipDescriptor,
                                        contextpointer request) {
  const boolean requestValid = (chipDescriptor == 201) && (request != nullptr);
  if (requestValid == false) {
    return false;
...
int32 XWalkGpioHostStub::readEvent(int32 lineDescriptor,
                                   contextpointer eventData, size length) {
  static_cast<void>(lineDescriptor);
...
uint8 XWalkGpioHostStub::pin() const noexcept { return pinValue; }
XWalkGpioMode XWalkGpioHostStub::mode() const noexcept { return modeValue; }
XWalkGpioPull XWalkGpioHostStub::pull() const noexcept { return pullValue; }
boolean XWalkGpioHostStub::value() const noexcept { return lineValue; }
size XWalkGpioHostStub::configureCount() const noexcept {
  return configureCountValue;
}
size XWalkGpioHostStub::readCount() const noexcept { return readCountValue; }
size XWalkGpioHostStub::writeCount() const noexcept { return writeCountValue; }
