 ...
 */
audiopcmhandle
XWalkAudioAlsa::openStream(const XWalkAudioStreamConfiguration &configuration) {
  ...
  audiopcmhandle pcmHandle =
      operations.openPcm(operationContextPointer, pcmDeviceNameValue);
  if (pcmHandle == nullptr) {
    XWALK_HAL_ERROR(XWALK_EXCEPTION,
                    "ALSA audio PCM device could not be opened");
  }
  ...
  XWALK_HAL_TRACE_UID2(
      RPI .086, "ALSA audio stream opened at %u Hertz with %u channel(s)",
...
void XWalkAudioAlsa::writeFrames(audiopcmhandle streamHandle,
  ...
  if (frameCount > periodFrames) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "ALSA audio write exceeds the configured period size");
  }
  ...
  if (frameCountBytesPerFramePcmDataInvalid) {
    XWALK_HAL_ERROR(
        XWALK_INVAL,
        "ALSA audio payload must contain complete configured frames");
  }
  ...
  while (writtenFrames < frameCount) {
    ...
    const size maximumAttemptFrames =
        static_cast<size>(std::numeric_limits<int32>::max());
    ...
    const int32 writeResult =
        operations.writePcm(operationContextPointer, streamHandle, pcmData,
                            byteOffset, attemptFrames);
    if (writeResult > 0) {
      const size completedFrames = static_cast<size>(writeResult);
      if (completedFrames > attemptFrames) {
        XWALK_HAL_ERROR(XWALK_RUNTIME,
                        "ALSA audio write returned too many frames");
      }
...
void XWalkAudioAlsa::setVolume(uint8 volumePercent) {
  if (volumePercent > 100U) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "ALSA audio volume exceeds one hundred percent");
  }
  const mutexlock lock(mutex);
  const hal::boolean volumeApplied =
      operations.setMixerVolume(operationContextPointer, mixerHandle,
                                mixerElementNameValue, volumePercent);
  if (volumeApplied == false) {
    XWALK_HAL_ERROR(XWALK_RUNTIME, "ALSA audio mixer volume update failed");
  }
  XWALK_HAL_TRACE_UID1(RPI .089, "ALSA audio volume set to %u percent",
