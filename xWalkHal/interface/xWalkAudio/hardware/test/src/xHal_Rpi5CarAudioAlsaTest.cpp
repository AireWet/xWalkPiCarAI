...
void testOwnershipAndWrites() {
  ...
  {
    XWalkAudioAlsa audio(&state, testOperations(), "test-pcm", "test-mixer",
                         "PCM");
...
void testStreamLimit() {
  TestAudioOperations state;
  XWalkAudioAlsa audio(&state, testOperations(), "pcm", "mixer", "PCM");
  for (size index = 0U; index < XHAL_RPI5CAR_AUDIO_MAXIMUM_STREAM_COUNT;
       ++index) {
    static_cast<void>(audio.openStream(testConfiguration()));
  }
...
void testValidationAndFailures() {
  ...
  xwalk::hal::test::expectFailure([&]() {
    const size excessiveFrames =
        static_cast<size>(testConfiguration().periodFrames) + 1U;
    audio.writeFrames(stream, bytevector(excessiveFrames * 4U, 0U),
                      excessiveFrames);
  });
  xwalk::hal::test::expectFailure([&]() { audio.setVolume(101U); });
  audio.closeStream(stream);
  ...
  XWalkAudioAlsa failedRecoveryAudio(&failedRecoveryState, testOperations(),
                                     "pcm", "mixer", "PCM");
  audiopcmhandle failedStream =
      failedRecoveryAudio.openStream(testConfiguration());
  xwalk::hal::test::expectFailure([&]() {
 ...
 */
int main() {
  const filesystempath traceConfigurationPath(
