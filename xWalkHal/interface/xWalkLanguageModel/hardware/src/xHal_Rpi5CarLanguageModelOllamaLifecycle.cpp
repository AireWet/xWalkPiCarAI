...
XWalkLanguageModelOllama::XWalkLanguageModelOllama(stringview endpoint,
    ...
    : XWalkLanguageModelOllama(
          nullptr, systemOperations(), XWalkLanguageModelHttpDialect::Ollama,
...
XWalkLanguageModelOllama::XWalkLanguageModelOllama(
    ...
    : XWalkLanguageModelOllama(
          context, backendOperations, XWalkLanguageModelHttpDialect::Ollama,
          endpoint, model, {}, timeoutMs,
          XHAL_RPI5CAR_LANGUAGE_MODEL_HTTP_DEFAULT_MAXIMUM_OUTPUT_TOKENS) {}
...
XWalkLanguageModelOllama::XWalkLanguageModelOllama(
    ...
    stringview model, stringview apiKey, uint32 timeoutMs,
    uint32 maximumOutputTokens)
    : transportContext(context), operations(backendOperations),
      endpointValue(endpoint), modelValue(model), dialectValue(dialect),
      apiKeyValue(apiKey), timeoutMsValue(timeoutMs),
 ...
 */
XWalkLanguageModelCallbacks
XWalkLanguageModelOllama::callbacks() const noexcept {
 ...
 */
XWalkLanguageModelHttpDialect
XWalkLanguageModelOllama::dialectFromString(stringview providerName) {
  if (providerName == "ollama") {
    return XWalkLanguageModelHttpDialect::Ollama;
  }
  if ((providerName == "openai") || (providerName == "chatgpt") ||
      (providerName == "gemini") || (providerName == "grok") ||
      (providerName == "xai") || (providerName == "claude") ||
...
void XWalkLanguageModelOllama::validateConstruction(
    const XWalkLanguageModelOllamaOperations &backendOperations,
    stringview endpoint, stringview model,
    XWalkLanguageModelHttpDialect dialect, stringview apiKey, uint32 timeoutMs,
    uint32 maximumOutputTokens) {
  ...
  const stringview chatSuffix =
      (dialect == XWalkLanguageModelHttpDialect::Ollama)
          ? stringview{"/api/chat"}
          : stringview{"/chat/completions"};
  const boolean hasChatSuffix =
      (endpoint.size() >= chatSuffix.size()) &&
  ...
  const hal::boolean endpointModelHttpEndpointInvalid =
      static_cast<hal::boolean>(endpoint.empty() || model.empty() ||
                                (!httpEndpoint && !httpsEndpoint) ||
                                !hasChatSuffix ||
                                (endpoint.find('\0') != stringview::npos) ||
                                (endpoint.find('\n') != stringview::npos) ||
  ...
  const hal::boolean ollamaCredentialConfigured = static_cast<hal::boolean>(
  ...
  const hal::boolean apiKeyNRInvalid = static_cast<hal::boolean>(
      (apiKey.size() >
       XHAL_RPI5CAR_LANGUAGE_MODEL_HTTP_MAXIMUM_API_KEY_BYTES) ||
  ...
  if ((timeoutMs == 0U) ||
    ...
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Ollama timeout is outside its supported range");
  }
  if ((maximumOutputTokens == 0U) ||
      (maximumOutputTokens >
       XHAL_RPI5CAR_LANGUAGE_MODEL_HTTP_MAXIMUM_OUTPUT_TOKENS)) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Language-model output-token limit is outside its range");
  }
}
...
void XWalkLanguageModelOllama::validateText(stringview text) {
  const hal::boolean textTooLarge = static_cast<hal::boolean>(
 ...
 */
XWalkLanguageModelOllama &
XWalkLanguageModelOllama::provider(contextpointer context) {
  if (context == nullptr) {
