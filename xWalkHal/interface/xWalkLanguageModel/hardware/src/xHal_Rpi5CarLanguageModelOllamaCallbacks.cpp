 ...
 * @throws std::out_of_range If the provider text limit is exceeded.
 */
void XWalkLanguageModelOllama::setWelcome(contextpointer context,
                                          stringview welcome) {
...
void XWalkLanguageModelOllama::setMaximumMessages(contextpointer context,
  ...
  if ((maximumMessages == 0U) ||
    ...
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Ollama history limit is outside its supported range");
  }
  ...
  const hal::boolean historyTooLarge =
      static_cast<hal::boolean>(self.history.size() > maximumSize);
  if (historyTooLarge) {
    const size eraseCount = self.history.size() - maximumSize;
    self.history.erase(
        self.history.begin(),
        self.history.begin() +
            static_cast<languagemodelollamamessagevector::difference_type>(
                eraseCount));
  }
}
...
void XWalkLanguageModelOllama::addMessage(contextpointer context,
  ...
  const XWalkLanguageModelOllamaMessage message{role, string(content),
...
string XWalkLanguageModelOllama::prompt(contextpointer context,
  ...
  const hal::boolean apiKeyAvailable =
      static_cast<hal::boolean>(!self.apiKeyValue.empty());
  if (apiKeyAvailable) {
    authorizationHeader = "Authorization: Bearer ";
    authorizationHeader += self.apiKeyValue;
  }
  const string responseJson = self.operations.postJson(
      self.transportContext, self.endpointValue, requestJson,
      authorizationHeader, self.timeoutMsValue,
      XHAL_RPI5CAR_LANGUAGE_MODEL_OLLAMA_MAXIMUM_RESPONSE_BYTES);
  const hal::boolean responseJsonTooLarge = static_cast<hal::boolean>(
      responseJson.size() >
      XHAL_RPI5CAR_LANGUAGE_MODEL_OLLAMA_MAXIMUM_RESPONSE_BYTES);
  if (responseJsonTooLarge) {
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Ollama response exceeds its bounded byte count");
  }
  ...
  XWALK_HAL_TRACE_UID0(RPI .157,
                       "HTTP language-model request completed successfully");
...
void XWalkLanguageModelOllama::storeMessage(
  ...
  const hal::boolean historyLimitExceeded =
      static_cast<hal::boolean>(history.size() > maximumSize);
