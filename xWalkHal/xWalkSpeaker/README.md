...
├── test/src/xHal_Rpi5CarSpeakerTest.cpp
├── CMakeLists.txt
└── README.md
```
...
```bash
cmake -S xWalkHal/xWalkSpeaker -B xWalkHal/xWalkSpeaker/build-host -DXWALK_SPEAKER_BUILD_HOST_TESTS=ON
cmake --build xWalkHal/xWalkSpeaker/build-host --parallel
ctest --test-dir xWalkHal/xWalkSpeaker/build-host --output-on-failure
```
...
```bash
cmake -S xWalkHal/xWalkSpeaker -B xWalkHal/xWalkSpeaker/build-rpi -DXWALK_SPEAKER_BUILD_HARDWARE_TESTS=ON
cmake --build xWalkHal/xWalkSpeaker/build-rpi --parallel
ctest --test-dir xWalkHal/xWalkSpeaker/build-rpi -N -L hardware
```
