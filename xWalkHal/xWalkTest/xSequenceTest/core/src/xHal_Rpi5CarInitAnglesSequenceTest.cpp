...
void runTest(XWalkHal::stringview configurationPath)
{
    ...
    XWalkHal::XWalkBoardControl boardControl(resetGpio, speakerGpio,
        batteryAdc, nullptr, &unusedSpeakerPrime);
    XWalkHal::XWalkPwmTimerState timerState;
    XWalkHal::XWalkPwm firstPwm(
        i2c, 10U, XHAL_RPI5CAR_I2C_ADDRESS_1, timerState);
    XWalkHal::XWalkPwm secondPwm(
        i2c, 11U, XHAL_RPI5CAR_I2C_ADDRESS_1, timerState);
    XWalkHal::XWalkPwm thirdPwm(
        i2c, 12U, XHAL_RPI5CAR_I2C_ADDRESS_1, timerState);
    ...
    xwalk::hal::test::XWalkInitAnglesSequence sequence(boardControl, robot,
        firstServo, secondServo, thirdServo);
    sequence.run();
    ...
    assert(robot.servoPositions() ==
        XWalkHal::float64vector({10.0, 45.0, -45.0}));
