...
XWalkRobotHat5MotorSequence::XWalkRobotHat5MotorSequence(
    XWalkMotor &firstMotor, XWalkMotor &secondMotor, XWalkMotor &thirdMotor,
  ...
  if (waitCallback == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Motor-sequence wait callback must not be null");
  }
}
...
void XWalkRobotHat5MotorSequence::run(uint32 cycleCount) {
  if ((cycleCount == 0U) ||
    ...
    XWALK_HAL_ERROR(XWALK_RANGE,
                    "Robot HAT v5 motor cycles must be from 1 to 100");
  }
