...
XWalkHal::XWalkGpioCallbacks gpioCallbacks()
{
    return {&configureGpio, &readGpio, &writeGpio,
        &interruptGpio, &cancelInterruptGpio};
}
...
void runTest()
{
    ...
    xwalk::hal::test::XWalkMotorSequence sequence(
        firstMotor, secondMotor, &waitState, &wait);

    sequence.run(1U);

    assert(directionD4.pin == 23U);
    assert(directionD5.pin == 24U);
    assert(waitState.durations ==
        XWalkHal::uint32vector({1'000U, 1'000U, 100U}));
    assert(waitState.observedSpeeds == XWalkHal::float64vector({
        -50.0, -50.0, 50.0, 50.0, 0.0, 0.0}));
    assert(waitState.observedDirections == std::vector<XWalkHal::boolean>({
