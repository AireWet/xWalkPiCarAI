...
void XWalkVoiceAssistantExampleLinux::run(
    stringview apiKey, XWalkVoiceAssistantExampleInputMode inputMode,
    uint32 maximumRounds, uint32 timeoutMs, stringview microphoneDevice,
    stringview voskLibrary, stringview voskModelPath,
    stringview piperExecutable, stringview playbackExecutable,
    stringview cameraConnection, stringview captureExecutable,
  ...
  XWalkSpeechToTextAlsa speechBackend(microphoneDevice, &recognizer,
  ...
  XWalkLanguageModelHttp modelBackend(
      XWalkLanguageModelHttpDialect::OpenAiChatCompletions, endpoint, modelName,
      apiKey);
...
XWalkVoiceAssistantExampleLinux &
XWalkVoiceAssistantExampleLinux::adapter(contextpointer context) {
  if (context == nullptr) {
 ...
 */
void XWalkVoiceAssistantExampleLinux::configure(
  ...
  if ((configuration.name != "Buddy") ||
      ...
      (configuration.speechLanguage != "en-us") || !configuration.withImage ||
      !configuration.keyboardEnabled || !configuration.wakeEnabled) {
    XWALK_HAL_ERROR(XWALK_INVAL,
...
boolean XWalkVoiceAssistantExampleLinux::readKeyboard(contextpointer context,
                                                      string &inputText) {
...
boolean XWalkVoiceAssistantExampleLinux::capture(contextpointer context,
...
void XWalkVoiceAssistantExampleLinux::report(contextpointer context,
                                             stringview text) {
  static_cast<void>(adapter(context));
  std::cout << text << '\n';
}
