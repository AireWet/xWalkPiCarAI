...
XWalkServoExampleLinux& XWalkServoExampleLinux::adapter(contextpointer context)
{
    if (context == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Servo Linux context must not be null");
    }
    XWalkServoExampleLinux& self =
        *static_cast<XWalkServoExampleLinux*>(context);
    if (self.servoObject == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Servo Linux adapter has no bound servo");
    }
    return self;
}

/** @brief Commands one angle on the bound servo. */
void XWalkServoExampleLinux::setAngle(
    contextpointer context, float64 angleDegrees)
{
    adapter(context).servoObject->setAngle(angleDegrees);
}

/** @brief Waits for one source-compatible duration. */
void XWalkServoExampleLinux::wait(
...
void XWalkServoExampleLinux::report(
    contextpointer context, int32 angleDegrees)
{
    static_cast<void>(adapter(context));
