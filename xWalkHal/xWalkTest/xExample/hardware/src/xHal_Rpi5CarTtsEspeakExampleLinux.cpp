    ...
    executableName(executable)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Espeak playback executable is required");
    }
}
...
XWalkTtsEspeakExampleLinux& XWalkTtsEspeakExampleLinux::adapter(
    contextpointer context)
{
    if (context == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Espeak example Linux context must not be null");
    }
...
void XWalkTtsEspeakExampleLinux::speak(contextpointer context, uint8 amplitude,
    uint16 speed, uint16 gap, uint8 pitch, stringview text)
{
    ...
    if (childProcess < 0)
    {
        XHAL_THROW_RUNTIME_ERROR("Espeak example process creation failed");
    }
    if (childProcess == 0)
    {
        ::execlp(self.executableName.c_str(), self.executableName.c_str(),
            "-a", amplitudeText.c_str(), "-s", speedText.c_str(),
            "-g", gapText.c_str(), "-p", pitchText.c_str(), ownedText.c_str(),
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("Espeak playback executable failed");
    }
}
