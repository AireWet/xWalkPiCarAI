...
XWalkSttVoskStreamExampleLinux &
XWalkSttVoskStreamExampleLinux::adapter(contextpointer context) {
  if (context == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Streaming Vosk Linux context must not be null");
  }
  XWalkSttVoskStreamExampleLinux &self =
      *static_cast<XWalkSttVoskStreamExampleLinux *>(context);
  if (self.speechToTextObject == nullptr) {
    XWALK_HAL_ERROR(XWALK_INVAL,
                    "Streaming Vosk Linux adapter has no speech binding");
  }
  return self;
}

/** @brief Captures one utterance and emits the final-result boundary. */
void XWalkSttVoskStreamExampleLinux::listen(
