 ...
 * @brief       Implements Linux Vosk composition for the streaming speech example.
 *
 * @details
...
void XWalkSttVoskStreamExampleLinux::run(uint32 sessionCount, uint32 timeoutMs,
    stringview microphoneDevice, stringview libraryName, stringview modelPath)
{
    ...
    const XWalkSttVoskStreamExampleCallbacks exampleCallbacks{
        &listen, &reportPrompt, &reportResult};
...
XWalkSttVoskStreamExampleLinux& XWalkSttVoskStreamExampleLinux::adapter(
    contextpointer context)
{
    if (context == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Streaming Vosk Linux context must not be null");
    }
    XWalkSttVoskStreamExampleLinux& self =
        *static_cast<XWalkSttVoskStreamExampleLinux*>(context);
    if (self.speechToTextObject == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Streaming Vosk Linux adapter has no speech binding");
    }
    return self;
}
...
void XWalkSttVoskStreamExampleLinux::listen(contextpointer context,
    uint32 timeoutMs, sttvoskstreamresultcallback resultCallback)
{
...
void XWalkSttVoskStreamExampleLinux::reportResult(contextpointer context,
