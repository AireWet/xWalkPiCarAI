    ...
    callbackContext(context), callbacks(exampleCallbacks)
{
    if ((callbacks.startListening == nullptr) || (callbacks.isWaked == nullptr) ||
        (callbacks.stopListening == nullptr) || (callbacks.wait == nullptr) ||
        (callbacks.report == nullptr))
    {
        XHAL_THROW_INVALID_ARGUMENT(
            "Threaded Vosk wake-word example requires complete callbacks");
    }
}

/** @brief Runs each bounded source-compatible wake-word detection attempt. */
void XWalkSttVoskWakeWordThreadExample::run(
    uint32 detectionCount, uint32 maximumPolls)
{
    if ((detectionCount == 0U) ||
        (detectionCount > XHAL_RPI5CAR_STT_VOSK_WAKE_WORD_MAXIMUM_DETECTIONS))
    {
        XHAL_THROW_OUT_OF_RANGE("Wake-word detection count is outside its range");
    }
    if ((maximumPolls == 0U) ||
        (maximumPolls > XHAL_RPI5CAR_STT_VOSK_WAKE_WORD_MAXIMUM_POLLS))
    {
        XHAL_THROW_OUT_OF_RANGE("Wake-word poll count is outside its range");
    }
    ...
    for (uint32 detectionIndex = 0U; detectionIndex < detectionCount; ++detectionIndex)
    {
        ...
        if (!detected)
        {
            XHAL_THROW_RUNTIME_ERROR("Wake word was not detected before the poll limit");
        }
