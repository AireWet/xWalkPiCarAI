...
void XWalkSttVoskWakeWordThreadExample::run(uint32 detectionCount,
  ...
  for (uint32 detectionIndex = 0U; detectionIndex < detectionCount;
       ++detectionIndex) {
    ...
    for (uint32 pollIndex = 0U; pollIndex < maximumPolls; ++pollIndex) {
      const hal::boolean wakedMatched =
          static_cast<hal::boolean>(callbacks.isWaked(callbackContext));
      if (wakedMatched) {
        detected = true;
        break;
      }
    ...
    if (!detected) {
      XWALK_HAL_ERROR(XWALK_RUNTIME,
                      "Wake word was not detected before the poll limit");
    }
