...
{
    static_cast<GrokExampleState*>(context)->instructions =
        XWalkHal::string(instructions);
}
...
void testConversation()
{
    ...
    xwalk::hal::example::XWalkGrokExample example(
...
void testValidation()
{
    GrokExampleState state;
    XWalkHal::XWalkLanguageModel model(&state, modelCallbacks());
    xwalk::hal::example::XWalkGrokExampleCallbacks incompleteCallbacks =
        consoleCallbacks();
    incompleteCallbacks.write = nullptr;
    XWalkHal::boolean rejectedCallbacks = false;
    try
    {
        xwalk::hal::example::XWalkGrokExample invalidExample(
