...
void blink(XWalkHal::contextpointer context, XWalkHal::uint32 cycleCount,
    XWalkHal::float64 toggleDelaySeconds, XWalkHal::float64 pauseSeconds)
{
...
void testNormalFlow()
{
    ...
    const XWalkHal::stringvector expectedOperations{
        "on", "off", "blink", "blink", "blink", "close"};
    const XWalkHal::stringvector expectedMessages{
        ...
        "Done"};
    const XWalkHal::uint32vector expectedWaits{
        2'000U, 2'000U, 5'000U, 5'000U, 5'000U};
...
void testValidationAndCleanup()
{
    ...
    try
    {
        xwalk::hal::example::XWalkLedExample invalidExample(
            &state, incompleteCallbacks);
    }
