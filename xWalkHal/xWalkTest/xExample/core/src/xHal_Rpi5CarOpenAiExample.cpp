...
XWalkOpenAiExample::XWalkOpenAiExample(XWalkLanguageModel& languageModel,
    contextpointer context, const XWalkOpenAiExampleCallbacks& consoleCallbacks):
    ...
    callbacks(consoleCallbacks)
{
    if ((callbacks.readPrompt == nullptr) || (callbacks.write == nullptr))
    {
        XHAL_THROW_INVALID_ARGUMENT(
            "OpenAI example requires complete console callbacks");
    }
}
...
void XWalkOpenAiExample::run(uint32 maximumPrompts)
{
        ...
        (maximumPrompts > XHAL_RPI5CAR_OPEN_AI_EXAMPLE_MAXIMUM_PROMPTS))
    {
        XHAL_THROW_OUT_OF_RANGE(
            "OpenAI example prompt count is outside its range");
    }
