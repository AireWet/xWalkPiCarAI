    ...
    consoleContext(context), callbacks(consoleCallbacks)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Doubao image example requires complete console callbacks");
    }
}
...
void XWalkDoubaoImageExample::run(uint32 maximumPrompts, stringview imagePath)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("Doubao image example prompt count is outside its range");
    }
