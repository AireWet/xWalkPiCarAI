...
XWalkVoiceAssistantExample::XWalkVoiceAssistantExample(
    contextpointer context,
    const XWalkVoiceAssistantExampleCallbacks &exampleCallbacks)
    : callbackContext(context), callbacks(exampleCallbacks) {
  if ((callbacks.configure == nullptr) || (callbacks.readKeyboard == nullptr) ||
      (callbacks.listen == nullptr) || (callbacks.capture == nullptr) ||
      (callbacks.prompt == nullptr) || (callbacks.speak == nullptr) ||
...
boolean XWalkVoiceAssistantExample::acquirePrompt(
  ...
  const string wakeText = callbacks.listen(callbackContext, timeoutMs,
                                           configurationValue.speechLanguage);
  if (wakeText != configurationValue.wakeWord) {
    return false;
  }
  const hal::boolean answerOnWakeAvailable =
      static_cast<hal::boolean>(!configurationValue.answerOnWake.empty());
  if (answerOnWakeAvailable) {
    callbacks.speak(callbackContext, configurationValue.ttsModel,
                    configurationValue.answerOnWake);
  }
  promptText = callbacks.listen(callbackContext, timeoutMs,
                                configurationValue.speechLanguage);
...
void XWalkVoiceAssistantExample::run(
  ...
  for (uint32 roundIndex = 0U; roundIndex < maximumRounds; ++roundIndex) {
    ...
    const string response =
        callbacks.prompt(callbackContext, configurationValue.languageModel,
                         promptText, selectedImagePath);
