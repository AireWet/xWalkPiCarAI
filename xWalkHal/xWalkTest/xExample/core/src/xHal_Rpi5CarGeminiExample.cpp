...
XWalkGeminiExample::XWalkGeminiExample(XWalkLanguageModel& languageModel,
    contextpointer context, const XWalkGeminiExampleCallbacks& consoleCallbacks):
    languageModelObject(&languageModel), consoleContext(context),
    callbacks(consoleCallbacks)
{
    if ((callbacks.readPrompt == nullptr) || (callbacks.write == nullptr))
    {
        XHAL_THROW_INVALID_ARGUMENT("Gemini example requires complete console callbacks");
    }
}
...
void XWalkGeminiExample::run(uint32 maximumPrompts)
{
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("Gemini example prompt count is outside its range");
    }
