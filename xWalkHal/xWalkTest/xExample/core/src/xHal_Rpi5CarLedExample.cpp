 ...
 */
XWalkLedExample::XWalkLedExample(contextpointer context,
    const XWalkLedExampleCallbacks& exampleCallbacks):
    callbackContext(context), callbacks(exampleCallbacks)
{
    if ((callbacks.on == nullptr) || (callbacks.off == nullptr) ||
        (callbacks.blink == nullptr) || (callbacks.close == nullptr) ||
        (callbacks.wait == nullptr) || (callbacks.report == nullptr))
    {
        XHAL_THROW_INVALID_ARGUMENT("LED example requires a complete callback table");
    }
}
