 ...
 */
int32 XWalkSpiLinux::openValidatedDevice(cstring devicePath,
    const XWalkSpiConfiguration& configuration)
{
    if ((devicePath == nullptr) || (devicePath[0U] == '\0'))
    {
        XHAL_THROW_INVALID_ARGUMENT("SPI device path must not be empty");
    }
    if (configuration.speedHz == 0U)
    {
        XHAL_THROW_INVALID_ARGUMENT("SPI speed must be greater than zero");
    }
    if (configuration.mode > XHAL_RPI5CAR_SPI_MAXIMUM_MODE)
    {
        XHAL_THROW_OUT_OF_RANGE("SPI mode must be between zero and three");
    }
    if ((configuration.bitsPerWord == 0U) ||
        (configuration.bitsPerWord > XHAL_RPI5CAR_SPI_MAXIMUM_BITS_PER_WORD))
    {
        XHAL_THROW_OUT_OF_RANGE("SPI bits per word must be between one and thirty-two");
    }
    const int32 descriptor = ::open(devicePath, O_RDWR | O_CLOEXEC);
    if (descriptor < 0)
    {
        XHAL_THROW_RUNTIME_ERROR("Unable to open Linux SPI device");
    }
    return descriptor;
}
...
void XWalkSpiLinux::configure(const XWalkSpiConfiguration& configuration)
{
        ...
        (::ioctl(fileDescriptor, SPI_IOC_WR_MAX_SPEED_HZ, &speedHz) < 0))
    {
        ...
        fileDescriptor = -1;
        XHAL_THROW_RUNTIME_ERROR("Unable to configure Linux SPI device");
    }
    modeValue = mode;
