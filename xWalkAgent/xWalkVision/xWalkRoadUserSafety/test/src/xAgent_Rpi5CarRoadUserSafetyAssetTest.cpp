...
namespace
{
constexpr std::array<const char*, 12U> SCENARIOS{
    "pedestrian_entering_crosswalk", "pedestrian_standing_safely",
    "vehicle_approaching_pedestrian", "bicycle_crossing", "multiple_road_users",
    "partial_occlusion", "poor_lighting", "motion_blur", "empty_road",
    "false_positive_challenge", "camera_interruption", "end_of_video"};
...
TEST(XWalkRoadUserSafetyAssets, AllReviewedScenariosDecodeDeterministically)
{
    ...
    {
        const filesystempath path = assetRoot() / "recorded_scenarios" /
            scenario / "scenario.avi";
...
TEST(XWalkRoadUserSafetyAssets, MalformedImagesFailWithoutProducingFrames)
{
    ...
    {
        const RecordedAssetSummary summary =
            decodeRecordedAsset(assetRoot() / "malformed_images" / filename);
