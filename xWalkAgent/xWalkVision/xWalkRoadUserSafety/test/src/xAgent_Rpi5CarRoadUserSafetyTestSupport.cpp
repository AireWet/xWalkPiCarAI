...
XWalkRoadRiskClassification classifyFeatures(
    const XWalkRoadRiskFeatures& features) noexcept
{
    const boolean immediatePathRisk = (features.absoluteLateralOffset <= 0.35) &&
    ...
    const boolean warning = (features.confidence < 0.5) ||
        ((features.absoluteLateralOffset <= 0.5) &&
            (features.distanceMeters <= 6.0) &&
            (features.closingSpeedMetersPerSecond > 0.5));
    return {XWalkRoadSafetyStatus::Ok,
        warning ? XWalkRoadRisk::Warning : XWalkRoadRisk::Safe};
}
...
uint32 recordedFrameIndex(const cv::Mat& frame) noexcept
{
    ...
    if (marker[0] < 40.0) { return 0U; }
    if (marker[0] < 80.0) { return 1U; }
    if (marker[0] < 120.0) { return 2U; }
    return 3U;
}
...
{
    cv::Mat lineMask;
    cv::inRange(frame.row(frame.rows - 1), cv::Scalar(180.0, 180.0, 180.0),
        cv::Scalar(255.0, 255.0, 255.0), lineMask);
    return cv::countNonZero(lineMask) >= (frame.cols / 2);
}

/** @brief Extracts one person detection from the generated red rectangle. */
boolean recordedPersonDetection(RecordedScenarioBackend& scenario,
    const cv::Mat& frame, XWalkRoadUserDetection& output)
{
    cv::Mat personMask;
    cv::inRange(frame, cv::Scalar(0.0, 0.0, 150.0),
        cv::Scalar(90.0, 90.0, 255.0), personMask);
    std::vector<std::vector<cv::Point>> contours;
    cv::findContours(personMask, contours, cv::RETR_EXTERNAL,
    ...
    const float64 boundingArea =
        static_cast<float64>(bounds.width * bounds.height) / frameArea;
    ...
    const float64 centerX = static_cast<float64>(bounds.x) +
        (static_cast<float64>(bounds.width) / 2.0);
    const float64 lateralOffset =
        (centerX - (static_cast<float64>(frame.cols) / 2.0)) /
        (static_cast<float64>(frame.cols) / 2.0);
    output = detection(XWalkRoadUserClass::Person, 0.95, distance,
        closingSpeed, lateralOffset, boundingArea);
    return true;
}
...
XWalkRoadUserDetectionBatch detectRecordedScenario(contextpointer context) noexcept
{
    RecordedScenarioBackend& scenario =
        *static_cast<RecordedScenarioBackend*>(context);
    cv::Mat frame;
...
void alertRecordedScenario(contextpointer context, XWalkRoadRisk risk,
    uint32 detectionCount) noexcept
{
    RecordedScenarioBackend& scenario =
        *static_cast<RecordedScenarioBackend*>(context);
    ++scenario.alertCount;
...
void faultRecordedScenario(contextpointer context,
    XWalkRoadSafetyStatus status) noexcept
{
    RecordedScenarioBackend& scenario =
        *static_cast<RecordedScenarioBackend*>(context);
...
XWalkRoadRiskClassification classify(contextpointer context,
    const XWalkRoadRiskFeatures& features) noexcept
{
    ScenarioBackend& value = backend(context);
    ++value.classifyCount;
...
XWalkRoadUserDetection detection(XWalkRoadUserClass roadUserClass,
    float64 confidence, float64 distanceMeters, float64 closingSpeedMetersPerSecond,
    float64 lateralOffsetNormalized, float64 boundingAreaNormalized)
{
    return {static_cast<int32>(roadUserClass), confidence, distanceMeters,
        closingSpeedMetersPerSecond, lateralOffsetNormalized, boundingAreaNormalized};
}
...
boolean createRecordedScenario(RecordedScenarioBackend& scenario) noexcept
{
    ...
    const filesystempath temporaryDirectory =
        std::filesystem::temp_directory_path(error);
    if (error)
    {
        return false;
    }
    scenario.directory = temporaryDirectory /
        (string("xwalk-road-scenario-") + std::to_string(timestamp));
    ...
    for (uint32 index = 0U; index < RECORDED_FRAME_COUNT; ++index)
    {
        ...
        cv::Mat frame(RECORDED_FRAME_HEIGHT, RECORDED_FRAME_WIDTH,
            CV_8UC3, cv::Scalar(background, background, background));
        if (index != 3U)
        {
            cv::rectangle(frame, cv::Rect(0, RECORDED_FRAME_HEIGHT - 2,
                RECORDED_FRAME_WIDTH, 2), cv::Scalar(255.0, 255.0, 255.0), cv::FILLED);
        }
        if (index == 1U)
        {
            cv::rectangle(frame, cv::Rect(27, 14, 10, 12),
                cv::Scalar(0.0, 0.0, 255.0), cv::FILLED);
        }
        else if (index == 2U)
        {
            cv::rectangle(frame, cv::Rect(15, 3, 34, 42),
                cv::Scalar(0.0, 0.0, 255.0), cv::FILLED);
        }
        writer.write(frame);
    }
...
{
    return {&detectRecordedScenario, &classifyRecordedScenario,
        &alertRecordedScenario, &stopRecordedScenario, &faultRecordedScenario};
}
