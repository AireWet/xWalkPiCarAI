...
agent::float64 XWalkFaceTracking::constrainAngle(
...
XWalkFaceTrackingResult XWalkFaceTracking::step()
{
    if (!startedValue)
    {
        XHAL_THROW_LOGIC_ERROR("Face tracking must be started before stepping");
    }
    ...
    if (result.face.count > 0U)
    {
        ...
        const agent::float64 horizontalCorrection =
            ...
            halfSpan;
        const agent::float64 verticalCorrection = halfSpan -
            (static_cast<agent::float64>(result.face.centerY) *
                configurationValue.correctionSpanDegrees /
                static_cast<agent::float64>(configurationValue.frameHeightPixels));
        panAngleDegreesValue = constrainAngle(
            panAngleDegreesValue + horizontalCorrection);
        tiltAngleDegreesValue = constrainAngle(
            tiltAngleDegreesValue + verticalCorrection);
