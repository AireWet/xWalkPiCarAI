 ...
 * @brief       Implements treasure-hunt construction, validation, and lifecycle.
 *
...
/**
 * @brief Binds caller-owned vehicle, speech, vision, and target-selection services.
...
/** @brief Stops active vision and requests non-throwing fail-safe vehicle shutdown. */
 ...
 * @return `true` after complete startup; otherwise `false` after cancellation or provider failure.
 */
...
/** @brief Reports whether the vision provider is active. @return Current provider state. */
agent::boolean XWalkTreasureHunt::started() const noexcept
{
    return startedValue;
}

/** @brief Returns the currently selected target color. @return Active target color. */
 ...
 * @throws std::invalid_argument If callbacks or finite numeric values are invalid.
 * @throws std::out_of_range If a bounded setting is outside its supported range.
 */
void XWalkTreasureHunt::validate(
    const XWalkTreasureHuntCallbacks& backendCallbacks,
    const XWalkTreasureHuntConfiguration& configuration)
{
    const XWalkComputerVisionCallbacks& vision = backendCallbacks.vision;
    if ((vision.start == nullptr) || (vision.stop == nullptr) ||
        (vision.setColor == nullptr) || (vision.observe == nullptr) ||
        (vision.delay == nullptr) || (vision.continueOperation == nullptr) ||
        (backendCallbacks.selectColor == nullptr))
    {
        XHAL_THROW_INVALID_ARGUMENT("Treasure hunt requires complete callbacks");
    }
    const agent::boolean configurationInvalid =
        static_cast<agent::boolean>(
            !XHAL_IS_FINITE(configuration.driveSpeedPercent) ||
        !XHAL_IS_FINITE(configuration.turnAngleDegrees));
    if (configurationInvalid)
    {
        XHAL_THROW_INVALID_ARGUMENT("Treasure-hunt numeric settings must be finite");
    }
    if ((configuration.detectionWidthThresholdPixels == 0U) ||
        (configuration.detectionWidthThresholdPixels > 7'680U) ||
        (configuration.driveSpeedPercent < 0.0) ||
        (configuration.driveSpeedPercent > 100.0) ||
        (configuration.turnAngleDegrees <= 0.0) ||
        (configuration.turnAngleDegrees > 45.0) ||
        (configuration.startupDelayMs > 10'000U) ||
        (configuration.promptDelayMs > 1'000U) ||
        (configuration.movementDelayMs == 0U) ||
        (configuration.movementDelayMs > 10'000U) ||
        (configuration.loopDelayMs == 0U) ||
        (configuration.loopDelayMs > 1'000U) ||
        (configuration.finalDelayMs > 1'000U))
    {
        XHAL_THROW_OUT_OF_RANGE("Treasure-hunt configuration is out of range");
    }
}
 ...
 * @return `true` after the complete delay; otherwise `false` after cancellation.
 */
 ...
 * @throws std::invalid_argument If the selection callback returns `Close` or an invalid color.
 */
