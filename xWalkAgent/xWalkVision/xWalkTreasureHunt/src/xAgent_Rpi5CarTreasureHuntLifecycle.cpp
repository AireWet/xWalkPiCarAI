 ...
 */
XWalkTreasureHunt::XWalkTreasureHunt(
...
agent::boolean XWalkTreasureHunt::start() {
  ...
  const agent::boolean startupDelayCompleted =
      wait(configurationValue.startupDelayMs);
  ...
  const agent::boolean promptDelayCompleted =
      wait(configurationValue.promptDelayMs);
...
void XWalkTreasureHunt::validate(
  ...
  if ((vision.start == nullptr) || (vision.stop == nullptr) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Treasure hunt requires complete callbacks");
  }
  ...
  if (configurationInvalid) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Treasure-hunt numeric settings must be finite");
  }
  if ((configuration.detectionWidthThresholdPixels == 0U) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "Treasure-hunt configuration is out of range");
  }
}
