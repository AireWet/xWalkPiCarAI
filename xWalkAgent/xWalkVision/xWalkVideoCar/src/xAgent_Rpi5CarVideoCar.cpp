...
XWalkVideoCarResult XWalkVideoCar::result(XWalkVideoCarEvent event,
...
XWalkVideoCarResult XWalkVideoCar::handleKey(const agent::string& key)
{
    if (!startedValue)
    {
        XHAL_THROW_LOGIC_ERROR("Video car must be started before handling keys");
    }
    ...
    const char command = (key.size() == 1U) ?
        static_cast<char>(std::tolower(static_cast<unsigned char>(key[0]))) : '\0';
    switch (command)
    {
        case 'o':
            if (speedPercentValue <=
                (configurationValue.maximumSpeedPercent -
