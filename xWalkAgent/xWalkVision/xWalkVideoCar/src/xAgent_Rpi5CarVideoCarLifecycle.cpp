    ...
    const XWalkVideoCarConfiguration& configuration)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Video car requires complete callbacks");
    }
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Video-car steering angle must be finite");
    }
    if ((configuration.speedStepPercent == 0U) ||
        ...
        (configuration.keyDelayMs == 0U) ||
        (configuration.keyDelayMs > 1'000U))
    {
        XHAL_THROW_OUT_OF_RANGE("Video-car configuration is out of range");
    }
}
