...
void XWalkBullFight::validate(
  ...
  const agent::boolean configurationInvalid = static_cast<agent::boolean>(
      ...
      !std::isfinite(configuration.maximumCameraAngleDegrees) ||
      !std::isfinite(configuration.speedPercent));
  if (configurationInvalid) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Bull-fight numeric settings must be finite");
  }
  if ((configuration.frameWidthPixels < 16U) ||
      ...
      (configuration.maximumCameraAngleDegrees <= 0.0) ||
      (configuration.maximumCameraAngleDegrees > 90.0) ||
      (configuration.speedPercent < 0.0) ||
      (configuration.speedPercent > 100.0) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "Bull-fight configuration is out of range");
  }
}
...
agent::boolean XWalkBullFight::started() const noexcept { return startedValue; }
