...
agent::float64 XWalkBullFight::constrainCameraAngle(
    agent::float64 angleDegrees) const noexcept
{
...
XWalkBullFightResult XWalkBullFight::step()
{
    if (!startedValue)
    {
        XHAL_THROW_LOGIC_ERROR("Bull fight must be started before stepping");
    }
    ...
    if (result.target.count > 0U)
    {
        ...
        panAngleDegreesValue = constrainCameraAngle(panAngleDegreesValue +
            (static_cast<agent::float64>(result.target.centerX) *
                configurationValue.correctionSpanDegrees /
                static_cast<agent::float64>(configurationValue.frameWidthPixels)) -
            halfSpan);
        tiltAngleDegreesValue = constrainCameraAngle(tiltAngleDegreesValue +
            halfSpan - (static_cast<agent::float64>(result.target.centerY) *
                configurationValue.correctionSpanDegrees /
