...
void XWalkComputerVision::pollQr(XWalkComputerVisionResult& result)
{
    ...
    const agent::boolean newQrCodeDetected =
        static_cast<agent::boolean>(
            !qrData.empty() && (qrData != lastQrData));
...
XWalkComputerVisionResult XWalkComputerVision::handleKey(agent::stringview keyText)
{
    if (!startedValue)
    {
        XHAL_THROW_LOGIC_ERROR("Computer vision must be started before handling keys");
    }
...
agent::string XWalkComputerVision::colorName(XWalkComputerVisionColor color)
{
    switch (color)
    {
        ...
        case XWalkComputerVisionColor::Purple:
            return "purple";
        default:
            XHAL_THROW_INVALID_ARGUMENT("Computer-vision color is invalid");
    }
}
