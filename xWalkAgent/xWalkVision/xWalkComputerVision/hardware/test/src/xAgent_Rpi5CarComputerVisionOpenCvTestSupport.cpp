...
RecordedVideoFixture::RecordedVideoFixture(agent::boolean colorSequence) {
  ...
  directory = agent::filesystempath("/tmp") /
              (agent::string("xwalk-opencv-") + std::to_string(stamp));
  ...
  if (!writer.isOpened()) {
    XWALK_RPIAGENT_ERROR(
        XWALK_RUNTIME, "Recorded-video test fixture could not open its writer");
  }
  if (colorSequence) {
    const cv::Scalar colors[]{
        {0.0, 0.0, 255.0}, {0.0, 128.0, 255.0}, {0.0, 255.0, 255.0},
        {0.0, 255.0, 0.0}, {255.0, 0.0, 0.0}, {255.0, 0.0, 255.0}};
