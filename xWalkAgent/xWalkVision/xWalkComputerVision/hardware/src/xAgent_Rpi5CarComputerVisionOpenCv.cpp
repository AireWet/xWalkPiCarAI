...
XWalkComputerVisionOpenCv::XWalkComputerVisionOpenCv(
  ...
  if (configurationInvalid) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Computer-vision paths must not be empty");
  }
  ...
  const agent::boolean cameraConfigurationInvalid = static_cast<agent::boolean>(
      ...
      (!configurationValue.faceCascadePath.empty() &&
       !agent::filesystempath(configurationValue.faceCascadePath)
            .is_absolute()));
  ...
  if ((configurationValue.widthPixels < 16U) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "Computer-vision dimensions are outside their range");
  }
  if (!configurationValue.faceCascadePath.empty() &&
    ...
    XWALK_RPIAGENT_ERROR(XWALK_RUNTIME,
                         "Computer-vision face cascade could not be loaded");
  }
}

XWalkComputerVisionOpenCvBackend
XWalkComputerVisionOpenCv::backendFromString(agent::stringview backend) {
  if (backend == "v4l2") {
  ...
  XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                       "computer_vision_camera_backend is unsupported");
}
...
XWalkComputerVisionOpenCv::provider(agent::contextpointer context) {
  if (context == nullptr) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Computer-vision provider context must not be null");
  }
 ...
 */
agent::boolean
XWalkComputerVisionOpenCv::startProvider(agent::contextpointer context) {
 ...
 */
void XWalkComputerVisionOpenCv::stopProvider(
...
cv::Mat XWalkComputerVisionOpenCv::readFrame() {
  ...
  if (frameUnavailable) {
    ...
    if (finiteSource) {
      XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                           "Computer-vision source reached end of sequence");
    }
    XWALK_RPIAGENT_ERROR(XWALK_RUNTIME,
                         "Computer-vision camera frame acquisition failed");
  }
  return frame;
}
 ...
 */
agent::string
XWalkComputerVisionOpenCv::capturePhoto(agent::contextpointer context) {
  XWalkComputerVisionOpenCv &backend = provider(context);
  const agent::filesystempath directory{
      backend.configurationValue.photoDirectory};
  ...
  if (currentTimeLocalTimeMatched) {
    XWALK_RPIAGENT_ERROR(XWALK_RUNTIME,
                         "Computer-vision photo timestamp failed");
  }
  ...
  if (timestampMDMatched) {
    XWALK_RPIAGENT_ERROR(XWALK_RUNTIME,
                         "Computer-vision photo timestamp formatting failed");
  }
  const agent::filesystempath outputPath =
      directory / (agent::string("photo_") + timestamp.data() + ".jpg");
  const agent::boolean photographWritten =
      cv::imwrite(outputPath.string(), backend.readFrame());
  if (photographWritten == false) {
    XWALK_RPIAGENT_ERROR(XWALK_RUNTIME,
                         "Computer-vision photograph could not be written");
  }
...
void XWalkComputerVisionOpenCv::switchFace(agent::contextpointer context,
  ...
  if (enabled && backend.faceCascade.empty()) {
    XWALK_RPIAGENT_ERROR(
        XWALK_LOGIC,
        "Computer-vision face detection requires a configured cascade");
  }
 ...
 */
XWalkComputerVisionDetection
XWalkComputerVisionOpenCv::detectColor(const cv::Mat &frame) const {
  ...
  for (const computervisionpointvector &contour : contours) {
    ...
    if (area > largestArea) {
      ...
      result.centerY =
          static_cast<agent::int32>(bounds.y + (bounds.height / 2));
 ...
 */
XWalkComputerVisionDetection
XWalkComputerVisionOpenCv::detectFace(const cv::Mat &frame) {
  cv::Mat grayscale;
 ...
 */
XWalkComputerVisionObservation
XWalkComputerVisionOpenCv::observeFrame(agent::contextpointer context) {
  XWalkComputerVisionOpenCv &backend = provider(context);
 ...
 */
XWalkComputerVisionCallbacks
XWalkComputerVisionOpenCv::callbacks() const noexcept {
  return {&startProvider, &stopProvider, &capturePhoto,
          &selectColor,   &switchFace,   &switchQr,
          &observeFrame,  nullptr,       nullptr};
}
