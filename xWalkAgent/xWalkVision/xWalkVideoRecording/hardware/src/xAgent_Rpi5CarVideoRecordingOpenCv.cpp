    ...
    configurationValue(configuration)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Video-recording paths must not be empty");
    }
    const agent::boolean videoPathConfigurationInvalid =
        static_cast<agent::boolean>(
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Video-recording paths must be absolute");
    }
    const agent::boolean videoFormatInvalid =
        static_cast<agent::boolean>(
    ...
    {
        XHAL_THROW_OUT_OF_RANGE("Video-recording configuration is out of range");
    }
}
...
XWalkVideoRecordingOpenCv& XWalkVideoRecordingOpenCv::provider(
    agent::contextpointer context)
{
    if (context == nullptr)
    {
        XHAL_THROW_INVALID_ARGUMENT("Video-recording provider context is null");
    }
    return *static_cast<XWalkVideoRecordingOpenCv*>(context);
}

agent::boolean XWalkVideoRecordingOpenCv::startCamera(
    agent::contextpointer context)
{
    ...
    backend.frameWidthPixelsValue = static_cast<agent::uint32>(std::lround(
        backend.camera.get(cv::CAP_PROP_FRAME_WIDTH)));
    backend.frameHeightPixelsValue = static_cast<agent::uint32>(std::lround(
        backend.camera.get(cv::CAP_PROP_FRAME_HEIGHT)));
    ...
    try
    {
        backend.captureThread = std::thread(
            &XWalkVideoRecordingOpenCv::captureLoop, &backend);
    }
    catch (...)
    {
        backend.camera.release();
...
agent::string XWalkVideoRecordingOpenCv::beginRecording(
    agent::contextpointer context, agent::stringview recordingName)
{
    XWalkVideoRecordingOpenCv& backend = provider(context);
    const agent::boolean recordingNameBackendWorkerFailedInvalid =
        static_cast<agent::boolean>(
            recordingName.empty() || backend.workerFailed.load());
    if (recordingNameBackendWorkerFailedInvalid)
    {
        XHAL_THROW_RUNTIME_ERROR("Video-recording provider is unavailable");
    }
    ...
    if (directoryExists == false)
    {
        ...
        {
            XHAL_THROW_RUNTIME_ERROR("Video directory could not be created");
        }
    }
    const agent::filesystempath path = directory /
        (agent::string(recordingName) + ".avi");
    std::lock_guard<std::mutex> lock(backend.stateMutex);
    const agent::boolean backendCaptureThreadJoinableInvalid =
        static_cast<agent::boolean>(
            !backend.captureThread.joinable() || backend.writer.isOpened());
    if (backendCaptureThreadJoinableInvalid)
    {
        XHAL_THROW_LOGIC_ERROR("Video recorder is not ready to start");
    }
    const int codec = cv::VideoWriter::fourcc('M', 'J', 'P', 'G');
    const agent::boolean writerOpened = backend.writer.open(path.string(), codec,
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("AVI output could not be opened");
    }
...
void XWalkVideoRecordingOpenCv::pauseRecording(agent::contextpointer context)
{
    ...
    {
        XHAL_THROW_LOGIC_ERROR("Video recording is not active");
    }
...
void XWalkVideoRecordingOpenCv::stopRecording(agent::contextpointer context)
{
    XWalkVideoRecordingOpenCv& backend = provider(context);
    std::lock_guard<std::mutex> lock(backend.stateMutex);
...
agent::string XWalkVideoRecordingOpenCv::timestamp(agent::contextpointer context)
{
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("Video timestamp failed");
    }
    agent::fixedarray<char, 32U> value{};
    const agent::boolean valueMDMatched =
        static_cast<agent::boolean>(
            ::strftime(value.data(), value.size(), "%Y-%m-%d-%H.%M.%S",
            &localTime) == 0U);
    if (valueMDMatched)
    {
        XHAL_THROW_RUNTIME_ERROR("Video timestamp formatting failed");
    }
    return value.data();
}
...
XWalkVideoRecordingCallbacks XWalkVideoRecordingOpenCv::callbacks() const noexcept
{
    return {&startCamera, &stopCamera, &beginRecording, &pauseRecording,
        &continueRecording, &stopRecording, nullptr, nullptr, &timestamp};
}
