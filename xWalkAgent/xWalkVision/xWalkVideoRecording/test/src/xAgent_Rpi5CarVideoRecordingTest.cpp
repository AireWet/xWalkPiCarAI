...
agent::string beginRecording(agent::contextpointer context,
...
xwalk::agent::XWalkVideoRecordingCallbacks callbacks()
{
    return {&startCamera, &stopCamera, &beginRecording, &pauseRecording,
        &continueRecording, &stopRecording, &delay, &continueOperation,
        &timestamp};
}
...
void testRecording()
{
    ...
    assert(result.videoPath ==
        "/tmp/xwalk-videos/2026-08-05-12.30.45.avi");
...
void testValidationAndCancellation()
{
    TestState state;
    xwalk::agent::XWalkVideoRecording recording(&state, callbacks());
    xwalk::hal::test::expectFailure([&recording]()
    {
    ...
    xwalk::hal::test::expectFailure([&state, &invalid]()
    {
