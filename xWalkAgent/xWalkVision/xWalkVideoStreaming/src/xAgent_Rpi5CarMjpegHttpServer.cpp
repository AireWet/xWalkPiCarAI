...
agent::boolean setOutput(XWalkMjpegHttpClient& client, agent::stringview output,
    agent::size maximumBytes, agent::uint64 nowMilliseconds) noexcept
{
...
agent::string statusBody(const XWalkMjpegHttpServer& server)
{
    ...
    if (server.stream != nullptr)
    {
        ...
        {
            if (std::numeric_limits<agent::uint64>::max() - droppedFrames <
    ...
    if (server.configuration.observability != nullptr)
    {
        ...
        safeStateTransitions = snapshot.counters[static_cast<agent::size>(
            ::xwalk::XWalkFailureEventId::SafeStateTransition)];
    }
...
agent::string httpResponse(int status, agent::stringview reason,
    agent::stringview contentType, agent::stringview body)
{
    return "HTTP/1.1 " + std::to_string(status) + " " + agent::string(reason) +
        "\r\nConnection: close\r\nCache-Control: no-store\r\nContent-Type: " +
        agent::string(contentType) + "\r\nContent-Length: " +
...
agent::stringview bearerToken(agent::stringview request) noexcept
{
    ...
    return request.substr(position, end == agent::stringview::npos ?
        request.size() - position : end - position);
}

/** @brief Authenticates external requests without retaining the credential. */
agent::boolean requestAuthorized(const XWalkMjpegHttpServer& server,
    agent::stringview request) noexcept
{
    ...
    return server.configuration.authenticate != nullptr &&
        server.configuration.authenticate(server.configuration.authenticationContext,
            server.configuration.authenticationReference, bearerToken(request));
}

/** @brief Parses and routes one complete request into bounded output state. */
agent::boolean routeRequest(XWalkMjpegHttpServer& server,
    XWalkMjpegHttpClient& client, agent::uint64 nowMilliseconds) noexcept
{
    XWalkMjpegHttpRequest parsed;
    if (parseMjpegHttpRequest(client.request,
            server.configuration.maximumRequestBytes, parsed) !=
    ...
    if (!requestAuthorized(server, client.request))
    {
        ...
        return setOutput(client, httpResponse(401, "Unauthorized", "text/plain",
    ...
    if (parsed.path == "/health")
    {
        ...
        client.mode = XWalkMjpegHttpClientMode::OneShotResponse;
        return setOutput(client, httpResponse(healthy ? 200 : 503,
            healthy ? "OK" : "Service Unavailable", "application/json",
            healthy ? "{\"healthy\":true}\n" : "{\"healthy\":false}\n"),
            server.configuration.maximumPendingBytes, nowMilliseconds);
    }
    if (parsed.path == "/status")
    {
        client.mode = XWalkMjpegHttpClientMode::OneShotResponse;
        return setOutput(client, httpResponse(200, "OK", "application/json",
            statusBody(server)), server.configuration.maximumPendingBytes,
            nowMilliseconds);
    }
    if (parsed.path == "/stream")
    {
        if (server.stream == nullptr || addMjpegStreamClient(*server.stream,
            client.identifier) != XWalkMjpegStreamStatus::Ok)
        {
            client.mode = XWalkMjpegHttpClientMode::OneShotResponse;
            return setOutput(client, httpResponse(503, "Service Unavailable", "text/plain",
                "stream unavailable\n"), server.configuration.maximumPendingBytes,
                nowMilliseconds);
        }
        client.mode = XWalkMjpegHttpClientMode::Stream;
        return setOutput(client, mjpegHttpResponseHeader(),
            server.configuration.maximumPendingBytes, nowMilliseconds);
    }
    client.mode = XWalkMjpegHttpClientMode::OneShotResponse;
    return setOutput(client, httpResponse(404, "Not Found", "text/plain", "not found\n"),
        server.configuration.maximumPendingBytes, nowMilliseconds);
}

/** @brief Removes one client and its matching bounded stream queue. */
void disconnectClient(XWalkMjpegHttpServer& server, agent::size index,
    agent::uint64 nowMilliseconds) noexcept
{
    ...
    if (server.configuration.observability != nullptr)
    {
        ::xwalk::recordFailureEvent(*server.configuration.observability,
            ::xwalk::XWalkFailureEventId::ClientDisconnection, nowMilliseconds);
    }
}
...
void acceptClients(XWalkMjpegHttpServer& server, agent::uint64 nowMilliseconds) noexcept
{
    while (true)
    {
        ...
        if (!makeNonBlocking(descriptor) ||
            server.clients.size() >= server.configuration.stream.maximumClients)
        {
        ...
        {
            ::xwalk::recordFailureEvent(*server.configuration.observability,
                ::xwalk::XWalkFailureEventId::ClientConnection, nowMilliseconds);
        }
    }
}

/** @brief Receives bounded request bytes and routes a complete header. */
agent::boolean readClient(XWalkMjpegHttpServer& server,
    XWalkMjpegHttpClient& client, agent::uint64 nowMilliseconds) noexcept
{
    ...
    if (received > server.configuration.maximumRequestBytes - client.request.size())
    {
        ...
        client.mode = XWalkMjpegHttpClientMode::OneShotResponse;
        return setOutput(client, httpResponse(431, "Request Header Fields Too Large",
    ...
    if (!routeRequest(server, client, nowMilliseconds))
    {
        incrementSaturated(server.invalidRequests);
        client.mode = XWalkMjpegHttpClientMode::OneShotResponse;
        return setOutput(client, httpResponse(400, "Bad Request", "text/plain",
            "invalid request\n"), server.configuration.maximumPendingBytes,
            nowMilliseconds);
    }
...
agent::boolean writeClient(XWalkMjpegHttpServer& server,
    XWalkMjpegHttpClient& client, agent::uint64 nowMilliseconds) noexcept
{
    if (client.outputOffset >= client.pendingOutput.size())
    {
        ...
        if ((client.mode == XWalkMjpegHttpClientMode::Stream) && (server.stream != nullptr))
        {
            ...
            const XWalkMjpegStreamStatus status = popMjpegMultipartFrame(*server.stream,
                client.identifier, client.pendingOutput, sequence);
    ...
    const ssize_t count = ::send(client.descriptor,
        client.pendingOutput.data() + client.outputOffset, remaining, MSG_NOSIGNAL);
    if (count < 0)
    {
        return (errno == EAGAIN) || (errno == EWOULDBLOCK) || (errno == EINTR);
    }
...
{
    if ((validateMjpegStreamConfiguration(configuration.stream) !=
            XWalkMjpegStreamStatus::Ok) ||
        (configuration.maximumRequestBytes < 64U) ||
        (configuration.maximumRequestBytes > MAXIMUM_REQUEST_BYTES) ||
    ...
    if (!loopbackAddress(configuration.stream.bindAddress) &&
        (configuration.authenticationReference.empty() ||
            (configuration.authenticate == nullptr)))
    {
        return XWalkMjpegHttpStatus::InvalidConfiguration;
    }
    ...
    agent::size maximumBytes, XWalkMjpegHttpRequest& request) noexcept
{
    ...
    if ((input.find('\0') != agent::stringview::npos) ||
        (input.find("\r\n\r\n") == agent::stringview::npos))
    {
        return input.find('\0') != agent::stringview::npos ?
            XWalkMjpegHttpParseStatus::Invalid : XWalkMjpegHttpParseStatus::Incomplete;
    }
    ...
    if ((line.size() <= PREFIX.size() + SUFFIX.size()) ||
        (line.substr(0U, PREFIX.size()) != PREFIX) ||
        (line.substr(line.size() - SUFFIX.size()) != SUFFIX))
    {
        return XWalkMjpegHttpParseStatus::Invalid;
    }
    request.path = line.substr(PREFIX.size(),
    ...
    const XWalkMjpegHttpConfiguration& configuration) noexcept
{
    ...
    const agent::string bindAddress = configuration.stream.bindAddress == "localhost" ?
        "127.0.0.1" : configuration.stream.bindAddress;
    ...
    {
        ::xwalk::recordFailureEvent(*server.configuration.observability,
            ::xwalk::XWalkFailureEventId::StreamStart, 0U);
    }
    return XWalkMjpegHttpStatus::Ok;
}
    ...
    agent::uint64 nowMilliseconds) noexcept
{
    ...
    while (index < server.clients.size())
    {
        ...
        const agent::boolean headerTimedOut =
            (client.mode == XWalkMjpegHttpClientMode::ReadingRequest) &&
            (elapsed(nowMilliseconds, client.connectedAtMilliseconds) >=
                server.configuration.headerTimeoutMilliseconds);
        const agent::boolean idleTimedOut = elapsed(nowMilliseconds,
            client.lastProgressMilliseconds) >= server.configuration.idleTimeoutMilliseconds;
        const agent::boolean slowTimedOut = !client.pendingOutput.empty() &&
            (elapsed(nowMilliseconds, client.outputQueuedAtMilliseconds) >=
...
XWalkMjpegHttpStatus stopMjpegHttpServer(XWalkMjpegHttpServer& server) noexcept
{
    ...
    ::xwalk::XWalkFailureObservability* observability =
        server.configuration.observability;
    ...
    {
        ::xwalk::recordFailureEvent(*observability,
            ::xwalk::XWalkFailureEventId::StreamStop, 0U);
    }
    return XWalkMjpegHttpStatus::Ok;
}
...
agent::uint32 mjpegHttpServerPort(const XWalkMjpegHttpServer& server) noexcept
{
    ...
    if (::getsockname(server.listener, reinterpret_cast<sockaddr*>(&address),
            &addressLength) != 0)
    {
        return 0U;
    }
