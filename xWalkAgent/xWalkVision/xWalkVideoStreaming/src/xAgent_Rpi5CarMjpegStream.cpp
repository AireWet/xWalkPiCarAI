...
agent::boolean validAddressText(agent::stringview address) noexcept
{
    return !address.empty() &&
        ...
        (address.find('\r') == agent::stringview::npos) &&
        (address.find(' ') == agent::stringview::npos) &&
        (address.find('\t') == agent::stringview::npos);
}

/** @brief Returns one client iterator under the caller-held stream lock. */
std::vector<XWalkMjpegClientState>::iterator findClient(
    XWalkMjpegStreamState& state, agent::uint32 identifier) noexcept
{
    return std::find_if(state.clients.begin(), state.clients.end(),
...
XWalkMjpegStreamStatus validateMjpegStreamConfiguration(
    const XWalkMjpegStreamConfiguration& configuration) noexcept
{
    const agent::boolean loopback =
        (configuration.bindAddress == "127.0.0.1") ||
        (configuration.bindAddress == "localhost") ||
        (configuration.bindAddress == "::1");
    if (!validAddressText(configuration.bindAddress) ||
        (!loopback && !configuration.allowExternalBind) ||
        (configuration.port == 0U) || (configuration.port > 65'535U) ||
        (configuration.maximumClients == 0U) ||
        (configuration.maximumClients > MAXIMUM_CLIENTS) ||
        (configuration.queueCapacity == 0U) ||
...
XWalkMjpegStreamStatus addMjpegStreamClient(XWalkMjpegStreamState& state,
...
XWalkMjpegStreamStatus publishMjpegFrame(XWalkMjpegStreamState& state,
    const agent::bytevector& jpeg) noexcept
{
    const std::lock_guard<std::mutex> lock(state.mutex);
    if (!state.started)
    {
    ...
    if ((jpeg.size() < MINIMUM_JPEG_BYTES) ||
        (jpeg.size() > state.configuration.maximumJpegBytes) ||
        (jpeg[0U] != 0xFFU) || (jpeg[1U] != 0xD8U) ||
        (jpeg[jpeg.size() - 2U] != 0xFFU) || (jpeg.back() != 0xD9U))
    {
...
XWalkMjpegStreamStatus popMjpegMultipartFrame(XWalkMjpegStreamState& state,
    agent::uint32 clientIdentifier, agent::bytevector& multipartFrame,
...
XWalkMjpegStreamStatus reportMjpegCameraLoss(XWalkMjpegStreamState& state) noexcept
{
    ...
    state.cameraAvailable = false;
    return state.started ? XWalkMjpegStreamStatus::CameraUnavailable :
        XWalkMjpegStreamStatus::NotStarted;
}
