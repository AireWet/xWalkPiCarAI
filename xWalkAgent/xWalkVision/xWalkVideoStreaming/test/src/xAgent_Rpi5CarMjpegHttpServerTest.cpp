...
TEST(XWalkMjpegHttpServer, ValidatesBoundsAndExternalAuthenticationPolicy)
{
    ...
    EXPECT_EQ(validateMjpegHttpConfiguration(configuration),
        XWalkMjpegHttpStatus::InvalidConfiguration);
...
TEST(XWalkMjpegHttpServer, StreamsMultipartFramesWithBoundedQueue)
{
    ...
    ASSERT_EQ(publishMjpegFrame(stream, jpegFrame(0x44U, 32U)),
...
TEST(XWalkMjpegHttpServer, ReportsPortCollisionAndCleansConnectedClients)
{
    ...
    const XWalkMjpegHttpConfiguration configuration = loopbackConfiguration();
    ASSERT_EQ(startMjpegHttpServer(firstServer, firstStream, configuration),
        XWalkMjpegHttpStatus::Ok);
    EXPECT_EQ(startMjpegHttpServer(secondServer, secondStream, configuration),
        XWalkMjpegHttpStatus::SocketFailure);
    int client = connectLoopback(configuration.stream.port);
    ASSERT_GE(client, 0);
...
TEST(XWalkFailureObservability, ConcurrentProductionIsBoundedOrderedAndResettable)
{
    ...
    const auto produce = [&observability](uint64 base)
    {
        for (uint64 index = 0U; index < 100U; ++index)
        {
            ::xwalk::recordFailureEvent(observability,
                ::xwalk::XWalkFailureEventId::DroppedFrame, base + index, index);
        }
    };
    ...
    const ::xwalk::XWalkFailureSnapshot snapshot =
        ::xwalk::failureSnapshot(observability);
    ASSERT_EQ(snapshot.eventCount, ::xwalk::XWALK_FAILURE_EVENT_CAPACITY);
    EXPECT_EQ(snapshot.counters[static_cast<size>(
        ::xwalk::XWalkFailureEventId::DroppedFrame)], 400U);
    for (size index = 1U; index < snapshot.eventCount; ++index)
    {
        EXPECT_EQ(snapshot.events[index].sequence,
            snapshot.events[index - 1U].sequence + 1U);
    }
    ::xwalk::resetFailureObservability(observability);
    const ::xwalk::XWalkFailureSnapshot reset =
        ::xwalk::failureSnapshot(observability);
