 ...
 */
void XWalkServoMotorCalibration::setServoAngle(agent::uint8 servoId,
  ...
  if (servoId == 0U) {
    ...
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "Servo calibration identifier must be 0, 1, or 2");
  }
}
 ...
 */
agent::boolean
XWalkServoMotorCalibration::applyServoOffset(agent::uint8 servoId) {
  if (servoId == 0U) {
    picarxObject->previewDirectionServoCalibration(
        resultValue.servoOffsets[servoId]);
  } else if (servoId == 1U) {
    picarxObject->previewCameraPanServoCalibration(
        resultValue.servoOffsets[servoId]);
  } else if (servoId == 2U) {
    picarxObject->previewCameraTiltServoCalibration(
        resultValue.servoOffsets[servoId]);
  } else {
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "Servo calibration identifier must be 0, 1, or 2");
 ...
 */
agent::boolean
XWalkServoMotorCalibration::setServoOffset(agent::uint8 servoId,
                                           agent::float64 offsetDegrees) {
  const agent::boolean offsetDegreesNotFinite =
      static_cast<agent::boolean>(!XHAL_IS_FINITE(offsetDegrees));
  if (offsetDegreesNotFinite) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Servo calibration offset must be finite");
  }
  if ((servoId > 2U) || (offsetDegrees < -20.0) || (offsetDegrees > 20.0)) {
    XWALK_RPIAGENT_ERROR(
        XWALK_RANGE,
        "Servo calibration requires identifier 0-2 and offset -20 through 20");
  }
 ...
 */
void XWalkServoMotorCalibration::setMotorDirection(agent::uint8 motorId,
                                                   agent::int32 direction) {
  picarxObject->previewMotorDirection(motorId, direction);
  resultValue.motorDirections[static_cast<agent::size>(motorId - 1U)] =
      direction;
}
...
void XWalkServoMotorCalibration::toggleMotorDirection(agent::uint8 motorId) {
  if ((motorId < 1U) || (motorId > 2U)) {
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "Motor calibration identifier must be 1 or 2");
  }
 ...
 */
const XWalkServoMotorCalibrationResult &
XWalkServoMotorCalibration::result() const noexcept {
  return resultValue;
}
