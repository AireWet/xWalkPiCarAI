...
void XWalkServoZeroing::validate(
  ...
  if ((backendCallbacks.setAngle == nullptr) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Servo-zeroing callbacks must be complete");
  }
  ...
  if ((configuration.pulseAngleDegrees < -90.0) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "Servo-zeroing configuration is out of range");
  }
}
 ...
 */
void XWalkServoZeroing::setCancellation(
    agent::contextpointer context,
    servozeroingcontinuecallback continueOperation) {
  if (continueOperation == nullptr) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Servo-zeroing cancellation callback is required");
  }
