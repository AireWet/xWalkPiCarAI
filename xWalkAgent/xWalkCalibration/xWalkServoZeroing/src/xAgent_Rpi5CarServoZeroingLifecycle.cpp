 ...
 */
void XWalkServoZeroing::validate(
    const XWalkServoZeroingCallbacks& backendCallbacks,
    const XWalkServoZeroingConfiguration& configuration)
{
    if ((backendCallbacks.setAngle == nullptr) ||
        (backendCallbacks.delay == nullptr) ||
        (backendCallbacks.continueOperation == nullptr))
    {
        XHAL_THROW_INVALID_ARGUMENT("Servo-zeroing callbacks must be complete");
    }
    const agent::boolean configurationInvalid =
        static_cast<agent::boolean>(
            !XHAL_IS_FINITE(configuration.pulseAngleDegrees) ||
        !XHAL_IS_FINITE(configuration.zeroAngleDegrees));
    if (configurationInvalid)
    {
        XHAL_THROW_INVALID_ARGUMENT("Servo-zeroing angles must be finite");
    }
    if ((configuration.pulseAngleDegrees < -90.0) ||
        (configuration.pulseAngleDegrees > 90.0) ||
        (configuration.zeroAngleDegrees < -90.0) ||
        (configuration.zeroAngleDegrees > 90.0) ||
        (configuration.commandDelayMs == 0U) ||
        (configuration.commandDelayMs > 10'000U) ||
        (configuration.idleDelayMs == 0U) ||
        (configuration.idleDelayMs > 10'000U))
    {
        XHAL_THROW_OUT_OF_RANGE("Servo-zeroing configuration is out of range");
    }
}
 ...
 * @return `true` after the complete interval; otherwise `false` after cancellation.
 */
agent::boolean XWalkServoZeroing::wait(agent::uint32 durationMs) const
{
    ...
    while (remainingMs > 0U)
    {
        const agent::boolean operationRequested =
            callbacks.continueOperation(cancellationContext);
    ...
    servozeroingcontinuecallback continueOperation)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Servo-zeroing cancellation callback is required");
    }
