 ...
 * @param[in] durationMs Requested duration in milliseconds; unused because speaker power is never enabled.
 */
...
/**
 * @brief Resets the Robot HAT MCU, composes the PiCar-X agent, and commands its safe closed state.
 * @return Zero after every physical operation succeeds; an exception reports hardware failure.
 * @warning This function accesses physical I2C, GPIO, motors, and servos. Lift the wheels and clear motion.
 */
XWalkHal::int32 main()
{
    xwalk::hal::XWalkConfigStore config(XWALK_PICARX_CONFIG_FILE);
    const XWalkHal::string i2cDevice = config.get(
        "hardware_i2c_device", XHAL_RPI5CAR_I2C_DEFAULT_DEVICE);
    const XWalkHal::string gpioDevice = config.get(
        "hardware_gpio_device", XHAL_RPI5CAR_GPIO_DEFAULT_DEVICE);
    const XWalkHal::string deviceTreeRoot = config.get(
        "hardware_device_tree_root", XHAL_RPI5CAR_DEVICE_TREE_ROOT);
    const XWalkHal::string requestedBoard = config.get("hardware_board", "auto");
    const XWalkHal::string gpioChipName = config.get("hardware_gpio_chip_name", "");
    const XWalkHal::string gpioChipLabel = config.get("hardware_gpio_chip_label", "");
    constexpr XWalkHal::uint32 minimumGpioLineCount{28U};
    ...
    {
        XHAL_THROW_RUNTIME_ERROR("Hardware test requires verified HAT detection or explicit v4 selection");
    }
    else if ((requestedBoard != "auto") && (requestedBoard != "robot_hat_v5"))
    {
    ...
    {
        xwalk::hal::XWalkGpioLinux resetBackend(gpioDevice.c_str(),
            gpioChipName, gpioChipLabel, minimumGpioLineCount);
        xwalk::hal::XWalkGpioLinux speakerBackend(gpioDevice.c_str(),
            gpioChipName, gpioChipLabel, minimumGpioLineCount);
        ...
        xwalk::hal::XWalkGpio speakerGpio(
            &speakerBackend, callbacks, deviceInformation.speakerEnablePin);
    ...
    xwalk::hal::XWalkGpioLinux triggerBackend(gpioDevice.c_str(),
        gpioChipName, gpioChipLabel, minimumGpioLineCount);
    xwalk::hal::XWalkGpioLinux echoBackend(gpioDevice.c_str(),
        gpioChipName, gpioChipLabel, minimumGpioLineCount);
    const xwalk::hal::XWalkGpioCallbacks callbacks = XHAL_GPIO_CALLBACKS(xwalk::hal::XWalkGpioLinux);
    ...
    const auto closePicarx = [&](xwalk::hal::XWalkMotors& motors) -> XWalkHal::int32
    {
    ...
    xwalk::hal::XWalkGpioLinux leftDirectionBackend(gpioDevice.c_str(),
        gpioChipName, gpioChipLabel, minimumGpioLineCount);
    xwalk::hal::XWalkGpioLinux rightDirectionBackend(gpioDevice.c_str(),
        gpioChipName, gpioChipLabel, minimumGpioLineCount);
