...
class XWalkPicarx
{
    ...
    protected:
        ...
        static hal::float64 constrain(hal::float64 value, hal::float64 minimum, hal::float64 maximum);
        /** @brief Parses one complete finite floating-point configuration value. */
        static hal::float64 parseFloat(hal::stringview text, hal::cstring name);
        /** @brief Parses exactly three signed integer values enclosed in square brackets. */
        static hal::linetrackervalues parseReferences(hal::stringview text, hal::cstring name);
        /** @brief Parses exactly two motor direction values enclosed in square brackets. */
        static hal::fixedarray<hal::int32, 2U> parseMotorDirections(hal::stringview text);
        /** @brief Formats three signed integer values using the Python-compatible list form. */
        static hal::string formatReferences(const hal::linetrackervalues& values);
        ...
        hal::float64 calibratedMotorSpeed(hal::uint8 motorId, hal::float64 speedPercent) const;
    ...
    public:
         ...
         */
        XWalkPicarx(hal::XWalkMotors& motors, hal::XWalkServo& directionServo,
            hal::XWalkServo& cameraPanServo, hal::XWalkServo& cameraTiltServo,
            hal::XWalkGrayscaleModule& grayscale, hal::XWalkUltrasonic& ultrasonic,
        ...
        hal::boolean cliffStatus(const hal::linetrackervalues& value) const noexcept;
        ...
        hal::float64 directionAngleDegrees() const noexcept;
};
