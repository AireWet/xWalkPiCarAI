...
agent::int32 XWalkVoiceControlledCar::run() {
  ...
  while (processingLoopRequested) {
    ...
    while (listeningRequested) {
      const agent::boolean listeningMayContinue = static_cast<agent::boolean>(
          callbacks.shouldContinue(callbackContext));
      if (listeningMayContinue == false) {
        break;
      }
      const agent::string transcript =
          normalize(speechToTextObject->listen(configuration.listenTimeoutMs));
      const agent::boolean transcriptEmpty =
          static_cast<agent::boolean>(transcript.empty());
      if (transcriptEmpty) {
        continue;
      }
      ...
      if (command == XWalkVoiceControlledCarCommand::Sleep) {
        callbacks.output(callbackContext,
                         "Sleeping. Say the wake word to activate me again.");
        break;
      }
    }
  }
...
XWalkVoiceControlledCarCommand
XWalkVoiceControlledCar::classifyCommand(agent::stringview transcript,
                                         agent::stringview sleepWord) {
  const agent::boolean sleepWordEmpty =
      static_cast<agent::boolean>(sleepWord.empty());
  if (sleepWordEmpty) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Voice-controlled-car sleep word must not be empty");
  }
  const agent::string normalized = normalize(transcript);
  const agent::boolean normalizedNormalizeSleepWordDifferent =
      static_cast<agent::boolean>(normalized.find(normalize(sleepWord)) !=
                                  agent::string::npos);
  if (normalizedNormalizeSleepWordDifferent) {
    return XWalkVoiceControlledCarCommand::Sleep;
  }
  const agent::boolean normalizedForwardDifferent = static_cast<agent::boolean>(
      normalized.find("forward") != agent::string::npos);
  ...
  const agent::boolean normalizedBackwardDifferent =
      static_cast<agent::boolean>(normalized.find("backward") !=
                                  agent::string::npos);
  if (normalizedBackwardDifferent) {
    return XWalkVoiceControlledCarCommand::Backward;
  }
  const agent::boolean normalizedLeftDifferent = static_cast<agent::boolean>(
      normalized.find("left") != agent::string::npos);
  if (normalizedLeftDifferent) {
    return XWalkVoiceControlledCarCommand::Left;
  }
  const agent::boolean normalizedRightDifferent = static_cast<agent::boolean>(
      normalized.find("right") != agent::string::npos);
...
agent::boolean
XWalkVoiceControlledCar::containsWakeWord(agent::stringview transcript,
                                          agent::stringview wakeWord) {
  const agent::boolean wakeWordEmpty =
      static_cast<agent::boolean>(wakeWord.empty());
  if (wakeWordEmpty) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Voice-controlled-car wake word must not be empty");
  }
...
void XWalkVoiceControlledCar::waitForMovement() {
  ...
  while (movementWaitRequested) {
    const agent::boolean movementMayContinue = static_cast<agent::boolean>(
        (remainingMs > 0U) && callbacks.shouldContinue(callbackContext));
    if (movementMayContinue == false) {
      break;
    }
    const agent::uint32 sliceMs =
        (remainingMs < maximumSliceMs) ? remainingMs : maximumSliceMs;
