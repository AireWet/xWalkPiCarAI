...
XWalkLocalVoiceChatbot::~XWalkLocalVoiceChatbot() { assistantObject->stop(); }
...
agent::int32 XWalkLocalVoiceChatbot::run() {
  ...
  while (processingLoopRequested) {
    ...
    const agent::boolean recognizedTextEmpty =
        static_cast<agent::boolean>(recognizedText.empty());
    ...
    assistantObject->say(cleanResponse.empty() ? configuration.emptyResponse
                                               : cleanResponse);
...
void XWalkLocalVoiceChatbot::stop() { assistantObject->stop(); }
...
void XWalkLocalVoiceChatbot::validate(
  ...
  if ((backendCallbacks.output == nullptr) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Local voice-chatbot callbacks must be complete");
  }
  if ((chatbotConfiguration.listenTimeoutMs == 0U) ||
    ...
    XWALK_RPIAGENT_ERROR(
        XWALK_RANGE, "Local voice-chatbot listen timeout is outside its range");
  }
  const agent::boolean chatbotConfigurationInvalid =
      static_cast<agent::boolean>(
          ...
          chatbotConfiguration.goodbye.empty() ||
          chatbotConfiguration.bye.empty());
  if (chatbotConfigurationInvalid) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "Local voice-chatbot messages must not be empty");
  }
}
