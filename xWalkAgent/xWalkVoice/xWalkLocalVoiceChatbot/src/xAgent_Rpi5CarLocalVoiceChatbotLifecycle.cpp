...
XWalkLocalVoiceChatbot::XWalkLocalVoiceChatbot(
    hal::XWalkVoiceAssistant& assistant, agent::contextpointer context,
 ...
 * @throws std::exception Propagates callback or voice-pipeline failures after
 * attempting the farewell and assistant shutdown.
 */
agent::int32 XWalkLocalVoiceChatbot::run()
{
    ...
    try
    {
        ...
        while (processingLoopRequested)
        {
            ...
            const agent::string recognizedText =
                assistantObject->listen(configuration.listenTimeoutMs);
            const agent::boolean recognizedTextEmpty =
                static_cast<agent::boolean>(
            ...
            assistantObject->say(
                cleanResponse.empty() ? configuration.emptyResponse : cleanResponse);
    ...
    const XWalkLocalVoiceChatbotConfiguration& chatbotConfiguration)
{
    ...
    const agent::boolean chatbotConfigurationInvalid =
        static_cast<agent::boolean>(
            chatbotConfiguration.welcome.empty() ||
        chatbotConfiguration.listeningMessage.empty() ||
        chatbotConfiguration.silenceMessage.empty() ||
        chatbotConfiguration.emptyResponse.empty() ||
        chatbotConfiguration.stoppingMessage.empty() ||
        chatbotConfiguration.goodbye.empty() || chatbotConfiguration.bye.empty());
    if (chatbotConfigurationInvalid)
    {
        XHAL_THROW_INVALID_ARGUMENT("Local voice-chatbot messages must not be empty");
    }
}

/**
 * @brief Speaks the farewell, stops recognition, and reports completion.
 * @throws std::exception Propagates a speech or output backend failure after
 * stopping the assistant whenever farewell speech fails.
 */
