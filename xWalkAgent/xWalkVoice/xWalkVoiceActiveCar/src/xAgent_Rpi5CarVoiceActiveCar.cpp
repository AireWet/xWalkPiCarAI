...
XWalkVoiceActiveCar::XWalkVoiceActiveCar(XWalkPicarx& picarx,
    ...
    hal::XWalkLed& led, agent::contextpointer context,
 ...
 */
XWalkVoiceActiveCarResponse XWalkVoiceActiveCar::parseJsonResponse(
    agent::stringview response)
{
    XWalkVoiceActiveCarResponse result;
    ...
    const agent::boolean actionArrayFound =
        static_cast<agent::boolean>(
            (actionsPosition != agent::string::npos) &&
    ...
    const agent::boolean responseEmpty =
        static_cast<agent::boolean>(
 ...
 */
XWalkVoiceActiveCarResponse XWalkVoiceActiveCar::parseConfiguredResponse(
...
void XWalkVoiceActiveCar::blink(agent::uint32 count,
    agent::uint32 toggleDelayMs, agent::uint32 pauseMs)
{
...
agent::string XWalkVoiceActiveCar::sensorPrompt()
{
    ...
    if ((distanceCm > 1.0) && (distanceCm < configuration.tooCloseCm))
    {
        ...
        callbacks.output(callbackContext,
            agent::string("Ultrasonic sense too close: ") + distance + "cm");
    ...
    const XWalkVoiceActiveCarConfiguration& carConfiguration)
{
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("Voice-active-car keyboard input callback is required");
    }
    ...
    if (wakeWordMissing)
    {
        XHAL_THROW_INVALID_ARGUMENT(
            "Voice-active-car wake word is required when wake detection is enabled");
    }
}
