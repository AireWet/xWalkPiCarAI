...
XWalkVoiceActiveCar::XWalkVoiceActiveCar(
    XWalkPicarx &picarx, XWalkSelfDrive &selfDrive,
...
agent::int32 XWalkVoiceActiveCar::run() {
  ...
  while (processingLoopRequested) {
    ...
    const agent::boolean promptEmpty =
        static_cast<agent::boolean>(prompt.empty());
    if (promptEmpty) {
      blink(2U, 100U, 800U);
      ...
      const agent::boolean receivedPromptEmpty =
          static_cast<agent::boolean>(prompt.empty());
      if (receivedPromptEmpty) {
        continue;
      }
      if (configuration.wakeEnabled && !wakeDetectedValue) {
        const agent::boolean wakePhraseMatched =
            static_cast<agent::boolean>(isWakePhrase(prompt));
...
XWalkVoiceActiveCarResponse
XWalkVoiceActiveCar::parseResponse(agent::stringview response) {
  ...
  if (actionStart != agent::string::npos) {
    agent::string actions =
        trim(response.substr(actionStart + delimiter.size()));
    const agent::boolean actionParsingRequested{true};
    while (actionParsingRequested) {
      const agent::boolean actionsAvailable =
          static_cast<agent::boolean>(!actions.empty());
  ...
  const agent::boolean actionsEmpty =
      static_cast<agent::boolean>(result.actions.empty());
  if (actionsEmpty) {
 ...
 */
XWalkVoiceActiveCarResponse
XWalkVoiceActiveCar::parseJsonResponse(agent::stringview response) {
  XWalkVoiceActiveCarResponse result;
  const auto parseString = [&response](agent::size &position) -> agent::string {
    agent::string value;
    const agent::boolean positionResponseInvalid = static_cast<agent::boolean>(
        (position >= response.size()) || (response[position] != '"'));
    ...
    while (stringDecodingRequested) {
      ...
      const agent::boolean escapeCharacterFound = static_cast<agent::boolean>(
          (character == '\\') && (position < response.size()));
  ...
  const auto valuePosition = [&response](agent::stringview key) -> agent::size {
    const agent::string quotedKey =
        agent::string("\"") + agent::string(key) + "\"";
  ...
  if (actionArrayFound) {
    ...
    while (actionArrayParsingRequested) {
      ...
      while (actionWhitespaceParsingRequested) {
        const agent::boolean actionsPositionResponseTRNInvalid =
            static_cast<agent::boolean>((actionsPosition < response.size()) &&
                                        ((response[actionsPosition] == ' ') ||
                                         (response[actionsPosition] == '\t') ||
                                         (response[actionsPosition] == '\r') ||
                                         (response[actionsPosition] == '\n') ||
                                         (response[actionsPosition] == ',')));
      ...
      const agent::boolean actionsPositionResponseInvalid =
          static_cast<agent::boolean>((actionsPosition >= response.size()) ||
                                      (response[actionsPosition] == ']'));
      ...
      const agent::boolean actionEmpty =
          static_cast<agent::boolean>(action.empty());
      if (actionEmpty) {
        break;
      }
  ...
  const agent::boolean responseEmpty = static_cast<agent::boolean>(
      result.text.empty() && result.actions.empty());
  if (responseEmpty) {
 ...
 */
XWalkVoiceActiveCarResponse
XWalkVoiceActiveCar::parseConfiguredResponse(agent::stringview response) const {
 ...
 */
void XWalkVoiceActiveCar::setInputMode(
    XWalkVoiceActiveCarInputMode inputMode) noexcept {
...
void XWalkVoiceActiveCar::blink(agent::uint32 count,
                                agent::uint32 toggleDelayMs,
...
agent::string XWalkVoiceActiveCar::sensorPrompt() {
  ...
  if ((distanceCm > 1.0) && (distanceCm < configuration.tooCloseCm)) {
    ...
    callbacks.output(callbackContext,
                     agent::string("Ultrasonic sense too close: ") + distance +
                         "cm");
    return agent::string("<<<Ultrasonic sense too close: ") + distance +
           "cm>>>";
  }
  return {};
}
...
void XWalkVoiceActiveCar::validate(
  ...
  if ((carConfiguration.tooCloseCm <= 1.0) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "Voice-active-car configuration is outside its range");
  }
  const agent::boolean wakeWordMissing = static_cast<agent::boolean>(
      carConfiguration.wakeEnabled && carConfiguration.wakeWord.empty());
  if (wakeWordMissing) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL, "Voice-active-car wake word is required "
