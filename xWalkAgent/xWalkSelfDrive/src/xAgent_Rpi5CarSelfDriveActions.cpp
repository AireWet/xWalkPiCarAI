...
namespace
{
    ...
    constexpr xwalk::hal::float64 PRESET_DRIVE_SPEED_PERCENT = 5.0;
    constexpr xwalk::hal::uint32 PRESET_DRIVE_DURATION_MS = 1'000U;
    constexpr xwalk::hal::cstring HORN_SOUND_FILE = "car-double-horn.wav";
    constexpr xwalk::hal::cstring ENGINE_SOUND_FILE = "car-start-engine.wav";
 ...
 */
hal::boolean XWalkSelfDrive::isActionSupported(hal::stringview action) noexcept
{
 ...
 */
hal::boolean XWalkSelfDrive::honking()
{
    const hal::filesystempath soundPath = hal::resolveResourcePath(
        soundDirectoryValue, HORN_SOUND_FILE);
    if (!hal::isReadableRegularFile(soundPath))
    {
        return false;
    }
 ...
 */
hal::boolean XWalkSelfDrive::startEngine()
{
    const hal::filesystempath soundPath = hal::resolveResourcePath(
 ...
 */
hal::boolean XWalkSelfDrive::doAction(hal::stringview action)
{
