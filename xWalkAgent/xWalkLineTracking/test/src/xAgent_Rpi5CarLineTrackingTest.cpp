...
struct TestBus
{
    XWalkHal::uint32vector samples{};
    XWalkHal::size nextSample{};
};
    ...
    XWalkHal::size length)
{
    ...
    const XWalkHal::uint32 sample = bus.samples[bus.nextSample];
    ++bus.nextSample;
    const XWalkHal::uint8 high = static_cast<XWalkHal::uint8>((sample >> 8U) & 0xFFU);
    const XWalkHal::uint8 low = static_cast<XWalkHal::uint8>(sample & 0xFFU);
    return {high, low};
}
...
void appendFrame(TestBus& bus, XWalkHal::uint32 left, XWalkHal::uint32 middle,
    XWalkHal::uint32 right)
{
 ...
 */
void testLineTrackingBehavior(XWalkHal::stringview configPath)
{
    TestBus bus;
