...
agent::boolean transportStart(agent::contextpointer context, agent::stringview,
    agent::stringview, agent::uint16)
{
...
agent::boolean continueOperation(agent::contextpointer context)
{
    State& value = state(context);
    ++value.continueCount;
    return value.continueResult &&
        (value.continueCount <= value.continueCallsBeforeCancel);
}
