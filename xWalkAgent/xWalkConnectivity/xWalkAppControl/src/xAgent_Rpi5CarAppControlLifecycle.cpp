...
XWalkAppControl::XWalkAppControl(
    XWalkPicarx &picarx, const XWalkAppControlCallbacks &providerCallbacks,
...
void XWalkAppControl::validate(
  ...
  if ((providerCallbacks.start == nullptr) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "App control requires complete callbacks");
  }
  ...
  if ((configuration.controllerPort == 0U) ||
    ...
    XWALK_RPIAGENT_ERROR(XWALK_RANGE,
                         "App-control configuration is out of range");
  }
}
...
agent::boolean XWalkAppControl::start() {
  ...
  lastLineState = 0U;
  const agent::boolean transportStarted = callbacks.start(
      callbacks.transportContext, configurationValue.controllerName,
      configurationValue.controllerType, configurationValue.controllerPort);
  if (transportStarted == false) {
    return false;
  }
  const agent::boolean visionStarted =
      callbacks.vision.start(callbacks.visionContext);
