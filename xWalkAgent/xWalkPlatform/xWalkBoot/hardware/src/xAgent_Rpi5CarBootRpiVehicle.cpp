...
{
    const agent::string i2cDevice = config.get(
        "hardware_i2c_device", XHAL_RPI5CAR_I2C_DEFAULT_DEVICE);
    const agent::string gpioDevice = config.get(
        "hardware_gpio_device", XHAL_RPI5CAR_GPIO_DEFAULT_DEVICE);
    const agent::string deviceTreeRoot = config.get(
    ...
    const agent::uint32 minimumGpioLineCount = parseUnsigned(config.get(
        "hardware_gpio_minimum_line_count", "28"),
        "hardware_gpio_minimum_line_count", hal::common::UINT32_MAXIMUM);
    ...
    hal::XWalkGpio speakerGpio(&speakerBackend, gpioCallbacks,
        deviceInformation.speakerEnablePin);
    hal::XWalkGpioLinux resetBackend(gpioDevice.c_str(), gpioChipName,
        gpioChipLabel, minimumGpioLineCount);
    hal::XWalkGpio resetGpio(&resetBackend, gpioCallbacks,
        config.get("hardware_mcu_reset_pin", "MCURST"));
    hal::XWalkAdc batteryAdc(i2c,
        config.get("hardware_battery_adc_channel", "A4"));
    hal::XWalkBoardControl boardControl(resetGpio, speakerGpio, batteryAdc,
        nullptr, &primeSpeaker);
    boardControl.resetMcu();
    const agent::uint32 resetSettleMs = parseUnsigned(config.get(
        "hardware_mcu_reset_settle_ms", "200"),
        "hardware_mcu_reset_settle_ms", hal::common::UINT32_MAXIMUM);
    ...
    const agent::boolean speakerRequired = static_cast<agent::boolean>(
        (selectedMode == XWALK_BOOT_SELF_DRIVE_REQ) ||
        (selectedMode == XWALK_BOOT_SOUND_REQ) ||
        (selectedMode == XWALK_BOOT_APP_CONTROL_REQ) ||
        (selectedMode == XWALK_BOOT_SOUND_BACKGROUND_MUSIC_REQ) ||
    ...
    hal::XWalkGpioLinux echoBackend(gpioDevice.c_str(), gpioChipName,
        gpioChipLabel, minimumGpioLineCount);
    hal::XWalkGpio trigger(&triggerBackend, gpioCallbacks,
        config.get("hardware_ultrasonic_trigger_pin", "D2"));
    hal::XWalkGpio echo(&echoBackend, gpioCallbacks,
        config.get("hardware_ultrasonic_echo_pin", "D3"));
    ...
    const agent::boolean v5MotorMode = static_cast<agent::boolean>(
        deviceInformation.motorMode == XHAL_RPI5CAR_DEVICE_V5_MOTOR_MODE);
    if (v5MotorMode)
    {
        hal::XWalkPwm leftForwardPwm(i2c,
            config.get("hardware_v5_left_forward_pwm_channel", "P12"), {}, timerState);
        hal::XWalkPwm leftReversePwm(i2c,
            config.get("hardware_v5_left_reverse_pwm_channel", "P13"), {}, timerState);
        hal::XWalkPwm rightForwardPwm(i2c,
            config.get("hardware_v5_right_forward_pwm_channel", "P14"), {}, timerState);
        hal::XWalkPwm rightReversePwm(i2c,
    ...
    hal::XWalkPwm leftPwm(i2c,
        config.get("hardware_v4_left_pwm_channel", "P13"), {}, timerState);
    hal::XWalkPwm rightPwm(i2c,
        config.get("hardware_v4_right_pwm_channel", "P12"), {}, timerState);
    ...
    hal::XWalkGpioLinux rightDirectionBackend(gpioDevice.c_str(), gpioChipName,
        gpioChipLabel, minimumGpioLineCount);
    hal::XWalkGpio leftDirection(&leftDirectionBackend, gpioCallbacks,
        config.get("hardware_v4_left_direction_pin", "D4"));
    hal::XWalkGpio rightDirection(&rightDirectionBackend, gpioCallbacks,
