...
agent::int32 XWalkBootRpi::runVoiceActiveMode(agent::uint8 mode,
    ...
    XWalkPicarx& picarx, agent::stringview gpioDevice,
    agent::stringview gpioChipName, agent::stringview gpioChipLabel,
    agent::uint32 minimumGpioLineCount,
    const hal::XWalkGpioCallbacks& gpioCallbacks)
{
    const agent::string voskLibrary = config.get(
        "voice_vosk_library", "/usr/lib/xwalk/libvosk.so");
    const agent::string voskModel = config.get("voice_vosk_model",
    ...
    const agent::string espeakExecutable = config.get(
    ...
    const agent::string piperPlaybackExecutable = config.get(
    ...
    agent::string modelEndpoint = config.get("voice_language_model_endpoint",
        config.get("voice_ollama_endpoint", "http://127.0.0.1:11434/api/chat"));
    const agent::string modelEnvironment = config.get(
        "voice_language_model_model_environment", "");
    const agent::boolean modelEnvironmentConfigured =
        static_cast<agent::boolean>(modelEnvironment.empty() == false);
    const agent::cstring configuredModel = modelEnvironmentConfigured ?
        std::getenv(modelEnvironment.c_str()) : nullptr;
    ...
    const agent::string modelApiKeyEnvironment = config.get(
    ...
    const agent::cstring configuredApiKey = apiEnvironmentConfigured ?
        std::getenv(modelApiKeyEnvironment.c_str()) : nullptr;
    agent::string modelApiKey = configuredApiKey == nullptr ?
        agent::string{} : agent::string(configuredApiKey);
    agent::string maximumOutputTokensText = config.get(
    ...
    if (rollyProfile)
    {
        profileApiKeyEnvironment = config.get(
            "voice_active_car_api_key_environment", "OPENAI_API_KEY");
        ...
        maximumOutputTokensText = config.get(
            "voice_active_car_maximum_output_tokens", "1024");
    }
    ...
    {
        profileApiKeyEnvironment = config.get(
            "gpt_car_api_key_environment", "OPENAI_API_KEY");
    ...
    {
        XHAL_THROW_RUNTIME_ERROR_DETAIL(profileApiKeyEnvironment,
            " must be set for the selected voice-active mode");
    }
    ...
    const agent::uint32 modelTimeoutMs = parseUnsigned(config.get(
    ...
    hal::XWalkTextToSpeechPiper piper(
        piperExecutable, piperPlaybackExecutable, piperModel);
    ...
    hal::XWalkTextToSpeech textToSpeech(
        boardControl, textToSpeechContext, textToSpeechCallback);
    hal::XWalkLanguageModelHttp modelBackend(modelDialect, modelEndpoint,
        modelName, modelApiKey, modelTimeoutMs, maximumOutputTokens);
    ...
    const agent::string cameraExecutable = csiSelected ?
        config.get("camera_csi_executable", "rpicam-still") :
        config.get("camera_usb_executable", "ffmpeg");
    hal::XWalkCameraLinux cameraBackend(cameraConnection, cameraExecutable,
        config.get("camera_usb_device", "/dev/video0"));
    hal::XWalkCameraConfiguration cameraConfiguration;
    cameraConfiguration.widthPixels = parseUnsigned(config.get(
        "camera_width", "640"), "camera_width", 7'680U);
    cameraConfiguration.heightPixels = parseUnsigned(config.get(
        "camera_height", "480"), "camera_height", 4'320U);
    cameraConfiguration.timeoutMs = parseUnsigned(config.get(
        "camera_timeout_ms", "5000"), "camera_timeout_ms", 300'000U);
    hal::XWalkCamera camera(
        &cameraBackend, cameraBackend.callback(), cameraConfiguration);
    XWalkCameraCapture cameraCapture(camera,
        config.get("camera_output", "/tmp/xwalk-voice-image.jpg"));
