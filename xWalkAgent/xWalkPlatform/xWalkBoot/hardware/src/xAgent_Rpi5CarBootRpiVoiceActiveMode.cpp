 ...
 */
agent::int32 XWalkBootRpi::runVoiceActiveMode(
    agent::uint8 mode, agent::contextpointer context,
    bootapplicationcallback callback, hal::XWalkConfigStore &config,
    hal::XWalkBoardControl &boardControl, XWalkPicarx &picarx,
    agent::stringview gpioDevice, agent::stringview gpioChipName,
    agent::stringview gpioChipLabel, agent::uint32 minimumGpioLineCount,
  ...
  const agent::string captureDevice =
      config.get("voice_capture_device", "default");
  const agent::string playbackDevice =
  ...
  const agent::string espeakExecutable =
      config.get("voice_espeak_executable", "espeak-ng");
  const agent::string espeakVoice = config.get("voice_espeak_voice", "en");
  const agent::string piperExecutable =
      config.get("voice_piper_executable", "piper");
  const agent::string piperPlaybackExecutable =
      config.get("voice_piper_playback_executable", "aplay");
  const agent::string piperModel = config.get(
      "voice_active_car_gpt_piper_model", XWalkVoiceActiveCarGpt::SPEECH_VOICE);

  agent::string modelProvider =
      config.get("voice_language_model_provider", "ollama");
  ...
  const agent::string modelEnvironment =
      config.get("voice_language_model_model_environment", "");
  ...
  const agent::cstring configuredModel =
      modelEnvironmentConfigured ? std::getenv(modelEnvironment.c_str())
                                 : nullptr;
  agent::string modelName =
      configuredModel == nullptr
          ? config.get("voice_language_model_model",
                       config.get("voice_ollama_model", ""))
          : agent::string(configuredModel);
  const agent::string modelApiKeyEnvironment =
      config.get("voice_language_model_api_key_environment", "");
  ...
  const agent::cstring configuredApiKey =
      apiEnvironmentConfigured ? std::getenv(modelApiKeyEnvironment.c_str())
                               : nullptr;
  agent::string modelApiKey = configuredApiKey == nullptr
                                  ? agent::string{}
                                  : agent::string(configuredApiKey);
  agent::string maximumOutputTokensText =
      config.get("voice_language_model_maximum_output_tokens", "1024");

  agent::string profileApiKeyEnvironment;
  const agent::boolean rollyProfile =
      static_cast<agent::boolean>(mode == XWALK_BOOT_VOICE_ACTIVE_CAR_REQ);
  const agent::boolean buddyProfile =
      static_cast<agent::boolean>(mode == XWALK_BOOT_VOICE_ACTIVE_CAR_GPT_REQ);
  if (rollyProfile) {
    profileApiKeyEnvironment =
        config.get("voice_active_car_api_key_environment", "OPENAI_API_KEY");
    modelEndpoint = config.get("voice_active_car_endpoint",
                               XWalkVoiceActiveCar::MODEL_ENDPOINT);
    modelName =
        config.get("voice_active_car_model", XWalkVoiceActiveCar::MODEL_NAME);
    maximumOutputTokensText =
        config.get("voice_active_car_maximum_output_tokens", "1024");
  } else if (buddyProfile) {
    profileApiKeyEnvironment = config.get(
        "voice_active_car_gpt_api_key_environment", "OPENAI_API_KEY");
    modelEndpoint = config.get("voice_active_car_gpt_endpoint",
                               XWalkVoiceActiveCarGpt::MODEL_ENDPOINT);
    modelName = config.get("voice_active_car_gpt_model",
                           XWalkVoiceActiveCarGpt::MODEL_NAME);
    maximumOutputTokensText =
        config.get("voice_active_car_gpt_maximum_output_tokens", "1024");
  } else {
    profileApiKeyEnvironment =
        config.get("gpt_car_api_key_environment", "OPENAI_API_KEY");
    ...
    maximumOutputTokensText =
        config.get("gpt_car_maximum_output_tokens", "1024");
  }
  const agent::cstring profileApiKey =
      std::getenv(profileApiKeyEnvironment.c_str());
  const agent::boolean profileApiKeyMissing = static_cast<agent::boolean>(
      (profileApiKey == nullptr) || (profileApiKey[0U] == '\0'));
  if (profileApiKeyMissing) {
    const std::string exceptionMessage =
        std::string(profileApiKeyEnvironment)
            .append(" must be set for the selected voice-active mode");
    XWALK_RPIAGENT_ERROR(XWALK_RUNTIME, exceptionMessage);
  }
  modelProvider = "openai";
  modelApiKey = profileApiKey;

  const agent::uint32 maximumOutputTokens = parseUnsigned(
      maximumOutputTokensText, "voice_language_model_maximum_output_tokens",
      XHAL_RPI5CAR_LANGUAGE_MODEL_HTTP_MAXIMUM_OUTPUT_TOKENS);
  const agent::uint32 modelTimeoutMs =
      parseUnsigned(config.get("voice_language_model_timeout_ms", "120000"),
  ...
  hal::XWalkSpeechToTextAlsa speechToTextBackend(captureDevice, &recognizer,
                                                 recognizer.operations());
  hal::XWalkSpeechToText speechToText(&speechToTextBackend,
                                      speechToTextBackend.callbacks());
  ...
  hal::XWalkTextToSpeechAlsa textToSpeechBackend(audioBackend, &espeak,
                                                 espeak.operations());
  hal::XWalkTextToSpeechPiper piper(piperExecutable, piperPlaybackExecutable,
                                    piperModel);
  agent::contextpointer textToSpeechContext = &textToSpeechBackend;
  hal::texttospeechspeakcallback textToSpeechCallback =
      textToSpeechBackend.callback();
  if (buddyProfile) {
    textToSpeechContext = &piper;
    textToSpeechCallback = piper.callback();
  }
  hal::XWalkTextToSpeech textToSpeech(boardControl, textToSpeechContext,
                                      textToSpeechCallback);
  hal::XWalkLanguageModelHttp modelBackend(modelDialect, modelEndpoint,
                                           modelName, modelApiKey,
                                           modelTimeoutMs, maximumOutputTokens);
  hal::XWalkLanguageModel languageModel(&modelBackend,
                                        modelBackend.callbacks());
  ...
  hal::XWalkVoiceAssistant voiceAssistant(speechToText, languageModel,
                                          textToSpeech, assistantConfiguration);
  ...
  XWalkSelfDrive selfDrive(
      picarx, music, nullptr, &selfDriveDelayMilliseconds, nullptr,
  ...
  hal::XWalkGpioLinux ledBackend(gpioDeviceValue.c_str(), gpioChipNameValue,
                                 gpioChipLabelValue, minimumGpioLineCount);
  hal::XWalkGpio ledGpio(&ledBackend, gpioCallbacks,
                         config.get("hardware_status_led_pin", "LED"));
  hal::XWalkLed statusLed(ledGpio);
  const hal::XWalkCameraConnection cameraConnection =
      hal::XWalkCamera::connectionFromString(
          config.get("camera_connection", "csi"));
  const agent::boolean csiSelected = static_cast<agent::boolean>(
      cameraConnection == hal::XWalkCameraConnection::Csi);
  const agent::string cameraExecutable =
      csiSelected ? config.get("camera_csi_executable", "rpicam-still")
                  : config.get("camera_usb_executable", "ffmpeg");
  hal::XWalkCameraLinux cameraBackend(
      cameraConnection, cameraExecutable,
      config.get("camera_usb_device", "/dev/video0"));
  hal::XWalkCameraConfiguration cameraConfiguration;
  cameraConfiguration.widthPixels =
      parseUnsigned(config.get("camera_width", "640"), "camera_width", 7'680U);
  cameraConfiguration.heightPixels = parseUnsigned(
      config.get("camera_height", "480"), "camera_height", 4'320U);
  cameraConfiguration.timeoutMs = parseUnsigned(
      config.get("camera_timeout_ms", "5000"), "camera_timeout_ms", 300'000U);
  hal::XWalkCamera camera(&cameraBackend, cameraBackend.callback(),
                          cameraConfiguration);
  XWalkCameraCapture cameraCapture(
      camera, config.get("camera_output", "/tmp/xwalk-voice-image.jpg"));
