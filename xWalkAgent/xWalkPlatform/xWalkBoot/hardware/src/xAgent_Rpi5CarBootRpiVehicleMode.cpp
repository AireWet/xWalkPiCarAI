 ...
 * @param[in,out] boardControl Caller-owned board controller valid through this call.
 * @param[in,out] picarx Caller-owned PiCar-X coordinator valid through this call.
    ...
    const hal::XWalkGpioCallbacks& gpioCallbacks)
{
    ...
    {
        return runVoiceActiveCar(context, callback, config, boardControl,
            picarx, gpioDevice, gpioChipName, gpioChipLabel,
            minimumGpioLineCount, gpioCallbacks);
    }
    else if (selectedMode == XWALK_BOOT_VOICE_ACTIVE_CAR_GPT_REQ)
    {
        return runVoiceActiveCarGpt(context, callback, config, boardControl,
            picarx, gpioDevice, gpioChipName, gpioChipLabel,
            minimumGpioLineCount, gpioCallbacks);
    }
    else if (selectedMode == XWALK_BOOT_GPT_CAR_REQ)
    {
        return runGptCar(context, callback, config, boardControl, picarx,
    ...
    XHAL_THROW_LOGIC_ERROR("xWalkBoot vehicle mode is not supported");
}
