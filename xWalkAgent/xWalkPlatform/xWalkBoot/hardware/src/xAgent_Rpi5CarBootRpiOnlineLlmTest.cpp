...
agent::int32 XWalkBootRpi::runOnlineLlmTest(agent::contextpointer context,
  ...
  const agent::string apiKeyEnvironment =
      config.get("online_llm_api_key_environment", "OPENAI_API_KEY");
  const agent::cstring apiKey = std::getenv(apiKeyEnvironment.c_str());
  const agent::boolean apiKeyMissing =
      static_cast<agent::boolean>((apiKey == nullptr) || (apiKey[0U] == '\0'));
  if (apiKeyMissing) {
    const std::string exceptionMessage =
        std::string(apiKeyEnvironment)
            .append(" must be set for online-llm-test");
  ...
  hal::XWalkLanguageModelOllama modelBackend(
      ...
      config.get("online_llm_endpoint",
                 "https://api.openai.com/v1/chat/completions"),
      config.get("online_llm_model", "gpt-4o"), apiKey);
