...
{
    const agent::string apiKeyEnvironment = config.get(
        "online_llm_api_key_environment", "OPENAI_API_KEY");
    const agent::cstring apiKey = std::getenv(apiKeyEnvironment.c_str());
    const agent::boolean apiKeyMissing = static_cast<agent::boolean>(
        (apiKey == nullptr) || (apiKey[0U] == '\0'));
    if (apiKeyMissing)
    {
        XHAL_THROW_RUNTIME_ERROR_DETAIL(apiKeyEnvironment,
            " must be set for online-llm-test");
    }
