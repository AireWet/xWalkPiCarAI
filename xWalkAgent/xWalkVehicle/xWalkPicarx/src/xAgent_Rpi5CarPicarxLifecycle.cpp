 ...
 * @brief       Implements PiCar-X coordinator lifecycle and configuration loading.
 *
 * @details
 * Binds caller-owned HAL dependencies, parses persisted calibration, and initializes safe actuator positions.
 *
 ...
 */
XWalkPicarx::XWalkPicarx(hal::XWalkMotors& motors, hal::XWalkServo& directionServo,
    ...
    : motorsObject(&motors), directionServoObject(&directionServo), cameraPanServoObject(&cameraPanServo),
      cameraTiltServoObject(&cameraTiltServo),
      grayscaleObject(&grayscale), ultrasonicObject(&ultrasonic), configStoreObject(&configStore)
{
    loadConfiguration();
}
...
{
    directionCalibrationDegreesValue = parseFloat(configStoreObject->get("picarx_dir_servo", "0"),
        "direction calibration");
    cameraPanCalibrationDegreesValue = parseFloat(configStoreObject->get("picarx_cam_pan_servo", "0"),
        "camera pan calibration");
    cameraTiltCalibrationDegreesValue = parseFloat(configStoreObject->get("picarx_cam_tilt_servo", "0"),
        "camera tilt calibration");
    configuredMaximumMotorOutputPercentValue = parseFloat(configStoreObject->get(
        "picarx_max_motor_output_percent", "20"), "maximum motor output");
    if ((configuredMaximumMotorOutputPercentValue < 0.0) ||
        (configuredMaximumMotorOutputPercentValue > 100.0))
    {
        XHAL_THROW_OUT_OF_RANGE("maximum motor output must be between 0 and 100 percent");
    }
    const agent::string calibrationVerifiedText = configStoreObject->get(
    ...
    {
        XHAL_THROW_INVALID_ARGUMENT("picarx calibration verification must be true or false");
    }
    maximumMotorOutputPercentValue = calibrationVerifiedValue ?
        configuredMaximumMotorOutputPercentValue :
        ((configuredMaximumMotorOutputPercentValue < 20.0) ?
            configuredMaximumMotorOutputPercentValue : 20.0);

    const agent::float64 motorSpeedCalibration = parseFloat(configStoreObject->get(
        "picarx_motor_speed_calibration", "0"), "motor speed calibration");
    if ((motorSpeedCalibration < -100.0) || (motorSpeedCalibration > 100.0))
    {
        XHAL_THROW_OUT_OF_RANGE(
            "motor speed calibration must be between -100 and 100 percentage points");
    }
    ...
    const agent::fixedarray<agent::int32, 2U> motorDirections = parseMotorDirections(
    ...
    const hal::linetrackervalues lineReferences = parseReferences(
        configStoreObject->get("line_reference", "[1000, 1000, 1000]"), "line references");
    cliffReferenceValues = parseReferences(
        configStoreObject->get("cliff_reference", "[500, 500, 500]"), "cliff references");
