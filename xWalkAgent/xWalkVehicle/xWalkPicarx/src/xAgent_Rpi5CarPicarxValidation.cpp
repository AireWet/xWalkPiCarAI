 ...
 */
agent::float64 XWalkPicarx::constrain(agent::float64 value,
                                      agent::float64 minimum,
                                      agent::float64 maximum) {
  const agent::boolean valueNotFinite =
      static_cast<agent::boolean>(!XHAL_IS_FINITE(value));
  if (valueNotFinite) {
 ...
 */
agent::float64 XWalkPicarx::parseFloat(agent::stringview text,
                                       agent::cstring name) {
  ...
  const agent::boolean parsedLengthTextValueInvalid =
      static_cast<agent::boolean>((parsedLength != text.size()) ||
 ...
 */
hal::linetrackervalues XWalkPicarx::parseReferences(agent::stringview text,
                                                    agent::cstring name) {
  const agent::boolean textInvalid = static_cast<agent::boolean>(
      (text.size() < 5U) || (text.front() != '[') || (text.back() != ']'));
  if (textInvalid) {
    const std::string exceptionMessage =
        std::string(name).append(" must be a three-element list");
    XWALK_RPIAGENT_ERROR(XWALK_INVAL, exceptionMessage);
  }
  ...
  for (agent::uint32 index = 0U; index < 3U; ++index) {
    ...
    agent::size parsedLength{};
    const agent::float64 parsed =
        hal::common::parseFloat64(text.substr(position), parsedLength);
    const agent::float64 rounded = XHAL_ROUND_NEAREST(parsed);
    const agent::boolean parsedRoundedInvalid = static_cast<agent::boolean>(
    ...
    if (parsedRoundedInvalid) {
      const std::string exceptionMessage =
          std::string(name).append(" must contain signed integers");
      XWALK_RPIAGENT_ERROR(XWALK_INVAL, exceptionMessage);
    }
    ...
    const agent::boolean positionTextExpectedInvalid =
        static_cast<agent::boolean>((position >= text.size()) ||
                                    (text[position] != expected));
    if (positionTextExpectedInvalid) {
      const std::string exceptionMessage =
          std::string(name).append(" must contain exactly three values");
      XWALK_RPIAGENT_ERROR(XWALK_INVAL, exceptionMessage);
    }
    ++position;
  }
  ...
  if (positionTextDifferent) {
    const std::string exceptionMessage =
        std::string(name).append(" contains trailing data");
    XWALK_RPIAGENT_ERROR(XWALK_INVAL, exceptionMessage);
  }
  return values;
}
 ...
 */
agent::fixedarray<agent::int32, 2U>
XWalkPicarx::parseMotorDirections(agent::stringview text) {
  const agent::boolean motorDirectionsTextInvalid = static_cast<agent::boolean>(
      (text.size() < 5U) || (text.front() != '[') || (text.back() != ']'));
  if (motorDirectionsTextInvalid) {
    XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                         "motor directions must be a two-element list");
  }
  ...
  for (agent::uint32 index = 0U; index < 2U; ++index) {
    ...
    const agent::boolean separatorMissing = static_cast<agent::boolean>(
        (position >= text.size()) || (text[position] != expected));
    if (separatorMissing) {
      XWALK_RPIAGENT_ERROR(XWALK_INVAL,
                           "motor directions must contain exactly two values");
    }
    ++position;
  }
 ...
 */
agent::string
XWalkPicarx::formatReferences(const hal::linetrackervalues &values) {
