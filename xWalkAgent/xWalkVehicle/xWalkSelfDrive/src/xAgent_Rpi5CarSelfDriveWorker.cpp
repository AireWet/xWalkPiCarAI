...
void XWalkSelfDrive::actionLoop() noexcept
{
    ...
    {
        const agent::boolean workerRunning =
...

        {
            ...
            else if (statusValue == XWalkSelfDriveStatus::Actions)
            {
                ...
                hasLastStatus = true;
                const agent::boolean actionQueueEmpty =
                    static_cast<agent::boolean>(
                        actionQueue.empty());
                if (actionQueueEmpty)
                {
        ...
        const agent::boolean actionAvailable =
            static_cast<agent::boolean>(
                !action.empty());
        if (actionAvailable)
        {
            static_cast<void>(doAction(action));
            const agent::boolean actionFailed =
                static_cast<agent::boolean>(
                    operationFailedValue.load());
            if (actionFailed)
            {
                agent::mutexlock lock(stateMutex);
                statusValue = XWalkSelfDriveStatus::Standby;
...
{
    const agent::boolean actionSupportedNotMatched =
        static_cast<agent::boolean>(
